...
const BOH_FLOORPLAN=(function(){
  ...
  const ZONES={
    ...
    /* These two ended in 'service' rather than 'bath', so they were never part of the
       ...
       a concourse is for. */
    /* *** AND THE DRY STORE, 9/13, [back of house]. *** bohemia_economy's header has
       said since 7/19 that downtown matters for its "deep casino/resort dry stores --
       THE reason downtown matters", and a casino is this zone. `leisure` was the one
       goods-heavy zone with no store in its list: the single room the economy is built
       around was the one room nobody laid out.
       REUSE-FIRST, and nothing new is drawn: `stockroom` already exists, retail already
       gets one, and bohemia_furnish already fills it wall to wall with racking and
       pallet stacks. It sits with the kitchen because that is where a dry store is --
       back of house, behind the floor, beside the thing it feeds. */
    leisure:    {roles:['concourse','counter','kitchen','stockroom','locker','restroom','service'],bulk:'concourse',minRoom:5},
