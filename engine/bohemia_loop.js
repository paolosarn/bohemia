...
(function (root, factory) {
  ...
  function makeQuestManager(opts) {
    ...
    /* ---- POSTURE: THE THIRD EFFECT, AUTHORED SINCE 7/25 AND READ BY NOTHING ----
       ...
       this instant. */
    /* OPTIONAL DEPENDENCY, resolved lazily on purpose. bohemia_belonging owns the
       one writer for the belonging count; the loop must not hard-require it,
       because a bare boot (tests, headless gates, the legacy carriers) has no
       surface modules and the world bridge cannot be blocked on one. Resolved at
       CALL time so browser load order does not matter either. */
    function belongingModule() {
      try {
        if (typeof require !== 'undefined') return require('./bohemia_belonging.js');
      } catch (_e) { /* not present on this carrier: fall through to the global */ }
      const g = (typeof globalThis !== 'undefined') ? globalThis : null;
      return (g && g.BohemiaBelonging) || null;
    }
    function applyWorldEffects(rt) {
      ...
      Object.keys(s.faction || {}).forEach(function (fid) {
        ...
        if (delta > 0 && save && save.meta) {
          /* ONE WRITER. The run moves this same number when you do the peripheral
             act on somebody's card, and two callers with two shapes is how a
             count starts disagreeing with itself -- so both go through
             bohemia_belonging's record(), which also folds the three spellings of
             a faction into one key. Falls back to the plain increment on a bare
             boot where the module is not loaded, so the world bridge is never
             blocked on a surface module. */
          const BEL = belongingModule();
          if (BEL && typeof BEL.record === 'function') BEL.record(save, real, null);
          else {
