...
(function(root){
  ...
  // love, NOT the auto-factory.
  // BUILT BY HAND, 8/18. That reservation was never "leave them blank forever", it was
  // "no auto-factory" — and for four months it was read as the first thing, so the three
  // biggest gaming types in the valley (resort 118 cells, strip 81, casino 5) generated
  // NOTHING and rendered as flat coloured diamonds on the map. These three modules ARE the
  // individual hand-crafted love: each researched off real Las Vegas site plans, each with
  // its own legend, dossier and layering, and each a different building from the other two
  ...
  var DISTGEN = {
    ...
    /* __THE_WELCOME_SIGN__ -- the Welcome sign, 9/12/26, COOK. Registered here for the same
    ...
    sign:       { mod:KIT.get('sign'),       foot:function(r){return r.footprints;}, zone:'leisure' },
  ...
  var SURFACEGEN = {
    ...
    /* LAS VEGAS BOULEVARD (8/18). `strip` has been in ROADSET since the overmap was
       written — every district beside it has always treated it as the street it fronts
       onto — and it has been in SURFACEGEN never, so all 81 cells of the resort corridor
       generated bare ground. It is a network tile like the arterial, not a lot: eight
       lanes, a palm median, a promenade at the back of curb, and the enclosed pedestrian
       bridges at the crossings. */
    strip:    { mod:STR, zone:'default' },
  ...
  /* THE RELAY LIVES IN THE OVERMAP NOW (8/21). It was here, and here is a module
     THE PAGE DOES NOT CARRY: slices/BOHEMIA_CITY_WORLD.html inlines the overmap and
     the district kit and never world.js, so the relay was real in the model and
     absent from the surface a body walks. Measured before the move: 357 stranded
     pockets, 541 cells never touching a street, 257 of them suburb -- and fixing the
     relay here moved walked_surface reachability by exactly ZERO cells, because the
     page had never had it in the first place.
     ONE CANONICAL BODY (ENGINE SYNC LAW). world.js supplies its own vocabulary --
     which districts are auto-built, and what counts as the same family -- and the
     page supplies its own; neither keeps a copy of the algorithm. */
  function buildLandlockConnect(m){
    return OM.landlockConnect(m, {
      isBuilt: function(d){ return !!DISTGEN[d]; },   // only real auto-factory districts relay
      familyOf: familyOf,
    });
  }
