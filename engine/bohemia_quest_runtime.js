...
function freshState(Q){
  var s={ stage:null, flags:{}, knows:{}, has:{}, roles:{}, faction:{}, bonds:{},
          ...
          doneTags:[], advanceTerritory:false, reward:{} };
...
Runtime.prototype._exec=function(text){
  ...
  switch(verb){
    ...
    case 'bond':    if(p[1]){ s.bonds[p[1]]=(s.bonds[p[1]]||0)+num(p[2]);
                    ...
                    break;
    /* CONTINUITY IS NOT ONLY BONDS (8/12). Paolo 8/7, ruling A: "a bond built in one
       quest opens a door in another. Continuity is the dynasty." Bonds were wired into
       the shared ledger that day and FACTION STANDING WAS NOT, so it lived only in the
       quest's own state -- and a quest's state dies with the quest. Measured 8/12 across
       a played day boundary: day 1 ended TRADES +8, day 2 opened {} . In a valley whose
       whole spine is factions, everything you did to one was forgotten by morning.
       Same shape as bonds, and simpler: a faction's NAME is already its identity in
       every quest, so there is no personKey to derive. */
    case 'faction': if(p[1]){ s.faction[p[1]]=(s.faction[p[1]]||0)+num(p[2]);
                      this._remember('faction',p[1],num(p[2])); } break;
    case 'faction_posture': if(p[1]){ s.posture[p[1]]=(s.posture[p[1]]||0)+num(p[2]);
                      this._remember('posture',p[1],num(p[2])); } break;
    ...
    /* WHAT THE JOB PAYS, SAID BY THE JOB (8/12). Paolo ruled it on 8/11, asked what a
       day's work should be worth: "Whatever currency the quest decida to give." The
       payday bridge was built that day to honour it -- questReward() reads
       questState.reward and pays exactly that -- and THE .bq LANGUAGE HAD NO WAY TO
       SAY IT. A quest could not declare a reward, so every finished job fell through
       to the empty global table and got NO_RULING. The ruling was made, the bridge
       was built, and the sentence could not be written.
       IT LIVES ON THE STAGE, not in a header, and that is his ruling too: the reward
       belongs to the OUTCOME. A quiet fix and a public spectacle are different jobs
       and should not pay the same, and the completing stage is already where a quest
       says what happened.
         @DO pay resources 3
       AMOUNTS ARE CONTENTS. Nothing here invents one: this is the verb, and what any
       quest actually pays is written in that quest's own file with the rest of its
       canon, which is exactly where he put it. */
    case 'pay': if(p[1]&&p[2]!=null){ s.reward=s.reward||{};
                  s.reward[p[1]]=(s.reward[p[1]]||0)+num(p[2]); } break;
    case 'play': s.log.push('play '+p.slice(1).join(' ')); break;
    default: s.log.push('UNHANDLED_DO: '+text);
  }
};

/* THE VALLEY'S MEMORY OF WHAT YOU DID TO IT. Writes a move into the shared ledger
   and keeps the REASON with it -- the completing stage's own @LOG line, so anything
   that shows your standing later shows the quest's words for why it moved and never
   prose written about it. No shared ledger means this is a no-op, which keeps a
   runtime built the old way bit-for-bit unchanged. */
Runtime.prototype._remember=function(kind,who,delta){
  if(!this.shared||!who||!delta) return;
  var book=this.shared[kind]||(this.shared[kind]={});
  book[who]=(book[who]||0)+delta;
  var log=this.shared.log||(this.shared.log=[]);
  log.push({ kind:kind, who:who, d:delta, why:this._why||null,
             quest:(this.Q&&(this.Q.title||this.Q.id))||null });
  if(log.length>200) log.shift();       /* a memory, not a hoard */
};
...
Runtime.prototype.setStage=function(n){
  ...
  if(st){
    ...
    this._why=st.log||null;      /* the reason a standing moved is the stage's own line */
...
/* what the valley remembers about a faction, counting every quest that moved it */
Runtime.prototype.standingWith=function(name){
  var here=(this.state.faction||{})[name];
  var there=(this.shared&&this.shared.faction)?this.shared.faction[name]:undefined;
  if(here==null&&there==null) return null;
