// BOHEMIA FACTION TOWNS (9/5/26, WORLD lane) — FACTION-TOWNS.
//
// Paolo 9/4, LOCKED (laws/BOHEMIA_ADDENDUM_FACTION_TOWNS_9_4_26.md):
//   "each part of Vegas is owned by a faction and that's where you can do all your
//    trading or whatever they'll have different buildings supporting them...
//    there's different sizes of cities so maybe the more bigger or more prominent
//    factions kind of feel like strong fortress parts... and then for the smaller
//    ones like the colorful maybe they just have... not a lot of goods not a lot of
//    buildings not a lot of good quests and it's just smaller."
//
// A faction's SEAT is where you trade. Three tiers: FORTRESS / TOWN / CAMP.
//
// ============================================================================
// THE TIER IS DERIVED, NEVER TYPED.
// His own faction graph already carries act1_power and act3_power for every
// faction (BOHEMIA_faction_graph.json, GDD v2 s9). Rank them, cut in thirds. So a
// FORTRESS in act 1 can be a CAMP by act 3 and the reverse, which is the CENTURY
// RULE with no new field and no second table to keep in step.
// THE DERIVATION CHECKS OUT AGAINST HIS OWN WORDS, which is why it is this and not
// a hand-written list: he named "the colorful" as the small one, and Colorful is
// act1_power 1 of 14 -- the bottom of the graph he wrote months earlier. Nothing
// was tuned to make that land.
// Every tier ships `draft:true`. Moving any faction up or down is one edit.
//
// ============================================================================
// *** WHY THIS MODULE OWNS THE SEAT RULE, AND IT IS A BUG FIX, NOT A FEATURE ***
//
// MEASURED 9/5, on one seed, before writing any of this:
//     the loop's district list   3,919 cells
//     the city's district list   4,009 cells
//     the 14 seats they produce  DIFFERENT
// Both are the same 96x96 valley on seed "bohemia". bohemia_loop.js's bootFactions
// strides over cells passing bohemia_world.js's `isAutoDistrict`; the walked
// surface cannot even load that module (it carries bohemia_overmap.js instead) and
// its own notion of a real district is bohemia_cityedit.js's `cat(d)==='sand'`.
// Ninety cells of disagreement, and therefore two different answers to "where does
// the Mob live".
//
// Nothing had noticed because nothing had ever asked the walked surface the
// question: the city's own FACTION_ASSIGN table is `{}` and its comment says so.
// The first thing that asked found the drift.
//
// SO THE RULE LIVES HERE, ONCE, AND CALLERS PASS THE MAP RATHER THAN THE ANSWER.
// districtsOf() reads the overmap through `cat`, which is the definition BOTH
// surfaces can reach, so the loop and the city can agree by construction instead of
// by coincidence. Same reason PRICES is built from the economy's own GOODS instead
// of a list typed by hand, and the same reason the street contract measures its
// connectors off built tiles instead of a declaration: two systems drift the day
// somebody edits one of them.
//
// ============================================================================
// MECHANISM MINE, CONTENTS HIS.
//   WHICH FACTION SITS WHERE IS HIS (MAP LAW: Claude never designs map layouts).
//   SEATS ships EMPTY. What derive() produces is a mechanical spread across
//   whatever the map already generated -- the same shape bootFactions has used
//   since it was written, and its own comment already argues why that is plumbing
//   rather than a layout decision. One line in SEATS moves a faction anywhere and
//   the override wins.
//   WHAT A TOWN SELLS is BB-WANTS' and BB-PRICE-PLACE's row, not this one. What is
//   here is HOW MANY goods a tier carries, which is his sentence "not a lot of
//   goods" turned into a count, taken off the economy's own list in the economy's
//   own order. No good is invented and no good is chosen.
//   WHAT ANYTHING COSTS is already ruled: everything is one battery (8/15 + 9/4).
//   A camp is not more expensive than a fortress, it is THINNER. Depth is the axis
//   his words actually name.
(function (root) {
  'use strict';

  var TIERS = ['fortress', 'town', 'camp'];

  /* HIS OVERRIDES. EMPTY, and it stays empty until he rules one:
       SEATS['Mob'] = { x: 61, y: 44 };
     An override wins over the derived seat and needs nothing else changed. */
  var SEATS = {};

  /* SAME DOOR FOR TIER. EMPTY:  TIER['Colorful'] = 'town'; */
  var TIER = {};

  function isSelectable(f) { return !!f && f.type === 'selectable'; }

  /* THE ROSTER, IN ONE ORDER, ALWAYS. Sorted by id so a seat is the same on every
     device and every load -- the same reason identity in the city is derived from
     three numbers rather than stored. */
  function selectable(graph) {
    var fs = (graph && graph.factions) || {}, out = [], k;
    for (k in fs) if (Object.prototype.hasOwnProperty.call(fs, k) && isSelectable(fs[k])) out.push(k);
    return out.sort();
  }

  /* ---------------------------------------------------------------------------
     THE TIER. Rank by the act's own power column, cut in thirds.
     A faction with no power number for this act keeps its act-1 rank, because a
     missing number is not a demotion -- `Custom` has act3_power null and is the
     player's own outfit, and dropping it to CAMP for a null would be a ruling.
     --------------------------------------------------------------------------- */
  function powerOf(f, act) {
    if (!f) return null;
    var p = (act === 3) ? f.act3_power : f.act1_power;
    if (p == null) p = f.act1_power;
    return (typeof p === 'number') ? p : null;
  }

  function tiers(graph, act) {
    var ids = selectable(graph), fs = (graph && graph.factions) || {};
    var ranked = ids.slice().sort(function (a, b) {
      var pa = powerOf(fs[a], act), pb = powerOf(fs[b], act);
      if (pa === pb) return a < b ? -1 : 1;          /* stable, never random */
      if (pa == null) return 1;
      if (pb == null) return -1;
      return pb - pa;                                 /* strongest first */
    });
    /* THIRDS, and the top third rounds UP so a fourteen-faction valley has five
       fortresses rather than four -- "the more prominent factions" is a group, not
       a podium. */
    var third = Math.ceil(ranked.length / 3);
    var out = {};
    for (var i = 0; i < ranked.length; i++) {
      var t = (i < third) ? 'fortress' : (i >= ranked.length - third ? 'camp' : 'town');
      var id = ranked[i];
      out[id] = {
        faction: id,
        tier: Object.prototype.hasOwnProperty.call(TIER, id) ? TIER[id] : t,
        ruled: Object.prototype.hasOwnProperty.call(TIER, id),
        rank: i + 1,
        power: powerOf(fs[id], act),
        act: act === 3 ? 3 : 1,
        draft: !Object.prototype.hasOwnProperty.call(TIER, id)
      };
    }
    return out;
  }

  /* ---------------------------------------------------------------------------
     THE DISTRICT LIST, READ OFF THE MAP RATHER THAN OFF A SECOND OPINION.
     `cat` is bohemia_cityedit's category function -- the game's own answer to "is
     this a real district you could put a building on". Row-major, which is the
     order bootFactions has always strided in.
     --------------------------------------------------------------------------- */
  /* AND A SEAT HAS TO FRONT A ROAD, because a market you cannot get to is not a
     market. His ruling calls a seat "where you can do all your trading", and the
     STREET-AWARE / DRIVABLE ACCESS LAW already says a plot is only served if the
     network reaches its curb.
     MEASURED, AND THIS IS WHY IT IS HERE RATHER THAN IN A CALLER: without it, seven
     faction seats across the gate's seed set landed on ground payday's own
     reachable() calls FALSE -- ground with no way in. Filtering in the CALLER was the
     obvious fix and it is the wrong one: payday's reachable() needs w.plot(), the
     walked surface has no plot API at all, so the loop would have nudged those seats
     and the city could not have, and the two would disagree again -- which is the
     exact drift this module was written to end. A ROAD NEIGHBOUR IS COMPUTABLE FROM
     `at` ALONE, so both surfaces get the same answer for the same reason. */
  function frontsRoad(m, cat, x, y) {
    var n = m.n | 0, d = [[1, 0], [-1, 0], [0, 1], [0, -1]];
    for (var i = 0; i < 4; i++) {
      var nx = x + d[i][0], ny = y + d[i][1];
      if (nx < 0 || ny < 0 || nx >= n || ny >= n) continue;
      var c = m.at(nx, ny);
      if (!c || !c.district) continue;
      var k = cat(c.district);
      if (k === 'road' || k === 'freeway') return true;
    }
    return false;
  }

  /* GROUND THAT NEVER CARRIES A BUILDING IS NOT A TOWN. His ruling says in its own
     words that WHAT SUPPORTS A TOWN IS BUILDINGS, and a solar array supports nothing:
     you cannot trade on a photovoltaic field or a runway.
     THIS LIST IS MEASURED, NOT IMAGINED, AND THE MEASUREMENT WAS WIDENED ONCE BECAUSE
     THE FIRST QUESTION WAS THE WRONG ONE.
     The first cut asked which kinds are ALWAYS empty and got four: airbase, airport,
     solar, strip. That still left two seats on ground with nothing on it -- because a
     seat lands on ONE cell, so what matters is not whether a kind is always empty but
     whether it can EVER be. A kind that is usually built and sometimes not will
     eventually put a market on nothing.
     Asked again, exhaustively, over every buildable kind across two seeds: EIGHT kinds
     have at least one plot with zero buildings and FORTY-NINE are built on every plot
     examined. All eight are out, and the unreachable-seat count went 7 -> 2 -> 0,
     re-measured at 84 seats over six seeds.
     It is also the reading that matches his ruling in plain words: a runway, a solar
     field, a fairway and a farm are not parts of a city you trade in.
     faction_towns_gate RE-MEASURES this and goes red naming the kind if any answer
     changes in either direction, so it cannot rot into a list somebody once believed. */
  var NOT_A_TOWN = { airbase: 1, airport: 1, datafort: 1, farm: 1,
                     golf: 1, solar: 1, speedway: 1, strip: 1 };

  function districtsOf(m, cat) {
    var out = [];
    if (!m || typeof m.at !== 'function' || !cat) return out;
    var n = m.n | 0;
    for (var y = 0; y < n; y++) {
      for (var x = 0; x < n; x++) {
        var c = m.at(x, y);
        if (!c || !c.district) continue;
        if (cat(c.district) !== 'sand') continue;
        if (NOT_A_TOWN[c.district]) continue;
        if (!frontsRoad(m, cat, x, y)) continue;
        out.push({ x: x, y: y, kind: c.district });
      }
    }
    return out;
  }

  /* ===========================================================================
     WHERE A FACTION LIVES. HIS OWN NOTES, READ.
     (9/5/26, FACTIONS lane, VAMILY row [faction homes] FACTION-SEATS, landing in
     THIS file because FACTION-TOWNS already moved "which cells count" here and a
     second placer somewhere else is the exact drift this file's header is about.)

     *** WHAT THIS REPLACED, AND IT IS WORSE THAN IT LOOKS. *** The old rule was
     "evenly strided, exactly as bootFactions has done since it was written". The
     district list is 100% ordered by y, so an even stride marches straight down
     the map, and the ids were SORTED. Measured on the real boot before this
     changed:

         correlation(alphabetical rank, seat y) = 0.9966

     Anarchists sat at y=2 and Volunteers at y=83 FOR NO REASON EXCEPT THEIR
     INITIALS. The alphabet was deciding the geography of the valley. And the
     kinds it landed on were nobody's home: the Church on a suburb, the Cartel on
     a solar farm, while the generated map holds a chapel, an arsenal, a police
     station, six dataforts and a hundred and eighteen resorts that nothing ever
     used. "It is the rule that already existed" was true and was not a defence.

     *** HIS NOTES ARE THE PLACEMENT AND THIS IS ONLY READING THEM. *** Every
     faction in BOHEMIA_faction_graph.json carries an authored `note`, and the
     notes name places outright: the Mob "Controls the Strip", the Homeless have a
     "Sewer/tunnel HQ", the Remnants are "Military/law-enforcement survivors incl
     Nellis", the Trades are "Best for vehicle/solar/construction". HOMES is that
     reading and nothing else -- every row quotes the words it rests on, so a
     wrong seat is a MISREADING somebody can check against his file, not a taste
     that has to be argued. MAP LAW is intact: this picks WHICH KIND of generated
     ground an outfit belongs on, it does not design a layout, and one line in
     SEATS still overrides any of it.

     WHERE HIS NOTE NAMES NO PLACE, THIS INVENTS NONE. Three factions say in his
     own words that they do not hold ground -- the Anarchists are "territorially
     inconsistent", the Colorful are "community-based not territorial", the Custom
     faction has "no preset philosophy" because it is the player's. They are
     absent from HOMES on purpose and fall through to spread, and the record lists
     them [PENDING Paolo]: a faction his canon calls non-territorial probably
     should not hold ground the way the Mob does, and that is a ruling.

     STRONGEST PICKS FIRST, USING HIS NUMBER. act1_power is canon, so the order is
     his too: the Remnants (14), Mob (13), Cartel (12) and Network (11) take their
     homes before the Volunteers (3) do, and a weak faction takes what is left.
     =========================================================================== */
  var HOMES = {
    // "Controls the Strip. Ownership/protection/tribute."
    Mob:        ['resort', 'casino', 'convention', 'downtown'],
    // "Military/law-enforcement survivors incl Nellis. The floor of civilization."
    Remnants:   ['arsenal', 'policestation', 'prison', 'firestation'],
    // "Only faction with a manufactured origin" / psyop
    Network:    ['datafort', 'radio', 'substation'],
    /* "Organized human predation. SUPPLY CHAINS MIRROR CARAVAN ROUTES."
       The scavenging ground first, then the SAME KINDS the Caravans want,
       because that second sentence says so in as many words. Measured on the
       shipped seed: not one boneyard, landfill, quarry or railyard in the valley
       fronts a road, so on the first four alone the Cartel -- his third-strongest
       faction, power 12 -- fell through to a random suburb. Loosening the road
       rule to rescue it would have cost the thing the WORLD lane measured
       (unreachable seats 7 -> 2 -> 0); reading his next sentence costs nothing.
       The Cartel outranks the Caravans, so it picks off that shared list first
       and the Caravans take the next one, which is what "mirror" looks like. */
    Cartel:     ['boneyard', 'landfill', 'quarry', 'railyard',
                 'storage', 'warehouse', 'terminal', 'swapmeet'],
    // "Neutral outside-world trade."
    Caravans:   ['truckstop', 'terminal', 'swapmeet', 'warehouse'],
    // "Best crisis organization." / evangelical
    Church:     ['chapel', 'cemetery', 'school'],
    // "Economic engine, compound-interest faction. Not military."
    Reds:       ['downtown', 'cityhall', 'courthouse', 'mall'],
    // "Environmentalist."
    Blues:      ['reservoir', 'watertreat', 'farm', 'park'],
    // "Skilled-worker guild... Best for vehicle/solar/construction."
    Trades:     ['industrial', 'solar', 'battery', 'warehouse'],
    // "Healthcare/education/food/knowledge."
    Volunteers: ['medical', 'campus', 'granary', 'school'],
    // "Sewer/tunnel HQ under the King Hobo."
    Homeless:   ['intake', 'pumpstation', 'reclaim', 'basin', 'wash'],
  };

  var MIN_APART = 6;          /* cells; two capitals on one block is not two capitals */
  /* AND OFF THE BOUNDARY ROWS, WHICH THE MAP GATE CAUGHT. The first cut of this
     put four capitals on the literal edge of the valley (y=0, y=3, y=89) and
     faction_between's own M2 went red: a base marker at the boundary is clipped
     by the map edge and painted 14 pixels against a floor of 20, which is
     "present" rather than "legible" -- the exact distinction that claim was
     written for. A capital on the boundary row is also just odd ground for a
     capital. INSET IS A PREFERENCE, NEVER A VETO: several kinds are unique in the
     whole valley (one arsenal, one truckstop, one granary), so if the only ground
     his note names is at the edge, HIS CANON WINS and the seat goes there. Same
     precedence as the spacing rule. */
  var EDGE_INSET = 5;

  /* TAKES EITHER SHAPE OF DISTRICT LIST. districtsOf() answers {x,y,kind} and the
     loop's own worldMap carries {pos:[x,y],id,kind}; one normaliser here beats two
     callers each remembering which one they hold. */
  function normSeat(d) {
    if (!d) return null;
    var p = d.pos || [d.x, d.y];
    if (p[0] == null || p[1] == null) return null;
    return { x: p[0], y: p[1], kind: d.kind || '?',
             id: (d.id != null) ? d.id : (p[0] + ',' + p[1]) };
  }

  /* THE ONE PLACER. Callers pass the map's districts and get back a seat per
     faction; nobody re-implements the choice. `ids` lets the loop seat every
     faction it loaded rather than only the selectable ones, without this function
     having two opinions about who exists. */
  function placeHomes(graph, districts, act, ids) {
    var out = {}, list = [], i;
    for (i = 0; i < (districts || []).length; i++) {
      var s = normSeat(districts[i]); if (s) list.push(s);
    }
    if (!list.length) return out;
    var fs = (graph && graph.factions) || {};
    var roster = (ids && ids.length) ? ids.slice() : selectable(graph);

    var byKind = {};
    for (i = 0; i < list.length; i++) {
      var k = list[i].kind;
      if (!byKind[k]) byKind[k] = [];
      byKind[k].push(list[i]);
    }

    /* THE VALLEY'S EDGE, ASKED OF THE LIST RATHER THAN TYPED. Callers hold the
       map; this only ever sees cells, so the boundary is the largest coordinate
       any district reached. Wrong only if a whole edge of the map generated no
       districts at all, in which case there is nothing there to sit on anyway. */
    var size = 0;
    for (i = 0; i < list.length; i++) {
      if (list[i].x + 1 > size) size = list[i].x + 1;
      if (list[i].y + 1 > size) size = list[i].y + 1;
    }

    var taken = {}, seats = [];
    function inland(d) {
      return d.x >= EDGE_INSET && d.y >= EDGE_INSET
          && d.x <= size - 1 - EDGE_INSET && d.y <= size - 1 - EDGE_INSET;
    }
    function farEnough(d) {
      for (var j = 0; j < seats.length; j++)
        if (Math.abs(seats[j][0] - d.x) + Math.abs(seats[j][1] - d.y) < MIN_APART) return false;
      return true;
    }
    /* AND A CAPITAL WITH NOBODY AROUND IS NOT A CAPITAL. (9/5, second pass.)
       The first cut picked the right KIND of ground and stopped there, which put
       the Homeless on the only intake in the valley and the Caravans on the only
       truckstop -- both real canon reads, both in empty corners. Who belongs to a
       faction is computed from proximity to its base, so a seat where nobody
       lives is a faction with no members, and faction_arc's coverage claims went
       red: five act mechanics have members in the valley and the walk could only
       reach three.
       So a candidate is scored by HOW MUCH CITY IS AROUND IT -- the count of
       districts within eight cells, which is the same number on both surfaces
       because it is read off the very list the caller passed. The loop's own
       districts carry a `zone` and the first version of this scored on that, but
       zone comes out of bohemia_world.js, which the walked surface cannot load at
       all -- scoring on it would have rebuilt the two-surfaces-disagree defect
       this whole file exists to stop. Cells are the shared currency.
       THIS DOES NOT OVERRIDE HIS NOTE. It chooses BETWEEN the sites his note
       allows, which is exactly the part he left open. Where a kind is unique
       there is nothing to choose and the faction still goes there. */
    var dens = {};
    function density(d) {
      if (dens[d.id] != null) return dens[d.id];
      var c = 0;
      for (var j = 0; j < list.length; j++) {
        var o = list[j];
        if (Math.abs(o.x - d.x) + Math.abs(o.y - d.y) <= 8) c++;
      }
      dens[d.id] = c;
      return c;
    }
    /* DETERMINISTIC AND NOT ALPHABETICAL: the same faction lands on the same
       ground in the same world every boot, but its initials have nothing to do
       with where. A capital that moves when you reload is not a capital. The
       hash only breaks ties between sites that are equally good. */
    function pickFrom(cands, fid) {
      if (!cands || !cands.length) return null;
      var h = 2166136261;
      for (var j = 0; j < fid.length; j++) { h ^= fid.charCodeAt(j); h = Math.imul(h, 16777619); }
      var salt = (h >>> 0);
      var best = null, bestScore = -1, edge = null, loose = null;
      for (var n = 0; n < cands.length; n++) {
        var d = cands[n];
        if (taken[d.id]) continue;
        if (!farEnough(d)) { if (!loose) loose = d; continue; }
        if (!inland(d)) { if (!edge) edge = d; continue; }
        var score = density(d) * 16
          + ((salt ^ Math.imul(d.x, 73856093) ^ Math.imul(d.y, 19349663)) & 15);
        if (score > bestScore) { bestScore = score; best = d; }
      }
      return best || edge || loose;               /* his canon beats both of my rules */
    }

    roster.sort(function (a, b) {
      var pa = powerOf(fs[a], act) || 0, pb = powerOf(fs[b], act) || 0;
      if (pb !== pa) return pb - pa;             /* strongest picks first, his number */
      return a < b ? -1 : 1;                     /* tie-break only */
    });

    for (i = 0; i < roster.length; i++) {
      var fid = roster[i];
      /* HIS OVERRIDE WINS OVER EVERYTHING, including his own notes. */
      if (Object.prototype.hasOwnProperty.call(SEATS, fid)) {
        var sx = SEATS[fid].x | 0, sy = SEATS[fid].y | 0;
        out[fid] = { x: sx, y: sy, kind: null, why: 'ruled', id: sx + ',' + sy, ruled: true };
        seats.push([sx, sy]);
        continue;
      }
      var wants = HOMES[fid] || [], d = null, why = null;
      for (var w = 0; w < wants.length && !d; w++) {
        d = pickFrom(byKind[wants[w]], fid);
        if (d) why = wants[w];
      }
      if (!d) {                                  /* no home in his note, or all taken */
        for (var n2 = 0; n2 < list.length && !d; n2++) {
          var c = list[(n2 * 7919) % list.length];
          if (!taken[c.id] && farEnough(c) && inland(c)) d = c;
        }
        why = d ? 'spread' : null;
      }
      if (!d) continue;
      taken[d.id] = 1; seats.push([d.x, d.y]);
      out[fid] = { x: d.x, y: d.y, kind: d.kind, why: why, id: d.id };
    }
    return out;
  }

  /* THE TOWN ROW: a seat from placeHomes, a tier from his power column. */
  function derive(graph, districts, act) {
    var ids = selectable(graph), ts = tiers(graph, act), out = [];
    var placed = placeHomes(graph, districts, act, ids);
    for (var i = 0; i < ids.length; i++) {
      var id = ids[i], p = placed[id];
      if (!p) continue;
      var ruled = !!p.ruled;
      var t = ts[id] || { tier: 'camp', draft: true };
      out.push({ faction: id, x: p.x, y: p.y, kind: p.kind || null, why: p.why || null,
                 tier: t.tier, rank: t.rank, power: t.power, act: t.act,
                 seatRuled: ruled, draft: !ruled || t.draft });
    }
    return out;
  }

  /* ---------------------------------------------------------------------------
     HOW DEEP A TOWN'S MARKET IS. His sentence, as a count.
     A CAMP IS NOT DEARER THAN A FORTRESS, IT IS THINNER -- everything costs one
     battery wherever you buy it (8/15 + 9/4), so the only honest axis his words
     give is how much a place has. Taken off the economy's own goods list, in the
     economy's own order: no good is invented and none is chosen by me. WHICH goods
     a particular town carries is BB-WANTS' row and belongs to its buildings.
     --------------------------------------------------------------------------- */
  var DEPTH = { fortress: 1, town: 2 / 3, camp: 1 / 3 };

  function goodsFor(tier, goods) {
    var all = goods || [];
    var f = Object.prototype.hasOwnProperty.call(DEPTH, tier) ? DEPTH[tier] : DEPTH.camp;
    var n = Math.max(1, Math.ceil(all.length * f));
    return all.slice(0, n);
  }

  /* the town you are standing in, and the one you could walk to. Chebyshev,
     because the valley is a grid and a body walks it in cells. */
  function nearest(towns, x, y) {
    var best = null, bd = Infinity;
    for (var i = 0; i < (towns || []).length; i++) {
      var t = towns[i];
      var d = Math.max(Math.abs(t.x - x), Math.abs(t.y - y));
      if (d < bd) { bd = d; best = t; }
    }
    return best ? { town: best, cells: bd } : null;
  }

  /* HOW BIG THE TOWN IS ON THE GROUND. A fortress spreads over its neighbours, a
     camp is one cell. This is the "supporting buildings" half of his ruling: the
     district cells inside the reach ARE the smithy and the dry store, and they are
     whatever the map already generated there. Nothing is placed. */
  var REACH = { fortress: 3, town: 2, camp: 1 };

  function townCells(town, m, cat) {
    var out = [];
    if (!town || !m || typeof m.at !== 'function') return out;
    var r = Object.prototype.hasOwnProperty.call(REACH, town.tier) ? REACH[town.tier] : 1;
    var n = m.n | 0;
    for (var y = town.y - r; y <= town.y + r; y++) {
      for (var x = town.x - r; x <= town.x + r; x++) {
        if (x < 0 || y < 0 || x >= n || y >= n) continue;
        var c = m.at(x, y);
        if (!c || !c.district) continue;
        if (cat && cat(c.district) !== 'sand') continue;
        out.push({ x: x, y: y, kind: c.district });
      }
    }
    return out;
  }

  /* ---------------------------------------------------------------------------
     WHO HOLDS THIS GROUND (9/5, BB-TURF).
     LIGHT=TERRITORY has been live code since 7/20 and the grid stamps an OWNER on
     every lit circuit -- but the owner is a CATEGORY, not a name. One circuit in
     five comes back owned by the generic word "faction", so the game already knew
     somebody held that block and could not say who. The seam test on the walked
     surface compares those words, which means today a Mob block and a Cartel block
     are the same block.
     THE NAME IS DERIVED FROM THE SEATS, WHICH ARE GEOGRAPHY THAT ALREADY EXISTS.
     A faction holds the ground around its own town, which is what "different parts
     of Vegas as different faction holdings" means in plain words, and it needs no
     canon: FACTION-TOWNS put the fourteen seats on the map and this reads them.
     A FORTRESS REACHES FURTHER THAN A CAMP, off the REACH table already above --
     his own "the more prominent factions feel like strong fortress parts", as a
     divisor rather than a new number. Ties go to the stronger faction, then to the
     name, so the answer is the same on every device and every load.
     WHO HOLDS WHAT IS STILL HIS: HOLDS ships EMPTY and an entry in it wins. */
  var HOLDS = {};        /* HOLDS['48,57'] = 'Mob';  -- empty, and it stays empty */

  /* =========================================================================
     EVERY DISTRICT HAS AN OWNER, AND THE BORDER RUNS ALONG SOMETHING YOU CAN
     SEE.  (9/6/26, FACTIONS lane, VAMILY row [who holds]
     EVERY-DISTRICT-HAS-AN-OWNER. Ruling: records/BOHEMIA_RULING_WHO_HOLDS_WHAT_
     9_5_26.md, decided by the coordinator 9/5; Paolo corrects it by playing.)

     WHAT WAS ALREADY TRUE. holderOf() above answers for any cell, so 100% of the
     valley already had an owner -- BB-TURF did that on 9/5 and this row does not
     redo it.

     WHAT WAS NOT. holderOf is a scaled Voronoi: nearest seat, divided by tier
     reach. That draws borders wherever two seats happen to balance, which is a
     diagonal line through the middle of a block. MEASURED on the shipped seed
     before this landed: of 709 places where two owners meet, only 466 ran along
     something a player can see -- 243 of them, one in three, cut straight across
     open buildable ground with nothing there to explain the edge.

     THE RESEARCH SAYS THAT IS BACKWARDS. Real armed-group territory does not run
     on invisible lines; it runs on landmarks you cannot miss -- highways, rail,
     washes, walls. Chicago's Dan Ryan Expressway splits the South Side factions.
     The ruling wrote it down as rule 3: "if you cannot see why the border is
     there, it is in the wrong place."

     SO THE UNIT OF OWNERSHIP IS NOT A CELL, IT IS A BLOCK. Cut the valley along
     everything a player can see -- road, freeway, rail, water, mountain, open
     desert -- and what is left are 465 blocks of buildable ground, each one ringed
     by visible edges. A block is owned whole. Then a border between two owners can
     only ever fall on the skeleton, because two buildable cells that touch are in
     the same block by construction. The property is STRUCTURAL, not tuned: there
     is no number here to nudge to make it come out right.

     AND THE STRONGEST TAKES THE BEST GROUND IT CAN REACH (rule 2), which needed a
     worth number, and the first two rulers were both wrong:
       - COUNT THE BUILDINGS: ranks trailer parks (26.9 per plot) and suburbs
         (19.9) above casinos (4) and resorts (4), because a resort is one enormous
         building and a trailer park is thirty tiny ones. It measures fragmen-
         tation. It would have made the trailers the richest ground in Las Vegas.
       - COUNT THE BUILT TILES ON THE CELL: puts `strip` at zero, because the strip
         district IS the boulevard and carries no buildings. It would have handed
         Las Vegas Boulevard to the weakest faction in the valley.
     The second one is right at the BLOCK level and wrong at the cell level: the
     Strip's boulevard cells sit in the same block as the 118 resort cells beside
     them, so the block scores enormous and the corridor rides along. Worth is
     summed over a block, never read off a cell, and that is the whole fix.
     ========================================================================= */

  /* HOW MUCH IS BUILT ON ONE PLOT OF EACH KIND, MEASURED, NOT RANKED BY TASTE.
     Built tiles (w*h summed over the plot's buildings), sampled eight plots per
     kind off the shipped valley. faction_turf_gate RE-MEASURES this and goes red
     naming the kind if any answer moves out of its band, the same contract
     NOT_A_TOWN carries above, so it cannot rot into a list somebody believes.
     The order it produces is the order the ruling names in words -- the Strip and
     downtown first (resort 12043, casino 11477, downtown 9624), then the ordinary
     city, then the industrial edge, then the desert (solar, airbase, airport and
     the boulevard itself at 0). That agreement is the reason to trust it. */
  var WORTH = {
    resort: 12043, casino: 11477, downtown: 9624, library: 8632, convention: 8496,
    fort: 8400, stadium: 7732, chapel: 7565, courthouse: 6693, medical: 6661,
    mall: 6431, school: 6011, industrial: 5030, campus: 4897, terminal: 4850,
    estate: 4118, commercial: 4103, policestation: 4054, town: 3990, suburb: 3950,
    storage: 3911, ballpark: 3911, gated: 3869, prison: 3708, jail: 3652,
    trailer: 3293, firestation: 3268, apartment: 3204, warehouse: 2550,
    pumpstation: 1860, truckstop: 1292, speedway: 896, minigp: 896, radio: 822,
    cemetery: 667, farm: 603, battery: 544, arsenal: 527, substation: 480,
    granary: 462, swapmeet: 455, drivein: 378, strat: 336, sphere: 336,
    highroller: 336, luxor: 336, sign: 336, waterpark: 282, golf: 226,
    datafort: 165, park: 164, solar: 0, airbase: 0, airport: 0, strip: 0
  };
  function worthOf(kind) {
    return Object.prototype.hasOwnProperty.call(WORTH, kind) ? WORTH[kind] : 0;
  }

  /* THE THINGS A PLAYER CAN SEE, which are the only legal places for a border.
     Read off cityedit's cat() -- the one definition both surfaces can reach --
     plus open desert, which is as plain an edge as a freeway. */
  var SEEN_EDGE = { road: 1, freeway: 1, rail: 1, water: 1, mount: 1, open: 1, none: 1 };

  /* CUT THE VALLEY INTO BLOCKS. 4-connected runs of buildable ground, each ringed
     by things you can see. Returns the id grid and the blocks themselves. */
  function blocksOf(m, cat) {
    var n = (m && m.n) | 0, out = { n: n, id: null, blocks: [] };
    if (!n || typeof m.at !== 'function' || !cat) return out;
    var id = new Int32Array(n * n); id.fill(-1);
    function edgeAt(x, y) {
      var c = m.at(x, y);
      if (!c || !c.district) return true;
      return !!SEEN_EDGE[cat(c.district)];
    }
    var blocks = [], stack = [];
    for (var y0 = 0; y0 < n; y0++) for (var x0 = 0; x0 < n; x0++) {
      if (id[y0 * n + x0] >= 0 || edgeAt(x0, y0)) continue;
      var k = blocks.length, cells = [], worth = 0, sx = 0, sy = 0;
      id[y0 * n + x0] = k; stack.push(x0, y0);
      while (stack.length) {
        var cy = stack.pop(), cx = stack.pop();
        cells.push(cx, cy); sx += cx; sy += cy;
        var cc = m.at(cx, cy);
        worth += worthOf(cc && cc.district);
        var around = [cx + 1, cy, cx - 1, cy, cx, cy + 1, cx, cy - 1];
        for (var a = 0; a < 8; a += 2) {
          var X = around[a], Y = around[a + 1];
          if (X < 0 || Y < 0 || X >= n || Y >= n) continue;
          if (id[Y * n + X] >= 0 || edgeAt(X, Y)) continue;
          id[Y * n + X] = k; stack.push(X, Y);
        }
      }
      var cnt = cells.length / 2;
      blocks.push({ i: k, cells: cells, count: cnt, worth: worth,
                    cx: Math.round(sx / cnt), cy: Math.round(sy / cnt) });
    }
    out.id = id; out.blocks = blocks;
    return out;
  }

  /* WHO HOLDS EVERY CELL OF THE VALLEY.
     1. HIS OVERRIDE FIRST: anything in HOLDS wins, always.
     2. A CAPITAL HOLDS ITS OWN BLOCK. A seat that did not own the ground under it
        would not be a seat.
     3. THE DRAFT, in HIS order: factions pick by act1_power, strongest first, and
        each pick is the most valuable unclaimed block TOUCHING ground it already
        holds. Territory grows outward from a capital the way real ground does,
        rather than teleporting across the valley. When nothing it holds touches
        free ground, it takes the nearest unclaimed block to its seat, so a boxed-in
        faction is never left with four cells -- which is what the old rule did to
        the Volunteers.
     4. THE SKELETON GOES TO WHOEVER IS NEAREST. A road IS the border, so the cells
        of the border belong to whichever side reaches them first; a breadth-first
        wave out of every owned block settles it, and ties go to the stronger
        faction and then the name so every device agrees. */
  function turf(m, cat, towns) {
    var res = { n: 0, id: null, ownerOf: null, blocks: [], byFaction: {}, at: function () { return null; } };
    var B = blocksOf(m, cat);
    var n = B.n;
    if (!n || !B.blocks.length || !towns || !towns.length) return res;

    var order = towns.slice().sort(function (a, b) {
      var pa = (a.power || 0), pb = (b.power || 0);
      if (pb !== pa) return pb - pa;
      return a.faction < b.faction ? -1 : 1;
    });
    var rank = {}; for (var r = 0; r < order.length; r++) rank[order[r].faction] = r;

    var owner = new Array(B.blocks.length).fill(null);

    /* 1. his overrides, by the block they land in */
    for (var hk in HOLDS) if (Object.prototype.hasOwnProperty.call(HOLDS, hk)) {
      var hp = hk.split(','), hx = hp[0] | 0, hy = hp[1] | 0;
      if (hx < 0 || hy < 0 || hx >= n || hy >= n) continue;
      var hb = B.id[hy * n + hx];
      if (hb >= 0 && !owner[hb]) owner[hb] = HOLDS[hk];
    }
    /* 2. every capital owns the ground under it */
    for (var s = 0; s < order.length; s++) {
      var t = order[s];
      if (t.x < 0 || t.y < 0 || t.x >= n || t.y >= n) continue;
      var bi = B.id[t.y * n + t.x];
      if (bi >= 0 && !owner[bi]) owner[bi] = t.faction;
    }

    /* which blocks touch which, across one cell of skeleton */
    var touch = [];
    for (var i = 0; i < B.blocks.length; i++) touch.push({});
    for (var yy = 0; yy < n; yy++) for (var xx = 0; xx < n; xx++) {
      if (B.id[yy * n + xx] >= 0) continue;                 /* skeleton cells only */
      var near = {}, ring = [xx + 1, yy, xx - 1, yy, xx, yy + 1, xx, yy - 1,
                            xx + 1, yy + 1, xx - 1, yy - 1, xx + 1, yy - 1, xx - 1, yy + 1];
      for (var q = 0; q < ring.length; q += 2) {
        var rx = ring[q], ry = ring[q + 1];
        if (rx < 0 || ry < 0 || rx >= n || ry >= n) continue;
        var rb = B.id[ry * n + rx]; if (rb >= 0) near[rb] = 1;
      }
      var ks = Object.keys(near);
      for (var u = 0; u < ks.length; u++) for (var v = 0; v < ks.length; v++)
        if (u !== v) touch[ks[u] | 0][ks[v] | 0] = 1;
    }

    /* 3. the draft */
    var left = 0;
    for (var z = 0; z < owner.length; z++) if (!owner[z]) left++;
    var guard = 0;
    while (left > 0 && guard++ < 4000) {
      var moved = false;
      for (var f = 0; f < order.length; f++) {
        if (!left) break;
        var fid = order[f].faction, pick = -1, best = -1;
        for (var bj = 0; bj < owner.length; bj++) {
          if (owner[bj]) continue;
          var adj = false, tk = touch[bj];
          for (var nb in tk) if (owner[nb | 0] === fid) { adj = true; break; }
          if (!adj) continue;
          var w = B.blocks[bj].worth;
          if (w > best || (w === best && pick >= 0 && bj < pick)) { best = w; pick = bj; }
        }
        if (pick < 0) {                       /* boxed in: nearest free ground */
          var nd = Infinity;
          for (var bk = 0; bk < owner.length; bk++) {
            if (owner[bk]) continue;
            var d = Math.abs(B.blocks[bk].cx - order[f].x) + Math.abs(B.blocks[bk].cy - order[f].y);
            if (d < nd || (d === nd && pick >= 0 && bk < pick)) { nd = d; pick = bk; }
          }
        }
        if (pick >= 0) { owner[pick] = fid; left--; moved = true; }
      }
      if (!moved) break;
    }

    /* 4. the skeleton, by a wave out of every owned block */
    var own = new Array(n * n).fill(null);
    var qx = [], qy = [];
    for (var bb = 0; bb < B.blocks.length; bb++) {
      if (!owner[bb]) continue;
      var cl = B.blocks[bb].cells;
      for (var ci = 0; ci < cl.length; ci += 2) {
        own[cl[ci + 1] * n + cl[ci]] = owner[bb];
        qx.push(cl[ci]); qy.push(cl[ci + 1]);
      }
    }
    var head = 0;
    while (head < qx.length) {
      var px = qx[head], py = qy[head]; head++;
      var who = own[py * n + px];
      var ar = [px + 1, py, px - 1, py, px, py + 1, px, py - 1];
      for (var g = 0; g < 8; g += 2) {
        var ax = ar[g], ay = ar[g + 1];
        if (ax < 0 || ay < 0 || ax >= n || ay >= n) continue;
        var idx = ay * n + ax;
        if (own[idx] === null) { own[idx] = who; qx.push(ax); qy.push(ay); }
        else if (own[idx] !== who && B.id[idx] < 0) {
          /* two waves met on the skeleton: the stronger name holds the road, so
             the answer does not depend on which cell the loop reached first */
          if ((rank[who] | 0) < (rank[own[idx]] | 0)) own[idx] = who;
        }
      }
    }
    /* his overrides win on the exact cell too */
    for (var hk2 in HOLDS) if (Object.prototype.hasOwnProperty.call(HOLDS, hk2)) {
      var hp2 = hk2.split(','), ox = hp2[0] | 0, oy = hp2[1] | 0;
      if (ox >= 0 && oy >= 0 && ox < n && oy < n) own[oy * n + ox] = HOLDS[hk2];
    }

    /* *** A CAPITAL CAN SIT ON SOMEBODY ELSE'S GROUND, AND THAT IS NOT A BUG. ***
       (9/6.) The gate's first version of this asserted every seat owns the block
       under it, and the world said no three times: the Church's chapel and the
       Mob's resort are both inside the 431-cell Strip block, and the Network, the
       Reds and the Volunteers all sit in one 35-cell block downtown. A block has
       one owner by construction -- that is the whole reason the borders are
       visible -- so the weaker capital is a GUEST.
       The reflex was to push the seats apart until my claim came true. Then his own
       canon turned out to have written this already: the Volunteers are "resource-
       poor by design. Nobody wants to be seen attacking them, EVEN CARTEL STAYS
       HANDS-OFF." A clinic that survives because nobody will touch it, standing on
       ground somebody else holds, is exactly that sentence. So the world keeps its
       answer and the claim is the thing that changes.
       IT IS NAMED RATHER THAN LEFT TO BE NOTICED. A fact this useful sitting in the
       data with nothing able to ask about it is the authored-but-unread disease, so
       turf() reports it and the invariant that MATTERS is locked instead: the
       stronger faction always holds the ground, never the weaker one. */
    var guests = [];
    for (var gi = 0; gi < towns.length; gi++) {
      var gt = towns[gi];
      if (gt.x < 0 || gt.y < 0 || gt.x >= n || gt.y >= n) continue;
      var gw = own[gt.y * n + gt.x];
      if (gw && gw !== gt.faction) guests.push({ faction: gt.faction, host: gw, x: gt.x, y: gt.y });
    }

    var by = {};
    for (var e = 0; e < own.length; e++) if (own[e]) by[own[e]] = (by[own[e]] || 0) + 1;
    var tierOf = {}; for (var ti = 0; ti < towns.length; ti++) tierOf[towns[ti].faction] = towns[ti].tier;

    res.n = n; res.id = B.id; res.blocks = B.blocks; res.ownerOf = owner;
    res.byFaction = by; res.own = own; res.guests = guests; res.rank = rank;
    res.at = function (x, y) {
      if (x < 0 || y < 0 || x >= n || y >= n) return null;
      var w = own[y * n + x];
      if (!w) return null;
      return { faction: w, tier: tierOf[w] || null,
               ruled: Object.prototype.hasOwnProperty.call(HOLDS, x + ',' + y),
               block: B.id[y * n + x], draft: true };
    };
    return res;
  }

  function holderOf(towns, x, y) {
    var k = x + ',' + y;
    if (Object.prototype.hasOwnProperty.call(HOLDS, k))
      return { faction: HOLDS[k], ruled: true, cells: 0, draft: false };
    var best = null, bs = Infinity, bd = 0;
    for (var i = 0; i < (towns || []).length; i++) {
      var t = towns[i];
      var d = Math.max(Math.abs(t.x - x), Math.abs(t.y - y));
      var r = Object.prototype.hasOwnProperty.call(REACH, t.tier) ? REACH[t.tier] : 1;
      var sc = d / r;
      if (sc < bs
          || (sc === bs && best && ((t.power || 0) > (best.power || 0)
              || ((t.power || 0) === (best.power || 0) && t.faction < best.faction)))) {
        bs = sc; best = t; bd = d;
      }
    }
    return best ? { faction: best.faction, tier: best.tier, ruled: false,
                    cells: bd, draft: true } : null;
  }

  /* ==========================================================================
     A FACTION MINES ITS LAND   (9/11, VAMILY row [power territory])

     THE ROW: "every faction's territory carries its own battery-making
     buildings, so what a faction is worth is what its land makes; a fortress
     makes more than a camp; losing a block loses its output."

     WHAT IT IS SET AGAINST, MEASURED FIRST. A faction's strength in this game is
     act1_power / act3_power, and his graph's own meta calls it an ORDINAL RANK.
     It is a number typed once. It decides the tier and the turf draft order and
     it NEVER MOVES: take every block off the Mob and the Mob is still 13. There
     was no reading anywhere that answered "what does this faction's land make".

     ---------------------------------------------------------------------------
     GROUND THAT MAKES ELECTRICITY, AND THE ONE THAT DOES NOT
     ---------------------------------------------------------------------------
     A SUBSTATION IS NOT IN THIS LIST AND THAT IS THE WHOLE POINT OF THE LIST. It
     steps voltage down and passes it along; it generates nothing. Counting it
     would be counting the wire as the well. Three kinds make power in this
     valley and they are the three real things that make it in a desert. */
  var MAKES = ['solar', 'dam', 'battery'];

  /* HIS RULING, NOT A NUMBER OF MINE. 7/26 BUILDINGS PRODUCE ONE OF THE THREE,
     8/15 EVERYTHING COSTS ONE, and WORLD's own [batteries mined] row says it in
     as many words: "start at ONE battery per day per building". Carried as a
     string beside the value the same way bohemia_production.js carries it, so a
     reader can see there is no hand-picked number here. */
  var MINE_RULING = '7/26 BUILDINGS PRODUCE ONE OF THE THREE + 8/15 EVERYTHING COSTS ONE';
  var PER_SITE_PER_DAY = 1;

  /* A SITE IS A BUILDING, NOT A CELL, and the map settles it rather than a rule
     of thumb: flood-filled runs of one kind. MEASURED ON THE REAL SURFACE -- 301
     solar cells are THREE arrays, and the dam comes back as ONE site of four
     cells, which is Hoover and is the check that this unit is right. Counting
     cells would have said the valley holds 301 generators. */
  function minesOf(m, cat, towns) {
    var out = { sites: [], byFaction: {}, perDay: {}, kinds: MAKES.slice(),
                ruling: MINE_RULING };
    if (!m || typeof m.at !== 'function') return out;
    var n = m.n | 0, seen = {}, x, y;
    function kindAt(px, py) {
      if (px < 0 || py < 0 || px >= n || py >= n) return null;
      var c = null; try { c = m.at(px, py); } catch (_e) { return null; }
      return (c && c.district) || null;
    }
    var holder = (typeof turf === 'function' && towns && towns.length)
                 ? turf(m, cat, towns) : null;
    function heldAt(px, py) {
      if (!holder || typeof holder.at !== 'function') return null;
      var t = null; try { t = holder.at(px, py); } catch (_e) { return null; }
      return (t && t.faction) || null;
    }
    for (y = 0; y < n; y++) for (x = 0; x < n; x++) {
      var k = kindAt(x, y);
      if (!k || MAKES.indexOf(k) < 0 || seen[x + ',' + y]) continue;
      var stack = [[x, y]], cells = [];
      seen[x + ',' + y] = 1;
      while (stack.length) {
        var cur = stack.pop(); cells.push(cur);
        var nb = [[cur[0] + 1, cur[1]], [cur[0] - 1, cur[1]],
                  [cur[0], cur[1] + 1], [cur[0], cur[1] - 1]];
        for (var i = 0; i < nb.length; i++) {
          var nx = nb[i][0], ny = nb[i][1];
          if (nx < 0 || ny < 0 || nx >= n || ny >= n) continue;
          if (seen[nx + ',' + ny] || kindAt(nx, ny) !== k) continue;
          seen[nx + ',' + ny] = 1; stack.push([nx, ny]);
        }
      }
      /* WHOSE IS A SITE THAT STRADDLES A BORDER? Whoever holds most of it, and
         this rule EARNS ITS KEEP rather than sitting there for a rainy day: the
         dam really is split on the boot seed, 3 cells to the Anarchists and 1 to
         the Mob, so a rule was needed and the majority is the one that does not
         depend on which cell the scan happened to reach first. Ties break on the
         faction name, stably, never on iteration order. */
      var hold = {}, f, c2;
      for (var j = 0; j < cells.length; j++) {
        f = heldAt(cells[j][0], cells[j][1]) || null;
        if (f) hold[f] = (hold[f] || 0) + 1;
      }
      var own = null, bestN = -1;
      for (c2 in hold) if (Object.prototype.hasOwnProperty.call(hold, c2)) {
        if (hold[c2] > bestN || (hold[c2] === bestN && own && c2 < own)) {
          bestN = hold[c2]; own = c2;
        }
      }
      out.sites.push({ kind: k, cells: cells.length, x: cells[0][0], y: cells[0][1],
                       faction: own, held: bestN < 0 ? 0 : bestN,
                       split: Object.keys(hold).length > 1, holders: hold });
      if (own) {
        if (!out.byFaction[own]) out.byFaction[own] = { sites: 0, cells: 0, kinds: {} };
        out.byFaction[own].sites++;
        out.byFaction[own].cells += cells.length;
        out.byFaction[own].kinds[k] = (out.byFaction[own].kinds[k] || 0) + 1;
      }
    }
    for (var fk in out.byFaction) if (Object.prototype.hasOwnProperty.call(out.byFaction, fk))
      out.perDay[fk] = out.byFaction[fk].sites * PER_SITE_PER_DAY;
    return out;
  }

  /* WHAT ONE FACTION'S LAND MAKES, per day, in batteries.
     DERIVED, NEVER STORED, so "losing a block loses its output" needs no rule of
     its own: the block changes hands, the site changes hands with it, and asking
     again is simply a different answer. A stored figure would need somebody to
     remember to move it, which is the trap the coalition and the roving rule both
     avoided in this lane already.

     *** AND IT IS NOT GATED ON THE LIGHTS, WHICH IS A TRAP I WALKED INTO AND
     MEASURED MY WAY OUT OF. *** The obvious version reads "a district pays only
     while it is yours AND lit AND patrolled" and asks the grid whether the site
     is live. Measured on the walked surface: ALL FIVE SITES HAVE ZERO LIT CELLS,
     because circuits only run along STREET cells and a solar array stands in the
     desert. That version would have made the entire valley produce nothing while
     every check stayed green. A generator MAKES power; whether the power reaches
     anybody is the grid's job and the grid already models it. The income rule
     governs what a district PAYS, not what a plant MAKES. */
  function minesFor(faction, m, cat, towns) {
    if (!faction) return null;
    var all = minesOf(m, cat, towns);
    var row = all.byFaction[faction] || null;
    return { faction: faction,
             sites: row ? row.sites : 0,
             cells: row ? row.cells : 0,
             kinds: row ? row.kinds : {},
             perDay: row ? row.sites * PER_SITE_PER_DAY : 0,
             ruling: MINE_RULING,
             /* an honest zero, said out loud, because ten of the fourteen really
                do hold nothing that makes power and that is the map, not a gap */
             makesNothing: !row };
  }

  /* ==========================================================================
     THE BLOCK PAYS ITS OWNER   (9/12, VAMILY row [block rent])

     THE ROW, off the coordinator's 9/5 generator-mafia research: in Lebanon and
     Iraq the faction that owns a neighbourhood's generator "charges every
     household on it monthly, by the ampere, sets the price, and cuts you off
     without warning"; it was 44% of an average family's income.

     MEASURED FIRST. Every cell of this valley has belonged to somebody since
     [who holds], and payTo() has answered "who do you pay for this block" since
     [light owners]. IT HAD EXACTLY ONE CALLER IN THE WHOLE GAME AND IT WAS A GATE.
     Living on somebody's ground cost nothing.

     AND payTo() IS THE WRONG DOOR FOR THIS, measured on the walked surface: it
     reads the GRID, and the grid only has status on STREET cells, so it answers
     NULL for 1,467 of 2,304 sampled cells INCLUDING THE BLOCK THE PLAYER WAKES ON.
     The grid says who owns the WIRE. TURF says who owns the GROUND, for 100% of
     the valley, and rent is ground.

     ---------------------------------------------------------------------------
     A FORTRESS CHARGES MORE THAN A CAMP, WITHOUT A PRICE NOBODY RULED
     ---------------------------------------------------------------------------
     EVERYTHING COSTS ONE (8/15), so a fortress cannot charge a bigger number.
     What it can do is collect on MORE OF WHAT YOU USED, which is what a bigger
     operator really does. That scaling is HIS THIRDS, already in this file as
     DEPTH, and it is applied exactly the way goodsFor applies it: to a COUNT,
     through Math.ceil, never to a price.
       fortress  bills all of the ground of theirs you used
       town      bills two thirds of it
       camp      bills a third
     One battery per billed block, the same shape as the night's per-circuit bill.
     HONEST LIMIT, stated rather than hidden: with ONE block used they all bill 1,
     because a third of one block rounds up to one. The tiers only separate once
     you have walked more of somebody's ground, which is when it should matter. */
  function rentOn(used, towns) {
    var out = { rows: [], total: 0, ruling: 'EVERYTHING COSTS ONE (8/15)' };
    if (!used || !towns || !towns.length) return out;
    var tier = {};
    for (var i = 0; i < towns.length; i++) tier[towns[i].faction] = towns[i].tier;
    var f;
    for (f in used) {
      if (!Object.prototype.hasOwnProperty.call(used, f)) continue;
      var n = used[f] | 0;
      if (n <= 0) continue;
      var t = tier[f] || 'camp';
      var share = Object.prototype.hasOwnProperty.call(DEPTH, t) ? DEPTH[t] : DEPTH.camp;
      var billed = Math.max(1, Math.ceil(n * share));
      if (billed > n) billed = n;
      out.rows.push({ faction: f, tier: t, used: n, billed: billed });
      out.total += billed;
    }
    /* stable, so the same day bills in the same order every time it is asked */
    out.rows.sort(function (a, b) { return a.faction < b.faction ? -1 : 1; });
    return out;
  }

  /* ==========================================================================
     THE LENDER VISITS THE HEIR   (9/12, VAMILY row [collector heir])

     THE ROW, off the 9/5 heir research: "on the first day after the fold, the
     faction your parent owed comes to you; the first line the heir hears in their
     own life is the parent's debt, on the beat, at the door."

     *** TWO LIVE BOARD ROWS CONTRADICT EACH OTHER HERE AND THE SHIPPED CODE
     SETTLES IT. *** WORLD's [debt carried] says every debt "survives the
     generation fold to the heir IN FULL". engine/bohemia_fold.js, shipped 9/7 and
     marked ruled:true off the DYNASTY study, says the opposite and says why:

       "a child is not personally liable for a parent's unsecured debts... YOU DO
        NOT INHERIT A BILL, YOU INHERIT LESS AND YOU INHERIT THE PEOPLE HE OWED,
        still standing there. That is a standing-web query, not a purse line."

     That sentence IS this row. The balance dies; the CREDITOR does not. So what
     crosses the fold is not an amount, it is the fact that this faction was
     stiffed, and it arrives as somebody at the door rather than as a number in a
     purse. The contradiction with WORLD's row is canon-level and goes to Paolo;
     this build follows the ruling that has a machine gate behind it.

     THERE IS EXACTLY ONE WAY TO OWE A FACTION IN THIS GAME TODAY, and it is this
     lane's own: [block rent], shipped last round. Rent you could not pay. Nothing
     else anywhere lends anybody anything -- WORLD's [someone lends] row says so in
     its own first line -- so a collector built on anything else would be a pipe
     with no water. */

  /* WHO IS OWED, WORST FIRST. `book` is {faction: {nights, lastDay}}, kept by the
     caller because a ledger of what happened is the city's to keep, not a map
     module's to invent. NO AMOUNT IS RANKED: the nights are a COUNT of times they
     were stiffed, which is a fact, and what a night of unpaid rent is WORTH is a
     weight and weights are his. */
  function owedTo(book) {
    var out = [];
    if (!book) return out;
    for (var f in book) {
      if (!Object.prototype.hasOwnProperty.call(book, f)) continue;
      var r = book[f] || {};
      var n = r.nights | 0;
      if (n <= 0) continue;
      out.push({ faction: f, nights: n, lastDay: r.lastDay | 0 });
    }
    /* worst first, and a tie breaks on the MORE RECENT night, then on the name --
       never on object key order, which is not a rule anybody can argue with. */
    out.sort(function (a, b) {
      if (b.nights !== a.nights) return b.nights - a.nights;
      if (b.lastDay !== a.lastDay) return b.lastDay - a.lastDay;
      return a.faction < b.faction ? -1 : 1;
    });
    return out;
  }

  /* WHAT THE COLLECTOR SAYS AT THE DOOR. Attempts, draft:true, eighth grade, and
     they are three because the row's own shape is that the heir did not do this.
       came    the parent's generation is over and they are here anyway
       nights  how many times, because "he owed you" is vaguer than a count
       clear   the one thing that is NOT said: a number. The balance died with
               him (the fold's ruling), so a collector who names a sum would be
               collecting a bill the game has already said the heir does not owe. */
  var COLLECTOR = {
    came:  'SOMEBODY IS AT THE DOOR AND THEY ARE NOT HERE FOR YOU',
    one:   'YOUR FATHER WENT A NIGHT WITHOUT PAYING THEM. THEY REMEMBER',
    many:  'YOUR FATHER WENT SHORT WITH THEM MORE THAN ONCE. THEY REMEMBER',
    still: 'THE DEBT DIED WITH HIM. THEY DID NOT'
  };
  function collectorAt(book, gen) {
    var list = owedTo(book);
    if (!list.length) return null;
    var top = list[0];
    return { faction: top.faction, nights: top.nights, gen: gen | 0,
             others: list.length - 1,
             came: COLLECTOR.came,
             what: top.nights > 1 ? COLLECTOR.many : COLLECTOR.one,
             still: COLLECTOR.still,
             draft: true };
  }

  var API = {
    TIERS: TIERS, SEATS: SEATS, TIER: TIER, DEPTH: DEPTH, REACH: REACH,
    MAKES: MAKES.slice(), MINE_RULING: MINE_RULING, rentOn: rentOn,
    owedTo: owedTo, collectorAt: collectorAt, COLLECTOR: COLLECTOR,
    PER_SITE_PER_DAY: PER_SITE_PER_DAY, minesOf: minesOf, minesFor: minesFor,
    selectable: selectable, tiers: tiers, powerOf: powerOf,
    districtsOf: districtsOf, derive: derive, NOT_A_TOWN: NOT_A_TOWN,
    HOMES: HOMES, placeHomes: placeHomes, MIN_APART: MIN_APART,
    frontsRoad: frontsRoad,
    goodsFor: goodsFor, nearest: nearest, townCells: townCells,
    HOLDS: HOLDS, holderOf: holderOf,
    WORTH: WORTH, worthOf: worthOf, SEEN_EDGE: SEEN_EDGE,
    blocksOf: blocksOf, turf: turf
  };
  if (typeof module !== 'undefined' && module.exports) module.exports = API;
  root.BohemiaTowns = API;
})(typeof window !== 'undefined' ? window : (typeof globalThis !== 'undefined' ? globalThis : this));
