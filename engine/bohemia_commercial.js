...
(function(root){
  ...
  var MARGIN=2, SERVLANE=M(4), STOREDEEP=M(11), SIDE=2, DOOR=M(6);
  ...
  function generate(seed,opts){
    ...
    // --- STORE STRIPS: [service alley][store][sidewalk], back doors onto the alley ---
    function strip(edge){
      if(edge==='N'){ var s0=MARGIN+SERVLANE, f0=s0+STOREDEEP;
        rect(g,MARGIN,MARGIN,W-1-MARGIN,s0-1,8);
        rect(g,MARGIN,s0,W-1-MARGIN,f0-1,2);
        hLine(g,MARGIN,W-1-MARGIN,f0,6,SIDE);
        for(x=MARGIN+3;x<W-MARGIN;x+=DOOR){ if(inb(x,s0))g[s0][x]=9; if(inb(x,f0-1))g[f0-1][x]=7; } }
      else if(edge==='W'){ var s0=MARGIN+SERVLANE, f0=s0+STOREDEEP;
        rect(g,MARGIN,MARGIN,s0-1,H-1-MARGIN,8);
        rect(g,s0,MARGIN,f0-1,H-1-MARGIN,2);
        vLine(g,MARGIN,H-1-MARGIN,f0,6,SIDE);
        for(y=MARGIN+3;y<H-MARGIN;y+=DOOR){ if(inb(s0,y))g[y][s0]=9; if(inb(f0-1,y))g[y][f0-1]=7; } }
    }
    if(back.N)strip('N'); if(back.W)strip('W');
    // service connector: the rear alley drives around the building END into the parking
    // (clears the store to the edge so no tiny sliver is left behind).
    if(back.N){ var cx=W-1-MARGIN-SERVLANE; rect(g,cx,MARGIN,W-1-MARGIN,pT-1,8); }
    if(back.W){ var cy=H-1-MARGIN-SERVLANE; rect(g,MARGIN,cy,pL-1,H-1-MARGIN,8); }
    ...
    var gas=null;
    if(isS('S')&&isS('E')){
      var gw=M(16), gh=M(15), gx0=pR-gw, gy0=pB-gh;
      rect(g,gx0,gy0,pR,pB,1);                                  // clear to asphalt
      rect(g,gx0+2,gy0+2,pR-2,gy0+2+M(6),10);                   // canopy
      for(var pmp=gx0+4; pmp<pR-3; pmp+=4) vLine(g,gy0+4,gy0+6,pmp,11,1);   // pump islands
      rect(g,gx0+2,pB-M(6),gx0+2+M(8),pB-1,2);                  // kiosk (mini-mart)
