...
(function(root){
  ...
  /* THE BINS (code 14), 8/21. The other half of "nothing on this plot stands up". The
     streetlight gave the suburb its first vertical that is not a house; this gives it the
     one object every single American kerb has, and it is the object that says PEOPLE LIVED
     HERE more plainly than any building does.

     WHERE THEY ARE IS A STORY DECISION AND IT IS NOT DECORATION. Las Vegas residential
     collection is wheeled carts: they live beside the garage and get rolled to the kerb the
     night before. So where a bin is tells you WHEN this street stopped: the ones at the kerb
     were put out for a collection that never came. That is the whole valley's premise in one
     prop, and it costs nothing to say it this way instead of scattering them at random.
     THE SPLIT IS DELIBERATE and it is the only number here: most houses kept theirs by the
     garage, a minority had already rolled theirs out. Nobody ruled on the ratio, so it is a
     mechanism default, not canon -- MECHANISM-MINE / CONTENTS-PAOLO'S.

     NEVER ON THE WALK, NEVER ON THE APRON. A bin is a solid cell. Paolo's walk is one grid
     wide (7/31, LOCKED), so a bin standing in it does not narrow the sidewalk, it severs it;
     and a bin on the driveway apron blocks the car the driveway exists for. Bare yard and
     gravel only, exactly like the pole. */
  function layBins(g){
    var W=Wd(g),H=Ht(g),x,y,i,n=0,d4=[[1,0],[-1,0],[0,1],[0,-1]];
    /* one decision per DRIVEWAY, because a driveway is a house: find each apron blob once,
       then place from its two ends. Scanning cells instead would put a bin beside every
       apron tile and line the street with them. */
    var seen={};
    for(y=1;y<H-1;y++)for(x=1;x<W-1;x++){
      if(g[y][x]!==3 || seen[x+','+y]) continue;
      ...
      while(st.length){ var p=st.pop(); cells.push(p);
        for(i=0;i<4;i++){ var nx=p[0]+d4[i][0], ny=p[1]+d4[i][1], k=nx+','+ny;
          if(seen[k]||!inb(g,nx,ny)||g[ny][nx]!==3) continue; seen[k]=1; st.push([nx,ny]); } }
      /* THIS HOUSE'S OWN COIN. Math.imul, not `*`: with these constants a plain multiply
         exceeds 2^53, JS rounds it as a double, and the low bits -- the only bits a %100
         ever reads -- come back degraded. That is a real latent bug and it is fixed here,
         but IT WAS NOT THE ONE I WENT LOOKING FOR, and the distinction is worth keeping.
         A 22-house block was standing 28 bins and I assumed the hash. MEASURED INSTEAD OF
         ASSUMED: the coin was clean (values 95,11,95,77,16,30,35,7,49,...) and 15 of 22
         blobs passed. The count was my own inverted condition -- `>=62 continue` passes
         SIXTY-TWO percent, not thirty-eight -- and I had almost written the wrong cause
         into this comment as a measurement I never took. */
      var hh=(Math.imul(cells[0][0],73856093)^Math.imul(cells[0][1],19349663)^0xB1)>>>0;
      /* WHO STILL HAS CARTS ON THE LOT. Every house in this valley owned two; a decade
         later some were taken, some blew into the wash, some are shut in the garage. Just
         over half still show. A mechanism default, not canon -- nobody ruled on it. */
      if((hh%100)>=55) continue;
      /* KERB END or GARAGE END: the apron cell nearest the walk, or nearest the garage */
      var kerb=null, gar=null;
      for(i=0;i<cells.length;i++){ var c=cells[i];
        for(var d=0;d<4;d++){ var ax=c[0]+d4[d][0], ay=c[1]+d4[d][1];
          if(!inb(g,ax,ay)) continue;
          if(g[ay][ax]===10||g[ay][ax]===1) kerb=kerb||c;
          if(g[ay][ax]===6) gar=gar||c; } }
      /* PREFERENCE, THEN FALL BACK -- the lesson the streetlights taught this morning. A
         first cut picked ONE end and gave up if the yard beside it happened to be house or
         walk, which is a lottery a cell has to win rather than a habit a street has: it put
         2 bins on one plot and 10 on the next. A bin goes where a bin fits; the coin decides
         WHICH END IT PREFERS, never whether the house owns one at all. */
      var ends=((hh>>7)%100<34) ? [kerb,gar] : [gar,kerb];
      ends=ends.filter(function(e){return !!e;});
      if(!ends.length) continue;
      var placed=false, bx=0, by=0;
      for(var ei=0;ei<ends.length&&!placed;ei++)
      for(i=0;i<4&&!placed;i++){ bx=ends[ei][0]+d4[i][0]; by=ends[ei][1]+d4[i][1];
        if(!inb(g,bx,by)) continue;
        if(g[by][bx]!==0&&g[by][bx]!==11) continue;
        g[by][bx]=14; n++; placed=true;
        /* THE SECOND CART. Two-stream collection is universal here, so a house showing one
           usually shows both, standing together. */
        if(((hh>>13)%100)<50){
          for(var j=0;j<4;j++){ var sx=bx+d4[j][0], sy=by+d4[j][1];
            if(!inb(g,sx,sy)) continue;
            if(g[sy][sx]!==0&&g[sy][sx]!==11) continue;
            g[sy][sx]=14; n++; break; } }
      }
    }
    return n;
  }
  /* THE BARREL FIRE (code 15). PAOLO, 8/21, asked whether anybody is out there: "Ofc ppl
     will warm themselves by barrel fire in act one." That is a ruling and it settles
     something much bigger than a prop -- ACT ONE HAS LIVING PEOPLE IN IT, they are outside,
     and they are cold.

     WHERE A FIRE BURNS IS THE POWER LAW MADE VISIBLE, which is why the fire is PLACED here
     and LIT somewhere else. The valley measures 94.5% dark (CLUSTERED POWER: 12% of circuits
     live, owned, the network eerily perfect). A streetlight is what burns on the small share
     somebody OWNS. A barrel is what burns on all the rest, where everybody else is. Same
     street, two kinds of light, and you can read who holds what by which one you can see.
     So this generator only says "a person would light one HERE"; the city decides whether it
     is actually burning by asking POWER, because nobody burns furniture on a block that
     still has electricity.

     OUT OF THE WIND AND OFF THE FRONTAGE. Against the inside face of the block wall, in a
     corner where two walls meet: a Mojave winter night runs to freezing and that wall is the
     only windbreak on the plot. RARE ON PURPOSE -- a fire means PEOPLE, people are the
     scarcest thing in this valley, and one barrel per neighbourhood says far more than one
     per block. HOW MANY AND WHERE IS MINE; THAT THEY EXIST AT ALL IS HIS. */
  function layFires(g){
    var W=Wd(g),H=Ht(g),x,y,i,d4=[[1,0],[-1,0],[0,1],[0,-1]];
    var hh=(Math.imul(SEED_CTX,2246822519)^0x1F13E)>>>0;
    /* PEOPLE ARE THE SCARCEST THING IN THIS VALLEY. About one neighbourhood in five has
       anybody left in it -- a mechanism default, not canon; Paolo ruled THAT they exist, the
       number is mine until he says otherwise. */
    if((hh%100)>=22) return 0;
    function look(needWalls){
      /* FROM 1, NOT 2. The first cut scanned from 2 and found NOTHING on 24 seeds, because
         the block wall is a one-thick ring at the very edge and the only cells with TWO wall
         neighbours are the inside corners at (1,1) -- exactly the cells the bound skipped.
         The corner is also the right answer: sheltered on two sides, off the frontage, out
         of sight of the street. */
      var out=[];
      for(var yy=1;yy<H-1;yy++)for(var xx=1;xx<W-1;xx++){
        if(g[yy][xx]!==0&&g[yy][xx]!==11) continue;      /* bare yard or gravel only */
        /* READ WITH A RAW BOUNDS CHECK, NOT inb(). This module's inb() is
           `x>=1 && y>=1 && x<W-1 && y<H-1` -- it means "a cell you may WRITE to", and it
           deliberately excludes the outer ring. THE BLOCK WALL IS THAT RING. Asking inb()
           whether the wall exists therefore answers NO, walls counted 0 everywhere, and 24
           seeds produced 24 empty neighbourhoods with the coin passing normally. A helper
           named for bounds that actually means "the writable interior" is safe to place
           with and wrong to look with. */
        var walls=0, frontage=false;
        for(i=0;i<4;i++){ var nx=xx+d4[i][0], ny=yy+d4[i][1];
          if(nx<0||ny<0||nx>=W||ny>=H) continue;
          if(g[ny][nx]===4) walls++;                     /* block wall / side fence */
          if(g[ny][nx]===10||g[ny][nx]===1) frontage=true;
        }
        if(walls>=needWalls && !frontage) out.push([xx,yy]);
      }
      return out;
    }
    /* COLLECT, THEN CHOOSE. Returning the FIRST match put the fire at (1,1) on every single
       plot -- 11 neighbourhoods, 11 fires, all in the same corner, because a scan in reading
       order always finds that corner first. Identical placement across every block is a
       pattern a player reads as a bug even when each one is individually plausible. */
    var cands=look(2); if(!cands.length) cands=look(1);
    if(!cands.length) return 0;
    var best=cands[((hh>>9)>>>0)%cands.length];
    g[best[1]][best[0]]=15;
    return 1;
  }
  /* THE CARS THAT NEVER LEFT (code 16), 8/23. Fourteen codes in the district he spawns in
     and not ONE of them was a vehicle -- every driveway on his street empty, which is the
     single thing a dead American suburb would never be.

     AND WHY THEY ARE STILL HERE IS THE PREMISE OF THE WHOLE GAME. In an EVACUATION people
     drive away and the drives are empty. In an ECONOMIC collapse they stay until they cannot,
     and the car dies where it sits -- no fuel, no parts, nowhere to go. Detroit is the
     reference and it is unambiguous: abandoned vehicles in drives, at kerbs and pushed onto
     yards are so defining a feature of a foreclosed neighbourhood that the city runs
     enforcement programmes for them. Bohemia's identity is "the most realistic economic crash
     simulator", so MOST DRIVEWAYS KEEP THEIR CAR. An empty drive is the exception and it
     means somebody got out.

     A CAR IN A TWO-WIDE DRIVE BLOCKS IT, and that is true rather than convenient: you cannot
     get a vehicle past it to the garage. What that costs the drive network is MEASURED below
     and in the gate, not assumed. The share is a mechanism default; that they are here at all
     is what the collapse being economic means. */
  function layCars(g){
    var W=Wd(g),H=Ht(g),x,y,i,n=0,d4=[[1,0],[-1,0],[0,1],[0,-1]],seen={};
    for(y=1;y<H-1;y++)for(x=1;x<W-1;x++){
      if(g[y][x]!==3 || seen[x+','+y]) continue;
      var st=[[x,y]], cells=[]; seen[x+','+y]=1;
      while(st.length){ var p=st.pop(); cells.push(p);
        for(i=0;i<4;i++){ var nx=p[0]+d4[i][0], ny=p[1]+d4[i][1], k=nx+','+ny;
          if(seen[k]||!inb(g,nx,ny)||g[ny][nx]!==3) continue; seen[k]=1; st.push([nx,ny]); } }
      var hh=(Math.imul(cells[0][0],2654435761)^Math.imul(cells[0][1],40503)^0xCA53)>>>0;
      if((hh%100)>=64) continue;                    /* a minority of families got out */
      /* BESIDE THE DRIVE, NOT IN IT -- and this was measured twice before it was believed.
         Filling the drive is the obvious reading and it is wrong for this model: our driveway
         is DVW=2 tiles, which at TILE=0.75 is 1.5 m, ONE car wide, not the 16-20 ft two-car
         apron a real Vegas tract house has. So a car in it seals the garage, and roadConnected
         -- this district's own contract that the road reaches every lot -- went from 1.000 to
         0.851 and FAILED ON ALL TWELVE test plots. An earlier cut that dodged that by taking
         cells within a radius of the garage end drew 1x3 and 1x1 blobs, a car one cell wide.
         THE DETROIT REFERENCE ALREADY HAD THE ANSWER: abandoned vehicles in YARDS are as
         defining as the ones in drives, enough that the city runs enforcement for them. A car
         pulled onto the dirt beside the drive is the truer image anyway -- it is what you do
         when it stops running and you still need the drive. It costs the network nothing. */
      var x0=cells[0][0],x1=x0,y0=cells[0][1],y1=y0;
      for(i=1;i<cells.length;i++){ var cc=cells[i];
        if(cc[0]<x0)x0=cc[0]; if(cc[0]>x1)x1=cc[0];
        if(cc[1]<y0)y0=cc[1]; if(cc[1]>y1)y1=cc[1]; }
      var dw=x1-x0+1, dh=y1-y0+1, along=(dh>=dw);      /* the drive runs up the page */
      var cw2=along?2:3, ch2=along?3:2;                /* the car lies alongside it */
      var spots=along ? [[x1+1,y0],[x0-cw2,y0]] : [[x0,y1+1],[x0,y0-ch2]];
      for(var si=0;si<spots.length;si++){
        var sx=spots[si][0], sy=spots[si][1], okp=true;
        for(var yy=sy;yy<sy+ch2&&okp;yy++)for(var xx=sx;xx<sx+cw2&&okp;xx++){
          if(!inb(g,xx,yy)) okp=false;
          else if(g[yy][xx]!==0&&g[yy][xx]!==11) okp=false;   /* bare yard or gravel only */
        }
        if(!okp) continue;
        for(yy=sy;yy<sy+ch2;yy++)for(xx=sx;xx<sx+cw2;xx++){ g[yy][xx]=16; n++; }
        break;
      }
    }
    return n;
  ...
  var PALETTE={
    ...
    0: '#463f30',1:'#33333c',2:'#8a8478',3:'#3f3f47',4:'#6b6152',5:'#c79a3f',6:'#6b6b74',9:'#9a938a',11:'#9b968a',13:'#7c7263',10:'#57575f',12:'#4a463f',14:'#55565a',15:'#6b4a2e',16:'#5a5f63'};
  ...
  var LEGEND={
    ...
    /* Named 'trash bin' so the city page's ONE prop table (legend name -> family) stands a
       bin here without knowing anything about this district -- the same mechanism that gives
       the mall its dumpster and the campus its bench. */
    14:{name:'trash bin / wheeled cart',kind:'prop', act1:'a wheeled collection cart, lid gaping, faded to chalk on the sun side -- the ones at the kerb were put out for a collection that never came'},
    /* Named 'fire barrel' so the city page's prop table routes it to the BURNING pool and
       not the dead rusted drum. Paolo 8/21: "Ofc ppl will warm themselves by barrel fire in
       act one." */
    15:{name:'fire barrel',       kind:'prop',       act1:'a steel drum somebody burns fence slats and furniture in, sheltered in the lee of the block wall -- the only warm thing on this street, and the proof that somebody is still out here'},
    /* kind:'vehicle', so the city's car branch stands one of the 20 approved top-down wrecks
       here without this district knowing anything about them. */
    16:{name:'dead car',          kind:'vehicle',    act1:'the car that never left: flat on its rims at the top of the drive, nose to the garage, glass crazed white on the sun side -- there was no fuel and nowhere to go'},
