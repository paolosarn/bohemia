...
/* WIRED IN THE GAME (8/11, SHOW IT IN A TAB NEVER A HUNT - his ruling, LOCKED):
   ...
   card. */
const WIRED = [
  { id: 'TF-ART-019', title: 'THE GRID HAS ITS MACHINES', img: 'ART_WIRED_TF-ART-019.png',
    what: 'The two districts that ARE the electrical grid were rendering as featureless slabs - the substation was one flat block field, the battery yard one giant smear of metal. Measured first: the world lays the substation in real transformer bays with firewall spacing, and the battery containers in banks exactly five forty-foot containers wide. Now the substation yard stands its transformers - the green-grey tank with its sky-lit top, radiator fin banks down both sides, three porcelain bushings stacked on top, the conservator drum - with the switchgear lattice running between them, the busbar conductors sagging OVERHEAD (you walk under them), and pedestal insulators dotting the gravel. All of it clean and straight, because the network is the eerily perfect half of who owns power in this valley. Next door the battery yard subdivides into container rows - corrugated lids with lit rims, dark slots between, door ends with locking bars, the dead thermal units with their stopped fans - cold and streaked, because that yard died. Live frames: the transformer bay on the left, the container banks on the right.' },
  { id: 'TF-ART-007', title: 'THE COURTHOUSE STOPPED BEING A TENNIS COURT', img: 'ART_WIRED_TF-ART-007.png',
    what: 'Two things at once, and the second is a confession. First, the civic buildings finished dressing: the courthouse draws its real precast panel seams on its own material (the world has named those joint cells all along - Clark County builds precast, so that is what your courthouse is), the chapel arcade stands its pale stone columns and keeps its dead leaded stained glass, and the dome, clerestory, oculus ring and the terminal curtain wall all wear dead dark glazing in thin steel mullions. Second: while wiring that, I found the courthouse was BLUE because of my own bug from the court-lines pass four days ago - the code matched any name containing "court", so the courthouse, the chapel memorial court, the industrial truck court, the library courtyard and the mall food court, nearly eight thousand cells, were all painted like basketball courts, and my composition audit missed it because it only walked the sports districts. Fixed at the root: only real sport-court names get acrylic now. Live frame at the courthouse wearing brick and seams instead of tennis blue.' },
  { id: 'TF-ART-016', title: 'THE SOLAR FARM READS AS POWER', img: 'ART_WIRED_TF-ART-016.png',
    what: 'The most recognisable industrial silhouette in the Mojave was drawing as a housing tract someone shrank - hundreds of little stucco wall squares standing on dirt. Now the solar farm is ranked panel tables: long near-black glass planes tilted at the sun, the galvanised back rail catching light on the high edge, the lit front lip over the deepest shadow in the district, torque-tube stubs with their one honest rust streak at every table end, and the pad-mount transformer cabinets standing on their poured pads. It is the largest light-against-dark contrast in the game and you can read the ranks from the next district over, which is the point of a landmark. And it is CLEAN - straight frames, an even dust haze, nothing sagging - because this plant still runs while the world around it is dead, and that is supposed to feel wrong. The battery yard next door is the other half of that story: its inverter racks are cold dark cabinets, one in seven hanging open and gutted, oil stains at their feet. Two power districts, one alive and eerie, one dead and looted. Live frame standing in the solar ranks.' },
  { id: 'TF-ART-017', title: 'THE BUILDINGS HAVE THICKNESS NOW', img: 'ART_WIRED_TF-ART-017.png',
    what: 'Until today every building in the valley was a cardboard flat: its vertical edges were lit and shaded strips that never turned, and a window was a picture pasted on the wall. Now the edges turn a second plane - a real 12-pixel return past the corner, which is exactly how thick a Clark County wall measures at this scale - with the value step that makes two planes read as two planes, and the one crack a thirty-year Vegas corner actually carries: the corner bead rusting inside the stucco, dead straight down the edge, with its little rust bleed. The windows became HOLES: a shaded jamb one side, a lit one the other, the soffit over the opening reading darkest the way a real underside does, and the only water stains a desert wall gets, starting at the sill corners and nowhere else. The whole thing is pure light-and-shadow geometry riding on top of whatever material the building already wears, so every house skin you approved and every civic material gets its thickness from one set of pieces. Houses, warehouses, the jail, everything with a corner. Live frame at the jail, before on the left, after on the right. SECOND SLICE 8/20: the INNER corners are in - where a building wraps around a pocket (the storage unit rows, the radio yard, thirty-five districts), the edge reads as the wind-eddy shade wedge with silt drifted into its foot instead of a sun-lit open corner, because the last place the sun reaches is where the wind drops its grit.' },
  ...
  { id: 'TF-ART-032', title: 'THE CHANNEL REMEMBERS ITS LAST FLOW', img: 'ART_WIRED_TF-ART-032.png',
    what: 'The riprap card promised the rest of the wash and here it is: 9,694 cells of the flood channel\'s own concrete - the invert floor and the sloped banks - were drawing as bare dirt between the two armor strips. Now the invert is jointed channel concrete with the dark meander stain of the last water that ever ran, wandering down the middle the way the real Las Vegas Wash carries its low-flow line, and the banks read darker with weep streaks running down the slope where groundwater seeped through the panel joints for thirty years. Two catches by looking: the weeps first came out LIGHTER than the slope (a weep is a shadow, not a highlight) and the first render put the stain on every cell at its own offset - a barcode - so now only the channel\'s centreline cells carry it, pinned to the tile centre at both edges so the line joins cell to cell. And the water plant got its floors free: 6,920 dry-basin cells joined your approved drought-bed cracks, because dried sludge and dried lake die the same way. Live frame in the channel.' },
  { id: 'TF-ART-031', title: 'WHAT FED THE LAST TRANSMITTER', img: 'ART_WIRED_TF-ART-031.png',
    what: 'The radio site had one more secret bigger than its wires: 3,376 cells named as propane tanks and ice bridges - the fuel banks that ran the backup generators and the elevated steel trays that carried the transmission lines from hut to tower - all drawing as bare ground. The measurement sorted the shared name into its two real things, so the art does too: the long thin runs are galvanised tray ribbons now, lit rims and crosswise ribs marching straight toward the masts on their little post shadows, and everything else is banks of bleached white 500-gallon cylinders - lit crown line, shadowed belly, rounded ends, saddle shadows, girth-weld rust on the old ones, the valve dome peeking over the top. A real transmitter site reads from the air as white tank rows plus silver ribbons converging on the towers, and now this one does. Live frame in the generator yard.' },
  { id: 'TF-ART-030', title: 'THE WIRES THAT HOLD THE TOWERS', img: 'ART_WIRED_TF-ART-030.png',
    what: 'The radio site\'s masts are guyed - held up by radial webs of steel wire running out to concrete anchors - and 2,798 cells of that rigging drew as nothing, which left flagpoles standing in empty scrub. Now the web is there: every wire cell computes the bearing to its NEAREST mast and lays a thin steel line pointing exactly at it, with a galv glint on the sun side and a faint shadow line offset south-east, so the whole fan converges on the tower the way the real KDWN array does from the air. Eight direction-snapped pieces cover the circle; a taut guy has no sag from above, so straight is honest - it is rigging, not hair. The anchor blocks and base plates the world already names catch the wires at the far end. Live frame in the fan at the radio site.' },
  { id: 'TF-ART-029', title: 'THE PLAZA NOBODY READS ON ANYMORE', img: 'ART_WIRED_TF-ART-029.png',
    what: 'The library fronts the most formal ground in the valley - 6,415 cells of entry plaza, forecourt, courtyard, plinth terrace and planters, the paving a civic architect actually drew - and all of it fell to the same gravel as a scrapyard. Now the plaza is back: pale concrete sawn into half-metre score grids with thirty years of dust settled INTO the joints (the grid darkens, the slabs stay pale - that is how real plazas age), the raised plinth the building stands on in bigger, lighter slabs because rain sheets off a plinth, and the planters full of dead soil and the dry straw rosettes of shrubs that died when the drip irrigation stopped. The first render drew a 5x7 planter blob as 35 little boxes - a waffle - and the look caught it: a planter that size is ONE BED, so the bed is one bed now, concrete rim only at its outside edge. Clark County Library and UNLV Lied are the references. Live frame on the entry plaza.' },
  { id: 'TF-ART-028', title: 'THE FORT THAT KEPT THE INTERNET', img: 'ART_WIRED_TF-ART-028.png',
    what: 'The datafort\'s hero is a four-hundred-thousand-square-foot box with no window anywhere - that is the most recognisable thing about it - and from above its 6,300 cells of roof drew as a smear of brown static. Now it reads like the satellite photo: the halls wear a pale membrane roof laid in real sheet courses, streaked grey where thirty years of drainage ran, a patch weld here, a clogged-drain ring there; the generator wings wear the darker standing-seam metal with oil stains and rust bleeding down the odd seam; and the cooling rows along every hall face are real chiller modules now, one per cell, each with its big fan ring and a STOPPED dark fan cross, because every fan in this building died with the grid. Front faces untouched - the walls keep their material. Membrane from your approved kerb pale cooled toward the approved galv so a roof never reads as desert sand. The Switch SUPERNAP campus in the real Vegas is the reference. Live frame on the hall roofs.' },
  { id: 'TF-ART-027', title: 'THE YARD KEEPS ITS DEAD TRAINS', img: 'ART_WIRED_TF-ART-027.png',
    what: 'The railyard is a district whose whole identity is cars stranded on tracks, and its 118 boxcars and dead locomotive - 3,346 cells of signature content - drew as windowless buildings. Now every car is a real one: measured on the walked world, 101 of the 118 are exactly 7x4 cells (a true 50-foot boxcar), and each blob gets ONE roof-read sprite the way your parked trailers already do - transverse panel seams, the running board down the centreline, end platforms with the brake wheel, a soft sun shadow east and south. Three weathers stand in one consist because a yard sorts cars from everywhere: faded oxide red, dusty bleach, the odd blue-grey repaint, all from your approved metal pools with rust blooming at the seams. The one dead switcher wears its radiator fan rings and cab band. No reporting marks, no numbers - the words are yours. Live frame on the classification tracks.' },
  { id: 'TF-ART-026', title: 'THE DUMP FINALLY LOOKS LIKE A DUMP', img: 'ART_WIRED_TF-ART-026.png',
    what: 'The landfill is a district whose whole identity is its ground, and its biggest surface - 6,400+ waste-fill cells per district cell, the fill itself - drew as the same yard gravel as everywhere else, while its berm rings drew as nothing at all. Now the fill reads true: dun cover soil rolled flat by compactors, dark refuse breaking through where the cover wore thin, the pale fleck of plastic that never rots and the odd rust scrap, one patch in three carrying the track pass of a compactor that died where it worked. And the earth berms that ring every fill cell stand up for free - they are the exact three-cell ridge your arsenal magazine berms already wear, so the approved pieces were rewired with zero new pixels: lit flank, shaded flank, crest. Apex Regional north-east of the real Vegas - the biggest landfill in America - is the reference, straight off the satellite. Live frame at the landfill, the fill field inside its berm ring.' },
  { id: 'TF-ART-025', title: 'THE LAKE THAT LEFT ITS BED', img: 'ART_WIRED_TF-ART-025.png',
    what: 'The intake named 4,109 cells of exposed lakebed - the drought bed Lake Mead left behind, the most photographed dead surface in Nevada - and it drew as plain gravel, along with the terminal hardpan and 7,390 cells of quarry floor across both pits. Now the drought bed cracks into pale silt polygons, half-metre plates with dark wandering seams and a sunlit lip on every crack, straight off the satellite photos. The pit floors read swept: bench rock polished flat by thirty years of loaders, the wheel-lane compaction still showing as broad matte bands after the machines died, the odd blast-scar drag line. And the golf course got its fairways back for free - 2,074 cells wired into the mow stripes your park lawns already wear, because a fairway is a lawn with a job. Live frames: the cracked bed on the left, the swept pit on the right.' },
  { id: 'TF-ART-024', title: 'SOMEBODY ONCE RAKED THESE BEDS', img: 'ART_WIRED_TF-ART-024.png',
    what: 'The police station named 1,131 landscaping cells - the xeriscape beds every civic building in this county wore, because the water rules pushed the whole valley to rock gardens decades before the crash - and they drew as wild dirt. Now the beds are back: fine warm granite mulch running in even bands around the station, its lot and the secure yard, clearly more MANICURED than the desert beside it, because rock stays raked for thirty years even when nobody comes back. About one cell in seven holds a dead agave, a dry straw rosette radiating from its dark heart, and the odd feature boulder sits where a landscaper placed it. No green anywhere - nothing here has been watered since the money died. Mulch from your approved dirt, straw and stone from your approved concrete. Live frame at the station.' },
  { id: 'TF-ART-023', title: 'THE WASH WEARS ITS ARMOR', img: 'ART_WIRED_TF-ART-023.png',
    what: 'The flood channel named 936 riprap cells - the dumped rock that armors its banks against scour - and they all drew bare dirt. Now two rock bands over a hundred cells long flank the channel and the scattered patches stand in the dead ground: packed angular stone, every rock a lit face over a shadowed face with dark earth in the gaps, a third of them warmed toward tan like real caliche. Three shuffled variants so the long strips never repeat. Facets from your approved kerb concrete, gaps from your approved dirt, drawn under the pixel craft laws (no outlines, no dots). Live frame at the wash, the armor strip running the channel edge. This is the wash’s first wired name; the channel bank and invert are next.' },
  { id: 'TF-ART-021', title: 'THE GRANARY LOADS OUT AGAIN', img: 'ART_WIRED_TF-ART-021.png',
    what: 'The grain elevator named 1,514 spout and dust-bin cells and they all drew dirt. Now the silo bases carry their cyclone dust collectors (squat steel bodies, lit rim, the cone falling to a dark throat) and the rail-spur side hangs its loadout spouts - the boom arm, the dark drop mouth, and the pale grain-dust bleach still on the concrete under each one after thirty years. A cell beside the spur gets a spout; everywhere else gets a cyclone. Steel from your approved galv, rust from the approved rail plates, all standing on the apron’s own concrete. VOLUME 8/26: the elevated conveyor GALLERY - the hundred-cell bridge running the whole silo row - wears its galvanized gable roof now: bright ridge cap, lit and shaded slopes, ribs running crosswise, one rust streak down the old side. The wide rail sheds deliberately keep their plain mass for now: the first attempt skinned them too and they read as one giant sheet of corduroy, so the roof stays where the roof is true. Live frame at the granary.' },
  { id: 'TF-ART-022', title: 'THE DEPOT CONTROLS ITS GROUND', img: 'ART_WIRED_TF-ART-022.png',
    what: 'The arsenal named 1,607 barricade-post cells across its storage ground and drew gravel. Now they stand as post-and-cable vehicle lines, the way a real ammo depot controls movement between its magazine rows without walls (blast safety wants open ground): short concrete-filled posts, half of them rusted through, strung with one sagging steel cable that joins seamlessly from cell to cell. Lines march between the berms and traverses, denser at the issue points, exactly where the world placed them. Live frame at the arsenal beside the new berms. Hawthorne, the real Nevada ammo depot an hour from Vegas, is the reference.' },
  { id: 'TF-ART-020', title: 'THE WATERWORKS HAS ITS HATCHES', img: 'ART_WIRED_TF-ART-020.png',
    what: 'The reservoir named 1,434 valve and hatch cells and every one drew dirt. Now the tank pad ring carries its round steel access lids (a real ellipse from the 45 view, hinge bar, rust weep on the old ones) and the buried transmission mains march their two-leaf vault covers through the setback - pale concrete collars, dark steel leaves with the centre seam, the way a service line actually reads from above. A lone cell is one lid; a cell in a corridor wears the vault on the run’s own axis. Steel from your approved galv roofs, rust from the approved rail plates, collars from the approved kerb concrete, everything riding on the pad’s own bought concrete. VOLUME 8/25: the buried reservoir’s DECK is in - 3,295 roof cells now read as a jointed concrete panel plate (a panel per cell, bitumen patches and hairline cracks here and there, a mushroom vent about every thirty-first cell because a buried tank breathes through its lid) - and the 1,028 OVERFLOW cells carry their splash basins: the collar, the dark weir mouth, and the dry stain fan from the last time water ever left. With that the whole waterworks reads finished. Live frame at the reservoir.' },
  { id: 'TF-ART-001', title: 'THE BLOCK WALLS GOT THEIR CAP', img: 'ART_WIRED_TF-ART-001.png',
    what: 'The plain concrete-block buildings (the unremarkable ones: solar, terminal, every district without a fancier material) finish properly now: a smooth poured cap beam runs the whole top course of the wall, and the odd vent block sits low in the courses. The orange starter parapet dies on these walls; the cap IS the top. VOLUME 8/24: THE ARSENAL BERMS - the twelve ammunition igloos each sit inside an earth berm ring the world already named, and it drew as a plain tan road until three looks proved what it was: a ring two cells thick, so it reads as a RIDGE now - lit flank, shaded flank, scrub and graded-earth mottle, the way a blast berm looks from above after thirty years. The concrete headwall and blast-door pieces are cooked and waiting in the bank for a shape that has a face; a berm has no headwall. Live frame at the arsenal, the ring around an igloo block.' },
];
