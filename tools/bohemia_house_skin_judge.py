...
// the composed house: roof top (3 tile rows) over the front face (1 tile row),
// on dead dirt - the exact read a suburb house has in the city
function compose(cv,m){
  ...
  if(m.cls==='roof'||m.cls==='yard'){ faceIds=DEFAULT_FACE; }
    ...
    else { faceIds[1]=m.id; faceIds[5]=m.id; } }
  // roof top
  const pat=ctx.createPattern(img(roofId),'repeat');
  ctx.fillStyle=pat; ctx.save(); ctx.translate(0,ROOF_Y); ctx.fillRect(0,0,W,FACE_Y-ROOF_Y); ctx.restore();
  // eave shadow between roof and face
  ctx.fillStyle='rgba(0,0,0,0.45)'; ctx.fillRect(0,FACE_Y-3,W,3);
  // front face
  for(let i=0;i<cols;i++) ctx.drawImage(img(faceIds[i]||faceIds[0]),i*T,FACE_Y);
...
function build(){
  ...
  for(const [name,cls,blurb] of SECTIONS){
    ...
    META.filter(m=>m.cls===cls).forEach(m=>{
      ...
      const cv=document.createElement('canvas'); cv.width=T*8; cv.height=T*3+T+24;
