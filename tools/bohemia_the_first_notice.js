...
/* ---- THE NOTICE TO QUIT (9/21, row [block strikes]). The oldest document in
        this family, and the one the block physically stands in front of. */
const quit = N.toQuit({
  at: [feeder.x, feeder.y], street: feeder.street, holder: feeder.s.faction,
  day: 2, clock: '07:40', out: N.WINDOW.finalDays
});
if (!quit.issued) { console.error('notice to quit refused: ' + quit.reason); process.exit(1); }

/* ---- THE COVENANT NOTICE (9/21, row [suburb walls]). The purest one in the
        family: a CC&R does not need a state to survive, so nothing about it has
        to be broken for it to be frightening. */
const cov = N.covenant({
  by: 'Sunridge Homeowners Association',
  at: [feeder.x, feeder.y], street: feeder.street,
  section: '4.2',
  violation: 'the perimeter wall on the east side of the lot is not maintained in its original colour',
  cure: N.WINDOW.disputeDays, day: 3, clock: '11:15'
});
...
court, or an office. <b>They run with the dirt forever.</b> So the fines are still
adding up, and the hearing you are owed is in front of a board that stopped
meeting years ago.</p>
...
This is the paper the man at the door was holding.</p>
<div class="ph"><div class="scr">${screen(quit.en)}</div></div>
