...
def main():
    ...
    h = sub(h,
        ...
        "  /* AMENDED SAME DAY, ON HIS WORD: \"the combat music is all fucked up...\n"
        "     try to find out what you previously did.\" The first cut of this called\n"
        "     MUS.stop() UNCONDITIONALLY here, and MUS.stop() CUTS THE MASTER GAIN TO\n"
        "     ZERO -- so opening combat reached into the studio engine and silenced it\n"
        "     even when it was not playing, right before musicPushToCombat sends the\n"
        "     song across. STOPPING A THING THAT IS NOT PLAYING IS NOT A NO-OP.\n"
        "     ONLY WHAT IS ACTUALLY MAKING NOISE GETS STOPPED NOW. That is all the\n"
        "     second-song bug ever needed: the city shuffle, when the city shuffle is\n"
        "     on. Nothing else is touched. */\n"
        "  if(t.dataset.p==='combat'){ try{ if(CITYMUS.on)CITYMUS.stopShuffle(); }catch(e){}\n"
        "                              try{ if(MUS.playing)MUS.stop(); }catch(e){} }\n"
