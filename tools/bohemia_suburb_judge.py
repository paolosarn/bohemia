...
function houseAt(g,cx,cy,hw,hh){ // returns true if placed
  var x0=(cx-hw/2)|0,y0=(cy-hh/2)|0;
  for(var y=y0-1;y<y0+hh+1;y++)for(var x=x0-1;x<x0+hw+1;x++){if(!inb(x,y))return false;if(g[y][x]!==0)return false;}
  for(var y2=y0;y2<y0+hh;y2++)for(var x2=x0;x2<x0+hw;x2++)g[y2][x2]=2;
  return true;
}
// place houses along every road cell, set back on each side, with a driveway
function lineHouses(g,r,setback,hw,hh,spacing){
  ...
  for(var i=0;i<roadCells.length;i++){
    ...
    // only frontage cells: has lawn on some side within setback
    for(var s=0;s<4;s++){
      var nx=[1,-1,0,0][s], ny=[0,0,1,-1][s];
      var fx=x+nx*(setback+hh/2), fy=y+ny*(setback+hh/2);
      var lawn=x+nx*2, lawnY=y+ny*2;
      if(!inb(lawn,lawnY)||g[lawnY][lawn]!==0)continue;
      if((k++ % spacing)!==0)continue;
      if(houseAt(g,fx,fy,hw,hh)){
        // driveway from house edge back to road
        for(var d=1;d<setback+2;d++){var dx=(x+nx*d)|0,dy=(y+ny*d)|0;if(inb(dx,dy)&&g[dy][dx]===0)g[dy][dx]=3;}
      }
    }
  }
}
...
function col(code){
  if(SUN){return ['#e9e0c8','#8a8a90','#6b5f4f','#c9b98a','#c9c2ad','#7ec77e'][code];}
  return ['#20281c','#3a3a44','#7a6f5f','#4a4030','#2a3220','#3f8c3f'][code];
}
...
function build(){
  ...
  STYLES.forEach(function(st){
    ...
    var row=document.createElement('div');row.style.cssText='display:flex;gap:8px;overflow-x:auto';
    for(var s=0;s<2;s++){var cv=document.createElement('canvas');cv.width=300;cv.height=300;cv.style.cssText='flex:0 0 auto;width:46%;max-width:300px;border-radius:6px;background:#000';row.appendChild(cv);
      var hc=render(cv,st.id,seedBase+s*97);}
