...
NEW_MAKE = """const DQ=BohemiaDemoQuests.make({BQ:BQ,BQRuntime:BQRuntime,sources:DEMO_BQ,loop:DAY,
  ...
  witness: function(file, stageN, questId){
    ...
    var pos = {}, minds = [], seenIds = {};
    for (var i = 0; i < drew.length; i++) {
      var d = drew[i]; if (!d || !d.p || !d.at) continue;
      pos[d.p.id] = { x: d.at[0], y: d.at[1] };
      minds.push(ctMind(d.p.id)); seenIds[d.p.id] = 1;
    }
    /* __CITY_JOBWITNESS__ -- AND THE MAN WHO GAVE YOU THE JOB FINDS OUT HOW YOU
       DID IT. HIS OWN FILE SAYS SO, and this reads it rather than deciding it:

           @ROLE lineman  REQ  faction=TRADES  block=browned

       REQ means the quest cannot be cast without him -- bohemia_bq.js fails the
       BUILD on an unfilled REQ role -- and `block=browned` binds him to the very
       block the job happens on. A required character bound to the place is IN
       THE SCENE. Optional roles are not added: you may never have met them.

       *** WHY THIS IS NOT THE KARMA IT REPLACES. *** The whole point of 8/28 was
       that a faction learns nothing it did not witness. This does not widen that
       to the valley -- it adds exactly the people the job WAS FOR, whom his own
       file marks required. Measured on the demo before it existed: 0 of 61
       loaded people affiliated, nearest base 30 cells, so you could do the
       Trades' job perfectly and the Trades would never hear of it. That is not
       a faction being unimpressed, it is a system nobody can reach.

       ctDayCast already casts these roles onto REAL people -- 380 people around
       the Trades' ground, 109 of them Trades, both roles filled -- and nothing
       had ever made them witnesses. The fid is stamped the same way
    ...
    try {
      ...
      for (var r = 0; r < qroles.length; r++) {
        ...
        var c = cast && cast[rl.name]; if (!c || !c.key) continue;
      ...
      if (!window.__JOBWITNESS_WARNED){ window.__JOBWITNESS_WARNED = 1;
        console.error('BOHEMIA: the quest cast could not be made witnesses, so '
          + 'the outfit that hired you learns nothing. ' + _e.message); }
    }
