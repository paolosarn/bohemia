 ...
 *
 *   node tools/bohemia_hardware_picture.js
 */
'use strict';
const fs = require('fs');
...
const OUT = path.join(REPO, 'slices/look/hem.png');
...
(async () => {
  ...
  const png = await p.evaluate(() => {
    ...
    const ROWS = [
      /* CROP WHERE THE CHANGE IS. A hem is at the BOTTOM of a garment, and the first
         cut of this picture framed the chest -- so three of four rows showed a perfect
         before/after of a region nothing had touched. */
      ['A SHIRT',     'base',  'WHITE TEE',         'S', 0.40, 0.58],
      ['A FLANNEL',   'base',  'SAGE FLANNEL',      'S', 0.40, 0.58],
      ['A LONG COAT', 'outer', 'WASTELAND DUSTER',  'S', 0.58, 0.78],
      ['A VEST',      'outer', 'LEATHER VEST',      'S', 0.40, 0.60],
    ];
    ...
    const Z = 16, PAD = 20, LBL = 190, HDR = 126, X0 = 0.30, X1 = 0.70;
    ...
    cx.fillText('THE HEM', PAD, 42);
    cx.font = '16px monospace'; cx.fillStyle = '#b8ab95';
    cx.fillText('cloth is hemmed, and the stitching is one pixel.', PAD, 72);
    cx.fillText('your shirts and coats had none, because there was no room.', PAD, 94);
    ...
    for (const r of ROWS) {
      ...
      /* NAME IT ONLY IF IT EXISTS. A garment name that is not in the catalogue draws
         NOTHING and the row looks like a real before/after where nothing changed --
         which is exactly the lie the last picture told before I checked the names. */
      if (!(window.GARMENTS || []).some(q => q.n === nm)) {
        cx.fillStyle = '#c05a4a'; cx.font = 'bold 12px monospace';
        cx.fillText('NOT IN THE CATALOGUE', PAD, y + 56); }
      /* *** THE TOOL MUST NOT BE ABLE TO SHOW A ROW WHERE NOTHING DIFFERS. ***
         Three pictures this week showed two identical halves for three different
         reasons: a flag one generator never read, a garment name that was not in the
         catalogue, and a crop framed away from the change. Every one of them looked
         like an honest "the change did nothing". Count the differing pixels INSIDE THE
         CROP and say so on the row when it is zero, so the picture can never quietly
         claim a null result it did not measure. */
      const A = frame(lay, nm, d, true), B = frame(lay, nm, d, false);
      let diff = 0;
      ...
      if (!diff) { cx.fillStyle = '#c05a4a'; cx.font = 'bold 12px monospace';
        cx.fillText('NO DIFFERENCE IN THIS CROP', PAD, y + 74); }
      blit(A, LBL + PAD, y, y0, y1);
      blit(B, LBL + PAD + cw + PAD, y, y0, y1);
      y += hOf(r) + PAD;
    }
    window.G_WORN = keep;
    try { HD_CACHE.map.clear(); FRAME_CACHE.map.clear(); } catch (e) {}
    cx.fillStyle = '#6f6455'; cx.font = '13px monospace';
    cx.fillText('22 of your 204 clothes still have no fine detail. it was 102 on friday.', PAD, cv.height - 30);
    cx.fillText('the stitch sits just above the fold, like real hemming. nothing moved or got thinner.', PAD, cv.height - 12);
