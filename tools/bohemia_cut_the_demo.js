...
function cut(src) {
  ...
  const epilogue = [
    ...
    '</script>',
    '',
    '<!-- THE DOOR TO THE FAMILY SCENE WAS TOO SMALL TO HIT.',
    '     FAMILY is the load-bearing theme of this whole game (7/19, LOCKED: "you do',
    '     not play a hero, you play a FAMILY across three generations"), and the cold',
    '     open -- DENISE, NINA, RAY and MARCO at the table, ten years ago -- is the',
    '     first and only place it reaches a new player. Its two buttons, WATCH and NOT',
    '     NOW, measured 79x25 on the demo at 390x844. THE THUMB says 44px minimum.',
    '     25 is 57% of it, on the FIRST TWO BUTTONS ANYBODY EVER TOUCHES IN THIS GAME.',
    '     AND THE THUMB GATE HAD NEVER LOOKED AT THEM, because it swept the CITY FRAME',
    '     and this overlay lives in the OUTER document -- an exemption written for',
    '     myself, which is the exact failure that gate\'s own header warns about. The',
    '     gate sweeps every document now.',
    '     Height only, and the width is left alone: these sit in a flex row inside a',
    '     390px overlay and stretching them is a layout change in another lane\'s',
    '     scene. The workshop keeps its 25px and the row is filed. -->',
    '<style id="demo-cut-thumb">',
    '  #openWatch,#openNot,#openSkip{min-height:44px !important;',
    '    display:inline-flex !important;align-items:center !important;',
    '    justify-content:center !important}',
    '</style>',
    '',
    '<!-- THE DEMO DOES NOT OFFER THE COLD OPEN (9/12, RUN). Two rulings and one',
    '     measurement meet here.',
    '     PAOLO 9/11: "we dont have a story yet bro so I dont have the final answer',
    '     for you. Your job is to make sure the rest of the game is playable and',
    '     fun." The coordinator amended [one question] the same round: step (2) THE',
    '     COLD OPEN IS DEFERRED WITH THE STORY, so it is no longer part of the',
    '     opening order at all.',
    '     AND MEASURED ON THIS EXACT BUILD: the cold hand presses the loudest thing',
    '     forty times and the SECOND thing it ever presses is the invite\'s own',
    '     WATCH -- a filled gold button on a dark screen, correctly the most',
    '     inviting thing there is. Tapping it starts a scene that NEVER ENDS: the',
    '     canvas changed once in fifty seconds, then held the same pixels for',
    '     eighty-five, still mid-flight at 128 seconds, nothing thrown. The walked',
    '     game sits underneath it, which is why the clock never moved.',
    '     SO THE DEMO STOPS OFFERING A DEFERRED SCENE THAT HANGS. HIDDEN FROM THIS',
    '     SIDE ONLY, the same way the builder drawer is: the cutscene engine is',
    '     another lane\'s file and this tool does not touch it, THE WORKSHOP KEEPS',
    '     THE INVITE AND THE SCENE, and the demo simply does not show the door to',
    '     it. When the scene ends on its own again, deleting these three lines',
    '     gives it straight back. -->',
    '<style id="demo-cut-coldopen">',
    '  #openInvite{display:none !important}',
    '</style>',
    '<script>',
    '',
    ...
    '  /* THE RULE ABOVE HAS BEEN INERT SINCE 9/7, AND THAT IS NOT A BUG TO FIX',
    '     HERE (measured 9/13, RUN). The pad was rebuilt from html buttons into an',
    '     SVG ring of eight <g> wedges, and CSS width/height DO NOT APPLY TO AN SVG',
    '     GROUP, so this line no longer touches the walk arrows at all.',
    '     I GREW THE WHOLE RING TO PUT THEM BACK OVER 44 AND MEASURED IT FIGHTING A',
    '     LOCKED RULING: Paolo 9/6, \"for the run right now make all the UI 50%',
    '     smaller, I dont give a fuck.\" NEWEST DATE WINS, the pad is one of the',
    '     controls the halving shrank, and thumb_gate already carries that exemption',
    '     -- narrow, named, and printed every run. The arrows are 20px BY HIS OWN',
    '     WORD. The line stays only because it still binds anything on the pad that',
    '     is not an SVG group; the 44 proxy in DEMO CURRENT was the thing that was',
    '     wrong, and that is where the fix went. */',
    '  /* AND THE FOUR MOST MEANINGFUL BUTTONS IN THE GAME (9/12, RUN). Measured on',
    '     this build, standing next to a scavenger with the conversation open:',
    '     ctask "Ask their name" 146x31, ctgive "Show up for them" 162x31,',
    '     ctfavour "Take it" 87x31, ctgo "Leave them to it" 162x31.',
    '     THIRTY-ONE PIXELS against a 44px law, and two of them SPEND AN HOUR OF THE',
    '     DAY when pressed (measured: +60 minutes each). No checker had ever seen',
    '     them, because none stands next to a person and opens a conversation -- the',
    '     thumb sweep only ever measured the first screen.',
    '     AND THE FIRST CUT OF THIS PUT THE RULE IN THE SHELL, WHERE IT DID NOTHING:',
    '     the conversation lives in the city FRAME, a different document, so a style',
    '     tag in the demo page cannot reach it. It belongs here, with the walk pad.',
    '     Height only, width left alone: they sit in a wrapped row inside a 390px',
    '     card and stretching them is a layout change in another lane\'s scene. */',
    '  \'#ctask,#ctgive,#ctfavour,#ctgo{min-height:44px !important;\'',
    '    + \'display:inline-flex !important;align-items:center !important;\'',
    '    + \'justify-content:center !important}\',',
    ...
    '  /* *** THE THUMB FLOOR ABOVE WAS ACCUSED OF THIS AND IT IS INNOCENT. ***',
    '     VAMILY [cutter flex]. The board says one line in this file causes Paolo\'s',
    '     "one button, ready to go, I press it, nothing happens", because',
    '     display:flex !important on #topbar>* and #blstack>* beats the INLINE',
    '     display:none the game hides a useless chip with. The reasoning is sound and',
    '     THE CONCLUSION IS WRONG, measured on the glass with the one driver: disable',
    '     this entire stylesheet at runtime and MARKET IS STILL ON SCREEN.',
    '     THE REAL CAUSE IS IN THE WALKED CITY, so it was never demo-only and the alpha',
    '     had it too: the 9/6 halving sheet declares',
    '         #blstack > .uihalf{ ... display:flex!important ... }',
    '     which is id+class, out-specifies the id+universal here, and sits in a later',
    '     stylesheet. It wins twice over. The fix went there, next to its cause.',
    '     THE RULE BELOW STAYS ANYWAY, and it is not decoration: it covers any chip in',
    '     these two bars that the game hides inline and the halving sheet does not',
    '     claim, which is every one that is not .uihalf. It is a second lock on the',
    '     same door, written knowing it is not the lock that was open. */',
    '  \'#topbar>[style*="display: none"],#topbar>[style*="display:none"],\'',
    '    + \'#blstack>[style*="display: none"],#blstack>[style*="display:none"]\'',
    '    + \'{display:none !important}\',',
    '',
    '  /* ====================================================================',
    '     *** THE CUT SUBTRACTS NOW. RULE 18g AND 18i, VAMILY [cut now]. ***',
    '',
    '     PAOLO 9/20: "I\'m overwhelmed and underwhelmed at the same time... a lot',
    '     going on and I see it but this shit is broken right now."',
    '     PAOLO 9/22: "I\'m really trying to push this demo out... I DON\'T NEED THE',
    '     WHOLE WORLD, I don\'t need everything." The four things and nothing else.',
    '',
    '     MEASURED FIRST, on the served cut at 390 x 844, every box with an id that a',
    '     player can actually see on the FIRST SCREEN. Twenty-four of them. Fourteen',
    '     were things rule 18g names for the bin:',
    '',
    '         MUSIC  SAVE                      the rail of chips',
    '         HUMAN MODE   SUBURB - ON FOOT    the mode and place readouts',
    '         DAY 1 - 06:00  (twice, the bar and the clock inside it)',
    '         STANDING  BUILD HERE  SCAVENGE - 8H  BIKE  SLEEP  MARKET',
    '         "walking your own block."         prose with no mouth (rule 19)',
    '',
    '     WHAT STAYS IS THE LIST, NOT MY TASTE: the world, the pad, the round action',
    '     button and its label (that is how a fight starts, item 3), NOTES (his own',
    '     direct ask, 9/22, and he had already complained he could not find it), the',
    '     phone, and the person who speaks.',
    '',
    '     *** NOTES IS NOT PUT BEHIND A GEAR, AND THAT IS DELIBERATE. *** 18g says',
    '     "the gear with the NOTES button behind it". With MUSIC and SAVE gone, the',
    '     right of the bar holds ONE control, which is what a gear was for. Building a',
    '     door to hide NOTES behind, one round after he said "how do I access the',
    '     notes... I cannot find them", would be adding a thing AND taking away the',
    '     thing he asked for. Less on screen is the point; a new button is not less.',
    '',
    '     *** AND THE FEED IS NOT STRIPPED, ALTHOUGH 18g LISTS IT. *** Newest date',
    '     wins: 18g was written 9/20, and on 9/22 he played it and said "I\'m very',
    '     impressed with the phone", which approved its look AND its feed, and he',
    '     ruled THE DRAWN PHONE IS THE PHONE BUTTON. So the feed is not a feed any',
    '     more, it is the phone and the only door to it. Stripping it would delete',
    '     the phone, which is on 18g\'s own KEEP list, to obey 18g\'s strip list.',
    '',
    '     HIDDEN FROM THIS SIDE ONLY, like the builder drawer and the cold open above:',
    '     the workshop and the alpha keep every one of these, with their code, their',
    '     handlers and their gates. Deleting this block gives them all back.',
    '     ==================================================================== */',
    '  \'#musbtn,#savebtn,\'',
    '    + \'#hmode,#hslot,\'',
    '    + \'#barmid,#hclock,\'',
    ...
    '    + \'#note\'',
    '    + \'{display:none !important}\',',
    '  /* AND NOTES STAYS ON THE RIGHT, WHERE HIS THUMB LEFT IT. Measured after the',
    '     first strip: with the left and middle groups emptied, the bar\'s only child',
    '     collapsed to x=16 and the one button in the game jumped to the FAR LEFT,',
    '     under the hand that is not holding the phone. Taking things away moved the',
    '     thing that stayed, which is a regression dressed as a cleanup. */',
    '  \'#barright{margin-left:auto !important}\'',
    '].join(\'\\n\');',
    '',
    '(function(){',
    '  var last = null;',
