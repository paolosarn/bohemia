      ...
      if(_pf && PROP_IMG[_pf] && PROP_IMG[_pf].length){
        ...
        if(_pf==='lighttower'){
          ...
          /* A FIELD IS SCATTERED, NOT ANCHORED. One-per-blob is right for a bin and absurd
             ...
             on a LATTICE instead -- roughly one heap per 4x4, and not at every station, so
             the ground reads as spill and collapse rather than as wallpaper. */
          if(((gx&3)===((_ph>>>2)&3)) && ((gy&3)===((_ph>>>5)&3)) && ((_ph%100)<62))
            c.post={p:_pf, v:_ph%PROP_IMG[_pf].length};
        } else {
          var _pw=(lx>0)?(m.kit[ly*FN+lx-1]===code):false;
          var _pn=(ly>0)?(m.kit[(ly-1)*FN+lx]===code):false;
          if(!_pw&&!_pn) c.post={p:_pf, v:_ph%PROP_IMG[_pf].length};
        }
      }
    }
