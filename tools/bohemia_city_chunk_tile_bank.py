...
if '<!-- __TILE_BANK__' in page:
    ...
    # *** CARRY FOREIGN TAGS FORWARD (8/27). ***
    # This used to be `page[:i] + many + page[j:]`, which replaces the WHOLE region and
    # therefore DELETES anything else another lane put inside it. That is not theoretical:
    # the ART lane's room-floor data loads from one line here, and every single run of this
    # tool removed it -- so slices/BOHEMIA_CITY_FLOORS.js stayed tracked, stayed in the
    # deploy list, and NOTHING LOADED IT. window.FLOOR_POOL_B64 was undefined on the page
    # Paolo walks and every interior floor fell back to the default.
    # IT LOOKED LIKE A REBASE ATE IT, and I fixed it as one. It came straight back the next
    # time the chunker ran, which is how the real cause surfaced: a TOOL that rewrites a
    # region owned by nobody in particular, and quietly discards its neighbours.
    # THE FILE SURVIVING IS WHY IT IS INVISIBLE -- a deleted file is loud, an orphaned one
    # looks exactly like a working one. gates/no_orphan_script_gate.js is the net.
    # So: keep every script tag in the region that is not one of OURS, verbatim, with its own
    # marker comments intact. Unrecognised is KEPT, never dropped -- when this tool cannot
    # tell whose a tag is, the safe answer is the one that does not delete somebody's work.
    _ours = set([os.path.basename(q) for q in paths] + [LOAD, READY])
    _keep = []
    for _line in page[i:j].split('\n'):
        _srcs = re.findall(r'<script[^>]*\bsrc="([^"]+)"', _line)
        if _srcs and not any(os.path.basename(u) in _ours for u in _srcs):
            _keep.append(_line)
    if _keep:
        print('CHUNK: carried %d foreign tag(s) through the tile-bank region: %s'
              % (len(_keep), ', '.join(re.findall(r'src="([^"]+)"', ' '.join(_keep)))))
    page = page[:i] + many + ('\n' + '\n'.join(_keep) if _keep else '') + page[j:]
elif one in page:
