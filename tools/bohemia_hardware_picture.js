...
const OUT = path.join(REPO, 'slices/look/hardware.png');
...
(async () => {
  ...
  const png = await p.evaluate(() => {
    ...
    const ROWS = [
      ['CHEST PLATE', 'gear', 'SCRAP CHEST PLATE', 'S', 0.26, 0.46],
      ['SHIN GUARDS', 'gear', 'SHIN GUARDS',       'S', 0.58, 0.80],
      ['KNEEPADS',    'gear', 'SCAV KNEEPADS',     'S', 0.46, 0.66],
      ['THE PACK',    'back', 'RUCK PACK',         'N', 0.24, 0.50],
    ];
    ...
    cx.fillText('THE HARDWARE', PAD, 42);
    cx.font = '16px monospace'; cx.fillStyle = '#b8ab95';
    cx.fillText('scavenged armour is bolted together. yours had no rivets.', PAD, 72);
    cx.fillText('a rivet is one pixel, so it could not exist until now.', PAD, 94);
    ...
    cx.fillText('44 of your 204 clothes still have no fine detail. it was 102 three days ago.', PAD, cv.height - 30);
    cx.fillText('a rivet recolours a pixel the plate already owned. nothing moved or got thinner.', PAD, cv.height - 12);
