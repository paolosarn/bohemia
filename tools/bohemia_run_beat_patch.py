...
# THE DEV SOURCE, NOT THE BUILD. slices/BOHEMIA_RUN_CURRENT.html is GENERATED by
# tools/build_run_slice.js and says "never edit this file directly" in its own
# header. The first version of this tool patched the generated file, and
# gates/run_gate.js caught it exactly the way it is supposed to: "regenerating
# changes nothing" went red, because the next person to run the builder would
# have silently deleted the beat receiver. Patch the source, then rebuild.
...
def main():
  ...
  try{ if(ev&&ev.data&&ev.data.type==='BOHEMIA_RUN_BEAT'){ RB.take(ev.data); return; } }catch(_e){}""", 1)
    ...
    # and REBUILD, so the generated run carries it and regeneration is a no-op
    import subprocess
    r = subprocess.run(['node', 'tools/build_run_slice.js'], capture_output=True, text=True)
    if r.returncode != 0:
        print('FAIL: the run slice would not rebuild:\n' + (r.stderr or '')[-800:])
        return 1
    print('  ' + (r.stdout or '').strip())
    if 'RUN BEAT RECEIVER' not in open(BUILT, encoding='utf8').read():
        print('FAIL: the rebuilt run does not carry the receiver')
        return 1
