...
<style>
  ...
  *{box-sizing:border-box}
  ...
  /* THE PAIR NEVER WRAPS. He votes on a phone, and the whole claim is a COMPARISON:
     stacked, he has to scroll from one face to the other and the point is gone. So no
     wrap, and the canvases size to the viewport instead of to a fixed 256. */
  .pair{display:flex;gap:10px;flex-wrap:nowrap;margin-bottom:10px;align-items:flex-start}
  .side{background:var(--card);border:1px solid var(--line);border-radius:6px;padding:7px;
        text-align:center;flex:1 1 0;min-width:0}
  .lab{font-size:10px;letter-spacing:1px;margin-bottom:5px;color:#6f6862;
       white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
  .hot{color:#c79a3f}
  canvas{width:100%;max-width:256px;aspect-ratio:1/1;height:auto;
         image-rendering:pixelated;background:#101319;border-radius:3px;display:block;margin:0 auto}
