...
/* THE MOBILE RENDER CONTRACT (laws/BOHEMIA_MOBILE_RENDER_CONTRACT_7_26_26.md):
   "smoothing off (non-integer scaling voids pixel art on 3x phones)". This page
   never set it, so it took the browser default of TRUE and bilinear-filtered
   every approved tile it drew - worst on exactly the phones Paolo looks at it
   on. Set here AND in fit(), because assigning cv.width/cv.height RESETS the
   whole 2D context state, so a resize silently turns smoothing back on. */
function crisp(c){ c.imageSmoothingEnabled=false;
  c.mozImageSmoothingEnabled=false; c.webkitImageSmoothingEnabled=false; c.msImageSmoothingEnabled=false; }
crisp(ctx);
...
function fit(){ cv.width=cv.clientWidth; cv.height=cv.clientHeight;
  crisp(ctx);                      /* the resize just wiped the context state */
