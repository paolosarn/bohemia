...
SONGS = [
    ...
    ("MENU — WHAT THE VALLEY KEPT",
     "{n:'MENU — WHAT THE VALLEY KEPT',menu:true,acc:'#9a8ac8',root:43,"
    ...
    ("MENU — NOBODY IS COMING",
     "{n:'MENU — NOBODY IS COMING',menu:true,acc:'#7a8a9a',root:38,"
...
def main():
    ...
    add = [lit for name, lit in SONGS if name not in have]
    if add:
        s = s[:j0] + ',\n ' + ',\n '.join(add) + s[j0:]
    ...
    missing = [n for n, _ in SONGS if n not in names]
    if missing:
