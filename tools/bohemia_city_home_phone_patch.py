...
function homePass(ox,oy,C){
  ...
  const sx=Math.round(ox+(h.x+h.w/2)*C), sy=Math.round(oy+h.y*C-Math.round(C*0.55));
  if(sx<-60||sy<-30||sx>cv.width+60||sy>cv.height+30)return;
  g.save();
...
/* TRUE AT BOOT ON PURPOSE, and this was a real bug for one measurement: the day
   loop's own first DAY.wake() runs where it is constructed, which is BEFORE this
   file gets to register a wake hook, so the first wake of a run was never observed
   and he dropped in 26 cells from a house he never saw. A run begins at 06:00 on
   day 1, which means he has just woken up, so the pending flag starts SET. */
var HOME_WAKE_PENDING=true;
function homeWake(){
