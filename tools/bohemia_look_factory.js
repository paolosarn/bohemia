...
const TYPE_ONLY = `/* GENERATED by tools/bohemia_look_factory.js. Do not edit.
   Paolo 8/27: ALL TYPEWRITER-WIDTH, C CUT, B HEAVY. Embedded, nothing fetched.

   *** HIS SHAPE, THIS LANE'S COLOURS. *** The city has its own --acc, --line,
   --ink and --bg, older than mine and belonging to another lane, and this block
   does not touch one of them. What it brings is the SHAPE he ruled -- the cut
   corner, the heavy line, one drawn arrow -- built out of THAT lane's palette,
   so the surface he plays stops being half in one look and half in another
   without anybody's room getting repainted from the hallway. */
${faces}
:root{
  --fmono:'BohemiaMono',ui-monospace,"SF Mono",Menlo,Consolas,monospace;
  --bcut:${CUT}px; --bbw:${BW}px;
  --bclip:${poly(CUT)};
  --bclipin:${poly(CUTIN)};
}

/* ---- THE BOX, in the city's own colours ---------------------------------- */
#sleepbtn, #rungbtn, #mktbtn, #bikebtn, #modechip, .dcgo{
  position:relative; isolation:isolate; border:0; border-radius:0;
  background:var(--line);
  -webkit-clip-path:var(--bclip); clip-path:var(--bclip); }
#sleepbtn::before, #rungbtn::before, #mktbtn::before, #bikebtn::before,
#modechip::before, .dcgo::before{
  content:''; position:absolute; inset:var(--bbw); z-index:-1; background:var(--face);
  -webkit-clip-path:var(--bclipin); clip-path:var(--bclipin); }
/* PRESSED IS A FLIP (his, 8/27 14:12): the whole box inverts, because a thumb
   covers the middle. */
#sleepbtn:active, #rungbtn:active, #mktbtn:active, #bikebtn:active,
#modechip:active, .dcgo:active{ background:var(--acc); color:#14100a; }
#sleepbtn:active::before, #rungbtn:active::before, #mktbtn:active::before,
#bikebtn:active::before, #modechip:active::before, .dcgo:active::before{
  background:var(--acc); }
/* THE LEFT COLUMN IS ONE CONTROL GROUP AND IT HAS TO LOOK LIKE ONE. #modechip is
   the CITY button and it was the only one of the five still square, which reads
   as "this one is different" about a button that is not. The ids were found by
   asking the live page what is in that column, not by grepping the file: a
   control built in JavaScript is not in the markup to grep for. */
/* *** #hmode IS NOT A BUTTON AND A BOX ON IT CUT HIS WORDS IN HALF. *** It is the
   HUD's own mode LABEL. The first cut swept it up with the four real buttons, and
   the chamfer that makes a stamped tag out of a button sliced the H off HUMAN MODE
   on the first screen of the demo. A shape rule has to know what it is shaping. */

/* ---- THE TOP BAR GETS ITS MARGIN BACK -----------------------------------
   Typewriter type is wider than the face this bar used to ask for and never
   load, so the day line ran from one edge of the phone to the other with
   nothing either side of it. */
body #hud{ padding:4px 10px; letter-spacing:1px; }
/* SPECIFICITY, NOT ORDER. This block is stamped at the TOP of the city's own
   stylesheet, so its own #hud rule comes later and wins a tie. A body-prefixed rule is one
   step heavier and settles it without moving anybody's code around. Measured
   before: 2px letter-spacing, 41 characters of mono, and the line ran off both
   ends of a 390px phone. */

/* ---- THE EIGHT ARROWS ARE DRAWN, AND BOTH FACES SURVIVE -----------------
   The ring was eight glyphs and no font carries all eight in one weight, so
   four came out thin and four came out heavy inside the same control.
   AND IT HAS TWO FACES, WHICH A NAIVE FIX WOULD HAVE THROWN AWAY: a single
   arrow means one step, a DOUBLE means a ninety-six metre map move, and the
   pad says which by toggling .mapmove on itself. So the shape is drawn twice --
   one triangle for walking, two stacked for travelling -- and the meaning the
   other lane built is kept exactly. */
.pb{ font-size:0; color:transparent; }
.pb::before, .pb::after{ content:''; position:absolute; left:50%; top:50%;
  width:0; height:0; border-left:6px solid transparent; border-right:6px solid transparent;
  border-bottom:14px solid var(--acc);
  transform:translate(-50%,-57%) rotate(var(--ba,0deg)); transform-origin:50% 50%; }
.pb::after{ display:none; }
/* travelling: two chevrons, the same shape said twice */
#pad.mapmove .pb::before{ transform:translate(-50%,-90%) rotate(var(--ba,0deg)) }
#pad.mapmove .pb::after{ display:block; transform:translate(-50%,-14%) rotate(var(--ba,0deg)) }
/* KEYED ON THE BUTTON'S OWN DIRECTION, NOT ON ITS POSITION IN THE LIST. The
   first cut used :nth-child and two of the eight came out pointing the wrong
   way, because the pad holds more than the eight buttons and the count slid.
   Every button already carries data-walk with the direction it means, put there
   by the lane that built it -- so the shape reads the meaning instead of
   counting siblings, and it cannot slide again if anything is ever added. */
.pb[data-walk="↑"]{ --ba:0deg }
.pb[data-walk="↗"]{ --ba:45deg }
.pb[data-walk="→"]{ --ba:90deg }
.pb[data-walk="↘"]{ --ba:135deg }
.pb[data-walk="↓"]{ --ba:180deg }
.pb[data-walk="↙"]{ --ba:225deg }
.pb[data-walk="←"]{ --ba:270deg }
.pb[data-walk="↖"]{ --ba:315deg }
.pb:active::before, .pb:active::after{ border-bottom-color:#fff; }
`;
