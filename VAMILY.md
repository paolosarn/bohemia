...
MODE: BUILD
STATE: build verbs and a picker EXIST in the aerial tab (engine/bohemia_cityedit.js, cityTapPlot) and the panel is PROVEN under a thumb on an iPhone profile (9/5). Placed buildings now PRODUCE on the wake beat: engine/bohemia_production.js walks the delta and calls the purse's produce(), 59 buildable types carry a row, resources 0 -> 1 across a night on the walked surface, and the morning card says so (9/5). NOT built: nothing houses anybody, building is free, the builder is not reachable from the walked surface or the demo, no century record exists. KNOWN: the demo is ONE DAY (CT_DEMO_DAYS=1) so no second wake beat exists there, and the demo cut hides the builder drawer -- both belong to [builder reachable]. Unverified: which building types make electricity or clout instead of resources is [PENDING Paolo].
...
- CLAIMED 9/5 city-1eztay  [building costs]  BUILD-COSTS-ITS-PRICE -- CE.build debits PRICES; building is free today and the 8/15 law says the pipe must be exercised
...
- CLAIMED 9/5/26 economy-knxaeh  [numberless economy]  Q5  An economy with no numbers on screen. How the best games show scarcity, price and wealth without a spreadsheet. Against our one-number rule.
