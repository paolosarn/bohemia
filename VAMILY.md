...
- SHIPPED 9/5 (7dd2383)  [first pictures]  E0  FIRST ROUND, BEFORE ANY RESEARCH: take a real screenshot of every tab of the alpha and the demo at iPhone size with the Chromium that is installed here, save them in one folder with a one-line note under each saying what looks wrong to you, commit it, and tell him where to look. He must SEE something on round one (Paolo 9/4: "I hope it's not a massive let down"). Then E3 builds the machine that does this every ship.
