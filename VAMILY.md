...
MODE: BUILD
STATE: build verbs and a picker EXIST in the aerial tab (engine/bohemia_cityedit.js, cityTapPlot) and the panel is PROVEN under a thumb on an iPhone profile (9/5). Placed buildings now PRODUCE on the wake beat: engine/bohemia_production.js walks the delta and calls the purse's produce(), 59 buildable types carry a row, resources 0 -> 1 across a night on the walked surface, and the morning card says so (9/5). BUILDING COSTS ONE BATTERY out of the same pocket the shop charges, checked before the tap and refused in words when he is broke, proven on the walked surface and in the cut demo (9/5). NOT built: nothing houses anybody, the builder is not reachable from the walked surface or the demo, no century record exists. KNOWN: the demo is ONE DAY (CT_DEMO_DAYS=1) so no second wake beat exists there, and the demo cut hides the builder drawer -- both belong to [builder reachable]. Unverified: which building types make electricity or clout instead of resources is [PENDING Paolo].
