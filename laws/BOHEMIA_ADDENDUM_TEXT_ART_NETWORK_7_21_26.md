...
## THE CAMERA APP (built later same conversation, 7/21)
Camera was the last dimmed "soon" home-screen tile; it's now built and completes the phone's app roster. It makes the TEXT-CAM concept playable instead of only ever happening automatically on quest completion: tap the shutter, the viewfinder "develops" (a beat of delay, matching the analog/low-tech feel already established elsewhere), and reveals a text-art shot pulled from the same art bank the feed's photos already draw from. Captured shots collect in a small gallery grid on the same screen.

Also generalized the underlying art system while building this: the 4 quest-specific TEXT-CAM pieces from the first pass (water tower, antenna, tunnel, fence) are now an explicit override table for THIS demo's specific quests; any OTHER quest id — including any real quest not yet written — falls back to a shared `ART_BANK` (8 more pieces: cactus, sunrise ridge, radio dish, toolkit, campfire, bird on a wire, broken sign, boots by a door), picked deterministically by a hash of the quest id, the same zero-manual-authorship pattern already used for avatar colors and voice archetypes. This means the system scales automatically to quests that don't exist yet — a real content gap this session kept surfacing ("we don't even have actual quests yet") without requiring hand-authored art for each one as they get written.
