...
PENDING — Paolo's calls (the real forks this raises, NOT decided):
- DIEGETIC vs META split: does the phone also hold the META layer (save/load, settings, the 100-year dynasty tree), or does meta live outside the phone frame? A phone is in-world; a save menu is not.
- DEAD ZONES / SIGNAL: the phone is the menu, but the phone needs signal. The tunnels (homeless) and maybe combat have no signal. If your MENU is the phone, what happens to it in a dead zone — does the UI itself degrade (world reaching into your interface), or is the menu always available and only the FEED/posting goes dark? This is the sharpest design question the phone-as-menu idea creates.
