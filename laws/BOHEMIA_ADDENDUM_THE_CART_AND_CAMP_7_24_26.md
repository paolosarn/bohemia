...
## BASE UPGRADES — THE SOLAR PANEL QUEST (LOCKED 7/24, Paolo — base-building, NOT the opening)
Distinct from the opening battery-recovery (see OPENING_QUEST_CRAFT: the thieves took the
charged power station; that hunt is about getting your stuff BACK). Getting a SOLAR PANEL is
its own later quest (side / ~third quest, timing Paolo's call) and it's a BASE-BUILDING
progression: mount it on the compound and your house stops merely BURNING a stored battery and
starts GENERATING its own power forever. The rebuild arc in two quests: first claw back what
you had (battery), then build past it into what you never had (self-sufficiency).
- ENGINE-REAL PAYOFF: the panel is a personal ELECTRICITY FAUCET on your base's district
  (the Economy engine already models solar as a faucet) — it feeds your battery, holds your
  lights, and GROWS YOUR LIT TERRITORY, which in this game means your safe ground grows
  (LIGHT=TERRITORY). Visible, felt reward, not a number going up.
- NOT A FETCH (CDPR law): panels are valuable, owned, contested — the quest needs a real
  situation and a fork (whose panel, taken how, who else needed it), never "go grab a panel."
- [PENDING Paolo: where the panel comes from + the fork; exact timing/slot.]
