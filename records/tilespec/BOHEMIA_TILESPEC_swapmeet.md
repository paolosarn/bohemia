...
| 2 | `#6a5f50` | building (food hall / office) | building | the market hall / food court + office, roll doors seized, signage dead | structure | yes | market-hall interior: permanent indoor stalls + a food-court counter up front, storage behind | 238 |
| 3 | `#4a4030` | junk / debris pile | tree-dead | a knocked-over stall / a pile of picked-over junk + torn tarp | prop | no | — | 32 |
| 4 | `#b0a690` | stall canopy / tent | overhead | a vendor canopy / tent over a stall row — fabric bleached + torn, poles leaning (you walk UNDER it) | overhead | no | — | 960 |
| 5 | `#c79a3f` | gate / entrance | gate | the market entrance off the street, amber curb, turnstile husks | portal | no | — | 9 |
| 6 | `#7a5a44` | vendor stall / table | prop | a vendor table / stall stand, goods long gone, some tipped | prop | yes | — | 320 |
| 7 | `#6f6656` | market aisle (packed-dirt walkway) | ground | the packed-dirt market aisle between the stall rows — the walkable grid | ground | no | — | 6653 |
...
| 12 | `#605844` | map kiosk / dead planter | prop | the you-are-here map kiosk / a dead planter at the market mouth | prop | yes | — | 45 |
...
**Gate:** `gates/swapmeet_gate.js` (+ the street-aware/drivable law via `gates/district_kit_gate.js`), the walkable-land law via `gates/walkable_gate.js`, and this dossier via `gates/tilespec_gate.js`.
