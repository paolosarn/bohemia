...
| animated doors (2 tiles tall) | INTEGRATED | door_anim | the approved 7/13 door-anim bank: 1 wide x 2 tall, 9-frame swing over 2 beats, a shut door really blocks. 8/23: THIS WAS NEVER A HOLE, it was the ledger asking the old address. All 90 frames of all 10 approved residential swing clips are in BOHEMIA_CITY_TILES.js verbatim (measured, 0 missing) -- the art moved house on 8/6 with the other 27 MB -- and `inEnter`, the one place a body goes through a door in the city, calls doorSwing; doorSwingDraw blits `C, C*2`. The probe now asks the caller and the blit, not a bank reference |
| floorplan interiors | INTEGRATED | floorplan_module | real generated rooms, interior === exterior footprint |
| interiors dressed (CITY's UP-only pool) | INTEGRATED | interior_pool | **REFUSED ON THE CITY, ON PURPOSE -- DO NOT 'CLOSE' THIS ROW.** (The status stays INTEGRATED because that is what the probe proves about the run slice, and this scoreboard's vocabulary is closed on purpose -- integration_gate rejected a compound status when I tried one, which is the gate outranking my convenience and it was right.) one floor per ROOM picked by the room's own function (dirt in a garage, tile in a wet room), props from the role's own buckets, walls from the constitution's own tile. Props are decoration, never collision. 8/24: The CITY lane opened the bank, rendered it to PNG and LOOKED, and wrote the refusal into the city above the furnish pass: the pool is banded oak barrels, burlap sacks, tavern benches, potion bottles and LIVE FLOWERING PLANTS -- a generic fantasy asset pack. Nobody in a 2020s Las Vegas apartment owns a coopered barrel, and live greenery breaks the dead-world standing law outright. The DEMO_PROP_POOL's containers are glowing sci-fi loot crates. So nothing is wired from either, deliberately: the city defines the TYPES and their footprints and the pixels are ART's. City interiors ARE furnished today -- typed COVER/LOW massing, edge-faced, 45-degree-law correct -- they are simply not furnished from this bank. Quoting the refusal verbatim because a ledger that says OWED will send somebody to re-do a rejected thing: *"Putting a fantasy barrel in the one surface he plays, because a bank existed and a law said reuse first, is how a build ends up looking like somebody else's game."* |
...
| walk feel, playable | INTEGRATED | walk_feel | 8/23, ON THE SURFACE HE PLAYS: GRID / SLIDE, switchable mid-walk from a chip in the builder's drawer, SLIDE the default. Measured in the canvas he looks at -- GRID moves 0.0% of a typical frame and then 61% of it in one, which IS the 56px teleport; SLIDE moves 58.8%. It cost no new loop: animate() already rAFed for one beat after every step. HYBRID and FREE are NOT carried and are owed: continuous movement needs a second position space with its own collision, doors, and answer for a day metered per CELL ENTERED. The RUN SLICE still has all four, on the panel nobody opens |
