...
# BATCH 5 -- THE TWO SHAPES THE CARD ASKS FOR BY NAME

## FIRST, A CORRECTION TO MY OWN CONCLUSION

Last round I wrote that this job could not be finished without a coordinator ruling,
because "every garment to the card" would mean repainting approved art. **THE CARD
ALREADY ANSWERS THAT and I had not read it closely enough:**
...
The mechanism is replacement by new cooks, not migration of old ones. No ruling was
needed and none was ever needed. **A blocker that does not exist is worse than a real
one** -- it stops work for nothing, and it did for a round.

## WHAT THE CARD ASKS FOR THAT THE WARDROBE DID NOT HAVE

| id | the card's words | what existed |
|---|---|---|
| RNWY-02 | pole A is "EITHER cut square OR **a full cocoon arc, one curve neck to elbow with NO shoulder point**" | only the square one |
| RNWY-05/08 | "at least 2 visible hem lines stacked, lower layer longer" | a composed FIT stacks; **no single garment carried its own second hem** |

Five garments on those two shapes. Wardrobe **256 -> 301 garments, 79 -> 106 shapes**;
the card's headline number is **43%**.

## THE ARC TOOK THREE ATTEMPTS AND THE GATE FAILED ALL THREE

1. **A swell added to each row's own edge. Measured: 4 px step.** A swell cannot
   remove a corner, because **the corner is in the body**: the torso is 10 cells
   across and the arms make it 16, so the outline jumps three cells each side on the
   row the arms begin. Fabric added outboard of that jump keeps the jump.
2. **Rebuilt as an absolute target edge per row, smoothed. Measured: 2 px step.** The
   backward smoothing pass had the forward pass's comparison, so it never widened the
   rows *above* the shoulder -- it only forbade steps, and forbidding a step does not
   make an outline widen. The jump moved down one row and survived.
3. **The ramp above the shoulder written explicitly. Measured: 1 px. Passes.** Each
   row above the arms sits exactly one cell inside the row below, so the outline has
   arrived at the arm's width by the time the arms do. The neck row is deliberately
   left alone: a bar of cloth up there is a collar shelf, not a shoulder.

And the layered hem failed twice for its own reasons: its two hems landed **one row
apart** with the only visible step being the arms ending (not a hem at all), and once
that was fixed its outline was **within 14% of the LONGLINE's** -- the dead V-NECK's
failure occurring between two of my own shapes. A layered top is a SHORT outer over a
LONG under; a longline is one long hem. Separating those two numbers is what makes
them two garments instead of two names.

## THE GATE NOW HOLDS BOTH, EACH WITH A CONTROL THAT MUST FAIL IT

```
    RNWY-02  the ARC's biggest edge step: 1 px          (an arc allows 1)
    CONTROL  a PLAIN top's biggest edge step: 3 px      -- a ruler finding none is
                                                          not looking at the shoulder
    RNWY-05  the LAYERED top's silhouette steps: 3      (a stack needs 3)
    CONTROL  a PLAIN top's steps: fewer than 3          -- no stack, correctly
```

That makes **five live controls in this gate** -- the dead V-NECK for shape, his
approved DUST MASK for the face rule, a PLAIN COAT for the hem rule, and now a PLAIN
TOP twice. Every one of them must FAIL the rule it guards, because a check nothing
