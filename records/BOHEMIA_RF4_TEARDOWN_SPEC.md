...
| **RF4-18** | **★ WALLS ARE MECHANICS, NOT SCENERY.** Infusion-of-Storms grants **+1 Power for ending a turn "wide open," meaning not adjacent to any walls** — and *"depending on balance, pillar type objects may also be included in this definition."* Abilities **read the room.** **[PRIMARY, refined by SOURCED]** | **ABSENT as a rule.** Cover and LOS are read, but nothing keys off wall adjacency or open-ness. **This is the item that justifies his indoor instinct** — it is the mechanic that makes a room mean something. **BUILT 8/25 as V180**, and it is the SECOND attempt: the 8/21 version paid +1 killshot on the chain and V62's per-weapon cap swallowed it on three guns of four, so it was CUT with the blocker written down — *a reward currency that is NOT weapon-capped*. V176 shipped one. It now feeds THE FINISHER CHARGE, which fills identically whatever gun you hold. Not merely outdoors: wide open ALONE is 55% of turns and 1.33 free finishers a fight, so it requires open ground **under their eyes** — 34% of turn ends, 56% of which have a gun that can reach you against 17% everywhere else. The condition is exactly what V179's rings already draw, so the information and the reward are the same thing. | BUILT |
...
| **RF4-28** | **★ ENEMIES ARE DESIGNED AS COUNTERS TO EFFECTIVE PLAYER ACTIONS**, deliberately, *"to force tactical adaptation and increase the overall tactical scope of gameplay."* **[SOURCED]** | **ABSENT as a design rule.** Nothing in the roster is built to punish a specific player habit. Our cover system is strong enough that a cover-ignoring or cover-destroying enemy would be a real counter. | BUILT |
| **RF4-29** | **No fights won before they begin.** You should not delete an unaware group with one opener; fights run *a bit longer* so advanced tactics can play out, while staying snappy. **[PRIMARY]** | **MEASURED 8/25 AND IT HOLDS.** Driving the REAL fire buttons with the dial hit perfectly every time -- possible only since V178, because until then nothing in this repo had ever fired the gun. Eight arenas averaging 4.3 men: 7 openers LANDED and took 427hp off the room, 2 bodies dropped, and **0 fights ended on the opener**. Full fights: the perfect player cleared 7 of 10 in a **median of 20 turns**; a random-timing player cleared 2 of 10 and **died in 8**. Across all 20 fights, **none was over inside two turns**. **AND THE 6/30 INVERSION IS NOT IN THE GAME:** that doc claims a perfect chain clearing a room in one turn as the master reward, and with a perfect dial it never happened once -- so it needs tactics beyond the trigger, or it was aspiration written down as behaviour. Limit stated: this player is perfect at the DIAL and uses no suppression, grenades, cover or target priority, so 20 turns is the trigger ceiling, not the tactical one. | BUILT |
