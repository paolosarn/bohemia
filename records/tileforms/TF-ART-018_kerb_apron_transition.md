...
## J. ADMIN
- STATUS: COOKED 8/16/26 AND WIRED 8/16/26 (M1 returns, second pass: the returns draw at every suburb street corner - sidewalk cell with two adjacent orthogonal roads - verified live at a real intersection; card in the ART tab). M5 ramp branch REMOVED after measurement: no named sidewalk cell ever borders a named crosswalk cell; tiles stay banked. Original cook note: (tools/tfcook/TF-ART-018_cook.py -> banks/tileforms/TF-ART-018_CANDIDATES_8_16_26.json, 12 pieces: M1 returns x4 at the bar, M5 ramps x4 at the bar, M2 drops x4 cooked but HELD - first render over-ringed, needs one more design pass; M4 parked, no world cell names a drain). Canvases harvested from the harmonized street pool per the 7/31 law; cap/face authored to the frozen starter kerb's measured 176/59. THE PLACEMENT MEASUREMENT (8/16, two live probes): plain 'sidewalk' cells exist ONLY in the suburb family - a 16x16-cell non-suburb sweep found ZERO - and the suburb ground stack is the dressed reference district, so wiring there is a deliberate next-pass job, not a drive-by hook. An inert hook was pulled rather than shipped. M2 WIRED 8/17/26 (redesigned as an RGBA overlay lip after the first pass over-ringed: low cap across the mouth, 6px flares to full kerb at the ends; fires on any driveway cell with exactly one road neighbour - the real curb-cut definition; verified live at a suburb driveway mouth). ALSO MEASURED: the 80 world-named gate/curb-cut cells all sit ON cell borders (x or y = 0, the fence line) - noted for a future pass to move the hash-placed fence gates onto those REAL entrances. | REQUESTED BY: ART lane (own queue, breaking down board row 15
