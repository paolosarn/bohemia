...
## J. ADMIN
- STATUS: APPROVED 8/11/26 AND WIRED 8/11/26: downtown's 'storefront (green)' structure cells draw the family (glazed bays with boarded/shutter/grille segments, one treatment per 2-cell segment); the earlier 'never caught on screen' was the PROBE standing at awning clusters, not a layer problem, and the awning overhead cells are explicitly refused by the structure hook. Awnings on the canopy layer, signbands, pilasters, ends and the smashed pair are the named volume. VOLUME 8/15/26: SIGNBANDS wired - cooked fresh (tools/tfcook/TF-ART-008_signband_cook.py, palette harvested from the family banks, REUSE+TASTE checks in the docstring) into banks/tileforms/TF-ART-008_SIGNBAND_VOLUME_8_15_26.json under the family approval (the form names signbands as volume). Placement: the TOP row of a storefront glass run wears the fascia band translated to the cell top 1:1 (measured: downtown glass is 2+ rows and touches roof, so no wall row exists above it - the first wall-above hook could never fire and was removed as dead code). Verified live downtown; card reshot. SECOND COOK 8/15/26: smash + pilasters (tools/tfcook/TF-ART-008_smash_pilaster_cook.py - the smash is cooked ON the family bay art so a broken bay matches the intact one beside it; piers at every 2-cell boundary, full height, band runs between). Glass segments only smash, bottom row only (looters enter at street level), 1 column in 13. Verified live downtown; card reshot. THE FAMILY IS COMPLETE: every volume item the form named (signbands, pilasters, ends, smashed pair) is wired. AWNINGS WIRED 8/17/26: the overhead-layer awning cells (154+ per downtown block, named all along) draw the approved two-cell tattered drops on SOUTH-FACING fronts (facade north of the cell), one colourway per 4-cell run matching the facade branch, sidewalk under, pass-under fade already live. MEASURED at the first render: the art is authored for south-facing fronts - on a vertical run it stamps sideways into a dark ribbon - so side-facing runs are excluded and side-facing drops are named volume. SIDE AWNINGS WIRED 8/18/26 (tools/tfcook/TF-ART-008_sideawning_cook.py - continuous N-S ribbons, fabric colours sampled from the family drops, sag scallop per 11px arm bay so runs tile seamlessly, torn gaps show the ground; east and west facades both). Every named awning cell in downtown now draws, all four facings. | REQUESTED BY: ART lane (own queue, breaking down board row 7)
