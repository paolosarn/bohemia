...
## J. ADMIN
- STATUS: APPROVED 8/11/26 AND WIRED 8/11/26: every 'dead turf' surface draws the mow-stripe field (park lawns, school field, stadium turf), courts/tracks/infields/putting/bunkers key on their own names, live in the RUN tab. The line WANG sets, track lanes and banking are the named volume. VOLUME 8/15/26: the painted lines wired - court_line_<NESW> WANG by the rim-mask pattern (fourth reuse) on court boundary cells, track_lane NS/EW + 4 corners where a track cell has exactly two track neighbours (wide pads stay plain field, honest). Verified live at a park court; card reshot. | REQUESTED BY: ART lane (own queue, breaking down board row 7)
