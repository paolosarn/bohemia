# BOHEMIA — THE BOSS LADDER: 18 CANDIDATES (v1 RESTORED, 8/4/26)

> "that kinda sucks too i liked your first version the best cant we build off that"
> — Paolo, 8/4/26

**VERSION 1 IS BACK, VERBATIM. The thirteen below are byte-for-byte the ones he liked**,
recovered out of git (commit `7da7c89`) rather than rewritten from memory. Five more are
added at the end in the same voice. Nothing is reframed.

## WHAT WENT WRONG TWICE, IN ONE SENTENCE

**I KEPT REPLACING THE PERSON WITH A THING.**

v1's bosses were PEOPLE — the Tap, the Ward, the Voice, the Judge — somebody who *owns* a
piece of the city, and beating them lets you do that thing anywhere. Then he said "concrete
boss" and I renamed them all after **substances**. Then he said "tool vibes" and I renamed
them all after **objects**. Both times I heard *rename* and stripped the character out, and
both times what I deleted was the only thing that made them interesting: **a boss is a
someone.**

The reward was never the problem. **THE PERSON WAS THE POINT.** So: nothing gets renamed
again. `laws/BOHEMIA_ADDENDUM_STOP_PRODUCING_7_26_26.md` says a second rejection ends the
feature for the session, so this is a RESTORE plus five additions and then I stop
proposing shapes for the ladder.

WHAT I AM CARRYING FORWARD FROM THE DEAD PASSES, because it was mechanism and he did not
reject it:
- **The act tiers**, fitted to `laws/BOHEMIA_ADDENDUM_THE_VALHEIM_SHAPE_8_4_26.md` §3
  (~15% / ~33-40% / 80-100%, anti-grind clauses ranked equal to the numbers, caps are
  CEILINGS never floors under §4b). **ACT 1 you do it · ACT 2 your crews do it · ACT 3 the
  city does it around you** — because the grind is *the last unit of progress costing the
  same as the first while rewarding less*, so the unit of work has to grow each act.
- **§4b: act 3's currency is ALLIANCE, not stuff.** The pseudo-mayor and the united front.
- **A boss grants the ABILITY, never the RESOURCE.** My own rule from the first revision,
  and the thing I then broke. It stands.

---

## ALL EIGHTEEN, NUMERICALLY (added 8/4 on his ask)

He asked for one cohesive numbered list, so here it is in one place, **re-sorted into act
order for reading.** Every entry: **who they are · what they hold · what beating them lets
you do anywhere.**

| # | THE BOSS | HOLDS | YOU CAN THEN | ACT |
|---|---|---|---|---|
| 1 | **THE TAP** | a live water main | draw water anywhere there is pipe | 1 |
| 2 | **THE BURN** | fuel, and who gets warm | make fire anywhere, heat a building | 1 |
| 3 | **THE TOLL** | a chokepoint | permanently clear a blocked road | 1 |
| 4 | **THE STRIPPER** | what may be taken apart | strip any building for materials | 1 |
| 5 | **THE WRECKER** | demolition | take a building down | 1 |
| 6 | **THE LIGHTS** | a lit block | switch on any street in the valley | 2 |
| 7 | **THE WARD** | the last working clinic | treat wounds anywhere | 2 |
| 8 | **THE VOICE** | the one transmitter | broadcast to the whole valley | 2 |
| 9 | **THE DRAIN** | where the waste goes | make a district habitable | 2 |
| 10 | **THE BOOK** | who owes whom | call in a debt anywhere | 2 |
| 11 | **THE FOREMAN** | the working hands | put a crew to work without you | 2 |
| 12 | **THE OPERATOR** | the heavy machines | run the heavy stuff (costs fuel forever) | 2 |
| 13 | **THE FIXER** | where the one working part is | repair instead of replace | 2 |
| 14 | **THE JUDGE** | what counts as a crime | pardon or condemn anyone | 3 |
| 15 | **THE SCHOOL** | who gets taught | teach a skill to somebody else | 3 |
| 16 | **THE SOIL** | the last living ground | make things grow again | 3 |
| 17 | **THE GRID** | the whole network | move power between districts | 3 |
| 18 | **THE BROKER** | the only line all factions still use | get two enemies into one room | 3 |

**FIVE IN ACT 1, EIGHT IN ACT 2, FIVE IN ACT 3.** Act 2 carries the most because it has the
biggest rebuild delta to cover (~15% up to ~33-40% under
`laws/BOHEMIA_ADDENDUM_THE_VALHEIM_SHAPE_8_4_26.md` §3) and it is the act where work starts
happening without you standing there.

**THIS TABLE RE-SORTS; THE SECTIONS BELOW DO NOT.** The numbering further down is v1's own,
untouched, because those thirteen are the ones he approved and they do not get renumbered to
suit a table. If the two ever disagree, **the sections below are the record.**

## FIRST: WHAT A "BIOME" MEANS HERE, BECAUSE IT IS NOT GEOGRAPHY

In Valheim a biome is **a climate you cannot survive yet**. Frost kills you until you have
the cape.

Bohemia has one valley and no climates. So:

**★ A BOHEMIA "BIOME" IS A MISSING FUNCTION, NOT A PLACE.**

A block you cannot hold because it is **dark**. A block you cannot work because it is
**sick**. Ground you cannot farm because it is **dead**. The lock is not the weather, it is
**which utility is absent there** — and that is why we do not need seven climates drawn on
a map, and why **the same street is a different biome in act 1 than in act 3.**

Three things fall out of it for free:

- **It obeys MAP LAW.** No lane draws biome zones. The tier is a property of what works
  there, not of where it is.
- **It makes the city-builder and the RPG the same system.** Restoring the function *is*
  building the city.
- **It explains the whole map.** 9,216 cells are not a checklist; they are everywhere a
  function is currently missing.

## SECOND: THE HOLE, AND THE RULE THAT PLUGS IT

His own example contains the trap: *"turn on any street in the game."* If a power works
everywhere for free, boss five ends the game.

Valheim never has this problem because **the biome grants the ABILITY and the ore grants
the AMOUNT** — knowing how to forge iron does not give you iron.

**THE RULE: A BOSS GRANTS THE ABILITY, NEVER THE RESOURCE.** You learn to light any
street. You still have to *pay the electricity*. Electricity is already one of the three
currencies, so this costs us no new system, keeps every power a real decision forever, and
means the ladder can be long without deflating.

**And the Destroyers can never be bosses.**
`laws/BOHEMIA_ADDENDUM_ACT1_PROCEDURAL_ENDING_AND_DESTROYERS_7_19_26.md` LOCKED them as a
FORCE, closer to weather, who own nothing and rebuild nothing. A boss **owns a function**.
They are exact opposites, which is a good sign the shape is right: **Destroyers are the
weather, bosses are the owners.**

---

## THE LADDER — THE ORIGINAL 13, UNCHANGED

Ordered by **how fundamental the need is**, which is also roughly the order civilisation
actually rebuilds in — his standard from R28, *what realistically happens.*

Each one: **the function locked · who holds it · the power you gain.**

### ACT 1 — ANIMAL. Bosses of SURVIVAL. Personal scale, one person's grip on one thing.

**1. THE TAP — water.**
Somebody is sitting on a live main and selling from it. **POWER: you can draw water
anywhere there is pipe.** The weakest boss in the game and the most human one — the first
thing a person needs, held by one guy with a wrench.

**2. THE BURN — heat and fuel.**
Controls what can be burned and who gets warm. **POWER: you can make fire anywhere, and
heat a building.** Cold stops being a threat and becomes a resource you hand out.

**3. THE TOLL — passage.**
Holds a chokepoint and charges for it. **POWER: you can permanently clear a blocked road.**
This one plugs straight into R9 — fast travel unlocks by having walked, and this is how the
walk becomes possible in the first place.

**4. THE STRIPPER — salvage.**
Decides what may be taken apart, and hurts people who take without asking. **POWER: you can
strip any building for materials.** The resources currency stops trickling and starts
flowing.

### ACT 2 — HUMAN. Bosses of CIVIC LIFE. Institutional scale, somebody running a service.

**5. ★ THE LIGHTS — power. HIS IDEA, and it belongs exactly here.**
Owns a lit block, and the lit 12% is owned by law already (CLUSTERED POWER, LIGHT =
TERRITORY). **POWER: you can switch on any street in the valley.** Act 2 is right for it,
not act 1: light is the first *civic* thing rather than a survival thing, and it is the
moment the game turns from surviving into governing.

**6. THE WARD — medicine.**
Holds the last working clinic and the drugs in it. **POWER: you can treat wounds anywhere.**
This one reaches into TRAUMATIC NOT GORY: a hurt body is a CLOCK, and this boss gives you
the ability to stop the clock — which makes every earlier fight retroactively survivable and
every later one a choice.

**7. ★ THE VOICE — the airwaves.**
Owns the one transmitter still broadcasting. **POWER: you can broadcast to the whole
valley.** The biggest mechanical unlock on the list, because it hands the player the STORY
SPREAD system as a *tool*: R20/R21 say stories spread from witnesses like a plague, and this
boss lets you **seed one deliberately** instead of waiting for somebody to see you.
Reputation stops being weather and becomes a lever.

**8. THE DRAIN — sanitation.**
Controls where the waste goes, and therefore which districts are sick. **POWER: you can
make a district habitable.** The unglamorous one, and the one that actually decides where
people can live — which is the gate the city-builder layer needs.

**9. THE BOOK — debt.**
Holds the ledger of who owes whom, which after ten years is the only real economy there is.
**POWER: you can call in a debt anywhere.** Clout becomes spendable. And it needs no new
currency and no market — no price moves by itself, so TEN YEARS COLD holds.

### ACT 3 — ANGEL. Bosses of CIVILISATION. The strongest, because each returns something the
### world had given up on.

**10. THE JUDGE — law.**
Decides what counts as a crime and who answers for it. **POWER: you can pardon or condemn
anyone.** The mercy ledger (R17, recorded silently) suddenly has somewhere to go, and every
choice you logged for a hundred hours becomes a thing you can spend.

**11. THE SCHOOL — knowledge.**
Controls who gets taught, and hoards what is left of how things work. **POWER: you can
teach a skill to somebody else.** The dynasty payoff: your heirs and your people improve
without you standing there. The boss that makes the third generation feel different from the
first.

**12. ★★ THE SOIL — life. THE STRONGEST BOSS IN THE GAME, AND IT SHOULD BE.**
Holds the last living ground and the last seed stock. **POWER: things grow again.**

Act 1 is a **DEAD WORLD BY LAW** — no trees, no grass, no pools, ever, gate-enforced
(settled question 9). Which means we spend the entire game teaching the player that nothing
grows here. **So the single most powerful thing in Bohemia is a green shoot**, and the law
that forbids it in act 1 is exactly what makes it land. Nothing else on this list can
compete with that as an ending beat.

**13. THE GRID — the network itself.**
The act-3 crumbling network (`laws/BOHEMIA_ADDENDUM_ACT3_MOONSHOT_STRUCTURE_7_19_26.md`).
Not one lit block — the whole thing. **POWER: you can move power BETWEEN districts, not
just switch it on.** Boss 5 gave you a switch; this gives you the machine. The city stops
being a map you own and becomes a system you run, which is the last thing you need before
the Moon.

---

## WHY THIS ORDER IS THE RIGHT ORDER

**Each power makes the previous one bigger, and none of them replaces an earlier one.**
Water lets you live somewhere; heat lets you live there in winter; roads let you reach it;
salvage lets you build it; light lets you hold it; medicine lets you survive holding it; the
airwaves let you tell people you hold it; sanitation lets others live in it; the ledger lets
you fund it; law lets you govern it; the school lets you outlive it; the soil lets it feed
itself; the grid lets you run all of it at once.

That is not thirteen features. **It is one sentence about rebuilding a city, cut into
thirteen pieces** — which is what he asked for.

**And every one can be taken without killing.** Each boss is a person holding a function, so
each has something they want, somebody they fear, and a reason to hand it over. That is what
makes this Bohemia and not Valheim, and it is what "least possible loss of life as the
desired outcome" needs in order to mean anything mechanically.

---

## THE FIVE ADDED (8/4/26) — same voice, same shape: a person who owns a thing

He asked twice for more, so here are five, written the way v1 was written. **Each is
somebody who holds one piece of the city, and beating them lets you do that thing
anywhere.** Names are handles to argue with, never canon.

### ADDED TO ACT 1

**14. THE WRECKER — demolition.**
Nobody clears anything any more, so whatever he pulled down stayed down and whatever he
left standing is his leverage. **POWER: you can take a building down.** The verb I had
missed completely across three passes, and it might be the best one on the whole list: **you
cannot rebuild Las Vegas without clearing it first**, and flattening a structure is a real
decision the moment the thing you flatten was somebody's home, somebody's shrine, or
somebody's cover.

### ADDED TO ACT 2 — these three are what make ~33-40% possible without a grind

**15. ★ THE FOREMAN — labour.**
The one the working hands answer to, and the reason nothing gets built without his say.
**POWER: a crew works while you are somewhere else.** The most important addition on this
list, because it is the exact moment the unit of work stops being one wall and becomes one
*building* — and the research on late-game tedium says delegation is the only real fix for a
long tail, needed *before* the grind rather than patched in after.

**16. THE OPERATOR — the heavy machines.**
Somebody kept an excavator and a crane running for ten years and nobody else can start
them. **POWER: you can run the heavy stuff**, and it eats fuel forever, so the ability
arrives with a permanent bill attached.

**17. THE FIXER — parts.**
Not salvage and not materials: the person who knows *where the one working part is*. **POWER:
you can repair instead of replace.** The quiet one, and the one that makes the whole valley
worth searching, because it turns every district into somewhere a specific missing thing
might be.

### ADDED TO ACT 3 — the last one, and it is the end of the ladder

**18. ★★ THE BROKER — the united front.**
The only person the factions all still talk to, and the reason none of them has finished the
others off. **POWER: you can get two people who hate each other to stand in the same room.**

This is §4b as a boss: the endgame currency is ALLIANCE, and **you never lay 60% of a city
yourself — you get the sixteen factions to agree and they lay it.** `records/factions/`
already holds all sixteen dossiers. It is the strongest boss in the game to *beat* and the
only one you cannot beat by force, which is exactly what "least possible loss of life as the
desired outcome" has been pointing at the whole time.

## WHERE THE ADDED FIVE SIT AGAINST THE ACT CAPS

| | THE VERB | THE BOSSES THAT MAKE IT POSSIBLE |
|---|---|---|
| **ACT 1** → ~15% | **YOU do it** | 1-4 and 14 — water, heat, passage, salvage, demolition |
| **ACT 2** → ~33-40% | **YOUR CREWS do it** | 5-9 and 15-17 — light, medicine, airwaves, sanitation, debt, labour, machines, parts |
| **ACT 3** → 80-100% | **THE CITY does it around you** | 10-13 and 18 — law, knowledge, life, the grid, the united front |

Every one is optional. **A player who beats none of them still finishes the game** (§4b),
and the caps are ceilings rather than floors.

## WHAT IS HIS ON THIS

- **Which of the 18 live, which die, and the order.** All of it. I ordered by real-world
  rebuild sequence; he may want light earlier, or the soil somewhere else entirely.
- **Whether the count is 18, or 7, or 30.** He said "much more than Valheim potentially";
  the number is his.
- **The names.** Every name above is a PLACEHOLDER — "The Tap", "The Burn", "The Voice" are
  handles to talk with, not canon. Naming is his, and the Destroyers precedent says naming
  is late polish anyway.
- **Who these people actually are**, their factions, their voices, their arcs. Contents his.
- **Whether a boss can be an institution rather than a person**, and whether any of them is
  unkillable.
- Every number, every cost, every dial.

## WHAT THIS DOES NOT DO

Supersedes nothing. Adds no canon. Draws no map — MAP LAW holds. Adds no fourth currency,
and no price moves by itself. Contradicts no live law: checked against the dead-world rule
(which boss 12 depends on), the Destroyers ruling (which boss-hood excludes them from), the
three acts, TRAUMATIC NOT GORY, and the three currencies.

**THREE GUARDRAILS THAT V1 PREDATED, KEPT WHEN IT WAS RESTORED.** The gate caught their
absence, which is the gate doing its job — a restore must not quietly roll back a
protection that was added after the thing being restored:

- **Sets no percentage of its own.** The act caps belong to
  `laws/BOHEMIA_ADDENDUM_THE_VALHEIM_SHAPE_8_4_26.md` §3; this document fits them and never
  adjusts them.
- **Writes no faction canon.** The sixteen dossiers in `records/factions/` are cited as
  existing. Who allies, in what order, on what terms — **I am not writing faction canon**,
  that is his.
- **No numbers, ever: NO DAMAGE BEFORE THE DIAL.** Not one cost, duration or value appears
  anywhere above.

Working: `records/BOHEMIA_THE_PROGRESSION_SPINE_PROPOSAL_8_3_26.md`.
