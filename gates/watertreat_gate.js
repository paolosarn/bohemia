// WATER TREATMENT GATE (7/21/26). A dead wastewater plant — circular clarifiers + rectangular
// aeration basins + a filter-bed grid + a control building + pipe galleries, fenced. Street-aware +
// drivable (thin service road), content dominates (WALKABLE-LAND). Research-first.
const D = require('../engine/bohemia_watertreat.js');
...
for (const cfg of CONFIGS) for (let s = 1; s <= 3; s++) {
  const r = D.generate(s * 11 + 3, { streets: cfg }), t = counts(r), g = r.g, W = g[0].length, H = g.length;
  if (!(t[2] > 1000 && (t[6] || 0) > 300 && t[7] > 1200 && (t[8] || 0) > 800 && t[4] > 3000 && (t[12] || 0) > 300 && (t[10] || 0) > 5)) anatomy = false;
...
ok('control building + circular clarifiers + aeration/filter basins + pipe galleries + basin floor + fence', anatomy);
ok('WALKABLE-LAND: content dominates pavement', contentDom);
ok('every tile named + low void', filled);
ok('DRIVABLE: service road reaches the plant from the curb', drive);
...
ok('watertreat registered + infrastructure', !!K.get('watertreat') && K.category('watertreat') === 'infrastructure');
ok('control building enterable + footprints', D.generate(7, { streets: ['S'] }).footprints.length >= 1 && /interior/i.test(D.legend[2].enter || ''));
ok('clarifier(6) structure, basin(7) ground, pipes(8) solid, road(1) drive', D.legend[6].kind === 'structure' && D.legend[7].kind === 'ground' && K.tileLayer(D.legend[8]).solid === true && D.legend[1].kind === 'drive');
ok('deterministic', JSON.stringify(D.generate(70, { streets: ['S'] }).g) === JSON.stringify(D.generate(70, { streets: ['S'] }).g));
console.log('WATER TREATMENT GATE: ' + pass + ' passed, ' + fail + ' failed  (' + CONFIGS.length + ' configs)');
