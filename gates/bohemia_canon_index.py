...
def main():
    ...
    # DAY-ONE LAYOUT (7/17): canon .md lives in /laws (plus /records for verdict
    # masters); /archive is history and is never indexed. Default scans those.
    root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    dirs = sys.argv[1:] or [os.path.join(root, 'laws'), os.path.join(root, 'records'), root]
    rows = scan(dirs)
    ...
    dest = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'laws', 'BOHEMIA_CANON_INDEX.md')
