...
GATES = [
    ...
    ('DEBT CARRIED',  ['node', 'gates/debt_carried_gate.js'],
     'THE-DEBT-NAMES-ITS-LENDER-AND-OUTLIVES-YOU (board row [debt carried], 9/12). From the 9/5 '
     'heir research: "where there is no state, the lender collects from the family. Every debt in '
     'the ledger names WHO is owed (a faction, a person) and survives the generation fold to the '
     'heir." TWO THINGS WERE WRONG AND BOTH WERE MEASURED FIRST. ONE: "debt" was THREE UNRELATED '
     'SYSTEMS SHARING A WORD and nothing asked all of them -- the favours you took for free '
     '(save.meta.owed, which has real teeth, since bohemia_claim lets an outfit you owe bypass '
     'the weekly ration and charges an extra rung per unpaid favour when you refuse), the nights '
     'you could not pay for somebody\'s ground (OWED_BOOK), and people you did not show up for, '
     'which is a charge and not an account at all. You could see the first only by standing in '
     'front of that one outfit and the second only after you died. bohemia_owing joins them into '
     'ONE BOOK that names every lender, owns neither store (THE DEBT GETS CALLED IN rule 4: two '
     'ledgers, two owners, numbers between them), and carries a COUNT AND NEVER AN AMOUNT, '
     'because what a favour or a night is WORTH is a weight and weights are his. TWO: the fold '
     'ruled the bill DIES and NOTHING EVER EXECUTED IT -- twelve favour debts across four outfits '
     'went into a fold on the walked surface and TWELVE CAME OUT, so the heir inherited the '
     'parent\'s bill and kept paying for it on every refusal. Now the bill dies at the fold, '
     'cleared through bohemia_favour\'s own writer, and THE PEOPLE HE OWED ARE KEPT BY NAME, in '
     'the fold\'s own words: "you do not inherit a bill, you inherit less and you inherit the '
     'people he owed, still standing there." THE RULING IS READ, NEVER COPIED: billDies() asks '
     'bohemia_fold\'s table, so whichever way Paolo settles the contradiction FACTIONS flagged, '
     'one row moves and the game moves with it -- proved by flipping that row under the module '
     'and watching it stop clearing. Red six ways: break the merge -> 2; hard-code the ruling -> '
     '2; stop clearing at the fold -> 2 (and the message reads 12 -> 12, the original bug); take '
     'the heading off the card -> 3; reach into the store it does not own -> 1; price a debt in '
     'a sentence -> 1.', True),
    ('WATER LIFTED',  ['node', 'gates/water_lifted_gate.js'],
     'THE-PUMPS-ARE-THE-CITY (board row [water lifted], 9/13). Harvested from ECONOMY '
     'round 7: "the valley floor is about 2,028 feet and Lake Mead hit 1,040.5, so '
     'everything Vegas drinks is lifted a thousand feet... WATER IS NOT SCARCE; PUMPING '
     'IS. Wire it: a pump station costs power, water costs pumping, thirst costs water, '
     'and whoever holds the pumps holds the valley." MEASURED FIRST AND IT WAS WORSE '
     'THAN THE ROW SAYS: the valley drinks 4 L a head a day and NOTHING IN THIS GAME HAD '
     'EVER PRODUCED A SINGLE LITRE -- advanceDay\'s `produced` comes only from YIELD, '
     'which is salvage and food, so water was the ONLY good with a need and no way to '
     'make it, a one-way countdown with no source. THE ENERGY IS PHYSICS, NOT A DIAL: '
     'lift 987.5 ft = 301.0 m, one litre is one kilogram by the definition of the litre, '
     'E = mgh = 2,952 J = 0.00082 kWh, over 0.75 wire-to-water efficiency = 0.0011 kWh '
     'per litre. Every number is the row\'s own real measurement or arithmetic on it, '
     'which is why none of it is his to rule, and the gate REFUSES any number in the '
     'module that is not physics, the row\'s measurement, or one NAMED display '
     'precision. WHAT A STATION DELIVERS IS NOT A DIAL EITHER: a municipal pump station '
     'is sized to the population it serves, so a running one covers the need and a dark '
     'one covers nothing -- binary and legible, no invented litres-per-day rating. '
     '*** AND THE FIRST CUT WAS DEAD CODE, WHICH ONLY MEASURING CAUGHT: *** it asked '
     'POWER.at on the plant\'s own cell and got id -1 on every water district in the '
     'valley, STRUCTURALLY, because circuits are contiguous street runs and a plant is '
     'not a street -- 358 live cells in the valley and every one is arterial, freeway or '
     'downtown. "A pump on a live circuit runs" could never have been true and would '
     'have shipped correct and doing nothing. Measured again: ALL SIX water districts '
     'touch a circuit at distance 1, so a plant is served by the STREET IT FRONTS, which '
     'is what a service connection is and is the same thing [lights bill] means by "a '
     'plot fronts a street feeder and that feeder is his". On the real map that leaves '
     'the pump station and both treatment plants running, the three reservoirs dark, and '
     'the ANARCHISTS holding the valley\'s water. THE POWER IS NOT CHARGED TWICE: the '
     'first cut subtracted the lift\'s kWh from the ledger\'s `power` stock and drove it '
     'to -5.249 in ten days, because that stock starts at zero and nothing ever fills '
     'it -- a stock that only goes down past zero is the hidden debt bohemia_purse '
     'refuses by design. A pump runs on a live circuit and a live circuit already costs '
     'its holder one battery a night. NOT BUILT ON PURPOSE and named in placeholders: '
     'the PLAYER\'s own thirst, which would be a FIFTH VERB and the four are frozen. '
    ...
    ('BB PLACES', ['node', 'gates/bb_places_gate.js'],
     'ROW [bb places], rule 33 school (Paolo 9/24, THE OVERWORLD IS BATTLE BROTHERS, "places '
     'are destinations"). BB researched not remembered: every settlement has ATTACHED '
     'LOCATIONS that "influence heavily the available goods, services and recruitment '
     'options"; the hinterland makes an item cheap, price rises 3% per attached location, and '
     'some attract recruits of particular backgrounds. THE GOOD NEWS: WE ALREADY BUILT THE '
     'ATTACHED LOCATIONS -- bohemia_towns.minesOf flood-fills solar/dam/battery runs and '
     'attributes each to a holder, and a SITE is a building not a cell (253 solar cells come '
     'back as one array, Hoover as one dam of four). Seed 1337: 4 sites, 308 cells, three '
     'factions. *** AND THE FINDING THAT PROVES US WRONG: FOURTEEN PLACES, THREE SHELVES. *** '
     'goodsFor(tier,goods) returns all.slice(0,n), so a shelf is a function of TIER ALONE: '
     'every camp sells the same four things, every town the same eight, every fortress the '
     'same eleven. A place differs from the tier above it and is IDENTICAL to its own tier. '
     'AND THE INVERSION: HOMELESS is rank 13 of 14 on his own power ladder, tier CAMP, shelf '
     'of FOUR -- and holds 254 of the valley\'s 308 generating cells, making more a day than '
     'anybody -- while three of the five fortresses, with the deepest shelves, hold no '
     'hinterland at all. The shelf never asks: minesOf has 3 callers in the walked city and '
     'none is the shop. *** AND BB\'S PRICE MECHANISM IS HELD OUT, because this lane learned '
     'it the expensive way: *** the 9/15 stranger surcharge made the first bag of rice cost '
     'two days\' work and his ruling came back "the surcharge is DEAD, never a number above '
     'one, the spread lives in ACCESS". So the hinterland must change WHAT IS STOCKED, never '
     'what it costs. THIS GATE ALSO CAUGHT ITS OWN AUTHOR: the school page, the tool head and '
     'the vote row all said "rank 14 of 14" and Homeless is 13; corrected before he read it. '
     'Red three ways: give every camp an extra good -> 1; let a second price appear -> 1; stop '
     'attributing the hinterland -> 3. School, nothing on a play surface. '
     'records/BOHEMIA_WORLD_BB_PLACES_THE_HINTERLAND_9_25_26.md', True),
    ('ACT STAMP', ['node', 'gates/act_stamp_gate.js'],
     ...
     '(A) THE ACT IS ON THE ENTRY, NOT ON A SUMMARY -- same reason the balance is a sum and '
     'not a field -- stamped in _post, the one private writer, and read off the purse AT THE '
     'TIME OF THE MOVEMENT so a later re-ruling cannot change hands in the past. balanceIn / '
     'through / acts are the century module\'s own three shapes, taken rather than '
     'reinvented. Going backwards is refused. (B) AN OLDER SAVE IS A PLAYABLE SAVE: a blob '
     'written before 9/24 has no act anywhere and every entry reads as ACT 1, which is not a '
     'fudge but rule 32(b) -- the game starts in the ruin, act 1 is the floor, so money that '
     'predates the count was act-1 money. (C) THE TREASURIES SURVIVE A RELOAD AND CANNOT '
     'MINT: the 9/14 reason for leaving the save off ("treasuries outliving a memory-only '
     'player purse would mint across a reload") is now the RULE rather than the excuse -- '
     'load() compares the blob\'s recorded supply against what the restored book is worth '
     'and refuses by name, leaving the book untouched. Measured: the hole left 0 of 10 and 0 '
     'holders on every reload. (D) *** AND MEASURING IT FOUND A BIGGER MINT THAN THE ONE I '
     'NAMED: *** STOCKED guards the opening stock (one battery per head, his 9/16 ruling) and '
     'its own comment says "an opening stock that can be re-run is a mint with a polite '
     'name". Save the treasuries without it and every reload runs it again: the valley opens '
     'with 80 and a reload leaves 160. THE WHOLE VALLEY, TWICE. STOCKED rides the save. '
     '(E) the player is deliberately NOT in this book\'s save, because the walked city saves '
     'his purse in its own slot and two copies of one balance is what adopt() prevents. '
     'Rule 22/29: the cook is DRAWN, slices/vote/WORLD_THE_CORNER_RECLAIMED.png, a REDO '
     'naming the id it replaces and quoting the words that killed it; and his sand ruling is '
     'checked on the approved ring road tile. Red five ways: drop the supply check -> 3; stop '
     'STOCKED riding the save -> 2; load an unstamped entry as act 0 -> 1; let the act run '
     'backwards -> 1; put the player in the book\'s save -> 2. Engine, not held (the row says '
     'so); both derived slices resynced in the same commit and the resync touched exactly '
     'these two modules with 136 already fresh. '
     'records/BOHEMIA_WORLD_THE_MONEY_KNOWS_WHICH_ACT_9_24_26.md', True),
    ('LIGHT CLUSTERS', ['node', 'gates/light_clusters_gate.js'],
     'THE CLUSTERED POWER LAW (Paolo 7/14, LOCKED, "I like the answers") says in its own '
     'words that street lighting fails BY CIRCUIT, so "outages/survivals are CLUSTERS, '
     'never alternating", 10-15% lit, all in clusters, every lit cluster OWNED. *** THE '
     'GRID ROLLS ONE INDEPENDENT COIN PER FEEDER, WHICH IS ALTERNATING. *** '
     '`const live = r() < litFraction`. Measured BEHAVIOURALLY across three seeds, because '
     'a source scan cannot tell a coin flip from a cluster: 432 lit cells in 178 SEPARATE '
     'BLOBS biggest 12 (1337), 391 in 158 biggest 13 (7), 422 in 154 biggest 14 (42). THE '
     'BIGGEST LIT THING IN LAS VEGAS IS TWELVE CELLS. Share-of-neighbours-lit falls from '
     '35% at range 1 to 15% at range 6 and 15% IS the global fraction, so past one feeder '
     'the light is statistically indistinguishable from scatter; the only clustering it '
     'has is the feeder\'s own six cells, a code artifact of slicing runs into sixes. '
     '*** AND IT BLOCKS RULE 31. *** DYNASTY school round one closed the do-nothing-future '
     'hole with Detroit and PLANNED SHRINKAGE -- "the power pulled back to the corridors '
     'that still pay, a small live core" -- and stated CLUSTERED POWER already decides '
     'which corridors are lit. The law does; the code does not. THERE IS NO CORE TO PULL '
     'BACK TO: thinning a uniform scatter leaves a thinner uniform scatter. The gate also '
     'BUILDS his law to prove the fix is small and the claim is true (light spreads from a '
     'source along touching feeders to the same fraction; the source count DERIVED, one '
     'per faction that holds ground, which is the law\'s own "every lit cluster is OWNED"): '
     'SAME LIGHT, 11 blobs against 178, biggest 104 against 12. Rule 29: the cook is DRAWN, '
     'slices/vote/WORLD_THE_VALLEY_AT_NIGHT.png, the whole valley after dark twice with the '
     'same lamps lit. MAP LAW is not in the way: nothing designs a layout, the map and the '
     'feeders are the generator\'s and untouched. Red three ways: cluster the grid -> 8 '
     '(and the message tells you to re-aim, not just that it is red); move the lit fraction '
     'out of his 10-15 band -> 11; make the right panel merely brighter -> 1. Nothing on a '
     'play surface (rule 18); the change is one place, the live roll in '
     'engine/bohemia_powergrid.js. '
     'records/BOHEMIA_WORLD_THE_LIGHT_IS_NOT_IN_CLUSTERS_9_24_26.md', True),
    ('FUTURE LEDGERS', ['node', 'gates/future_ledgers_gate.js'],
     'RULE 31, THE THREE ACTS AT ONCE (Paolo 9/23): act 2 and act 3 are DERIVED from the '
     'earlier acts\' ledgers, never hand-placed. The board row [future city] says claim '
     'nothing until DYNASTY\'s school lands, so this lane did NOT build the derive -- it '
     'measured the INPUTS, which is the half nobody else can. *** THE LAW NAMES FOUR '
     'LEDGERS AND EXACTLY ONE OF THEM CAN SAY WHICH ACT IT MEANS. *** WHAT WAS BUILT '
     '(bohemia_century) is a real per-act ledger, written, saved and loaded by the walked '
     'city, totals derived from entries so they cannot drift: READY. BATTERIES: the '
     'player\'s purse saves {id,day,entries} with NO ACT, and the 14 faction treasuries '
     '(bohemia_pockets) export no save and no load at all, so who holds the valley\'s money '
     'is gone on every reload. TERRITORY: turfGrid() is keyed on seed + map size and '
     'NOTHING else -- a pure function of the seed, recomputed identically every boot, and '
     'the city\'s save never mentions turf, so what the player took is written down '
     'nowhere. WHO LIVED: saves, no act. Feed a derive three snapshots and his sentence '
     'stops being true -- the future cannot be built like shit because of act 1 if nothing '
     'recorded what act 1 did, which is the century module\'s own warning in its head. Also '
     'pins TIERS EMPTY (what poor and rebuilt MEAN is his) and the loader treating a broken '
     'blob as an empty memory. Rule 29: the cook is DRAWN, slices/vote/WORLD_THE_SAME_CORNER.png, '
     'one corner twice by ONE function a century apart, and the gate holds the picture\'s own '
     'claims as numbers (the ruin survives 43% pixel-for-pixel so it is the same corner and '
     'not a second drawing; the block wall keeps 81% against the asphalt\'s 52%, which is how '
     'the Mojave works). Red four ways: a default in TIERS -> 2; the loader crashing on a bad '
     'blob -> 1; the ruin sharing nothing with the corner -> 1; the asphalt outlasting the '
     'block wall -> 1. Nothing on a play surface (rule 18). '
     'records/BOHEMIA_WORLD_THE_FUTURE_IS_DERIVED_FROM_ONE_LEDGER_9_23_26.md', True),
    ...
    ('PEOPLE CHARGE', ['node', 'gates/people_charge_gate.js'],
     ...
     'jesus christ mfs can charge batteries." MEASURED LIVE ON THE WALKED SURFACE AND HE '
     'IS RIGHT: cellsNightlyCharge() is the ONLY maker of batteries in this game, it walks '
     'the lit circuits, finds the FACTION holding that ground and credits THAT FACTION one '
     'cell (reason string "a day on a live wire"). A person never makes one, the player '
     'never makes one -- the whole money supply of Las Vegas is minted by fourteen outfits '
     'holding wire, and every human being is locked out of the one act that creates money. '
     'engine/bohemia_charge.js fixes it. HOLDS FIVE THINGS. (A) the defect is READ off the '
     'real surface, so the day somebody wires a person in the check says so rather than '
     'staying red forever. (B) A BUILDING HELPS, IT DOES NOT OWN THE ACT -- his sentence as '
     'a test: take every building away and a person with a rig and a source still charges. '
     '(C) the physics is real and not tunable (3.75 Wh a cell, 0.70 charger efficiency, 5 '
     'Mojave peak-sun-hours, each with its source) while the RIG TABLE SHIPS EMPTY and asking '
     'whose rig it is answers NO_RULING. (D) *** THE BAYS ARE THE CEILING, AND THAT IS WHAT '
     'KEEPS HIS PILLAR ALIVE: *** run the real arithmetic and an ordinary 10 W panel means '
     'the sun OFFERS 9.33 cells a day against a day\'s work paying ONE, which would end '
     'EVERYTHING COSTS ONE inside a week. The sun was never the real limit on charging AA '
     'cells -- a bay holds one cell -- so output is capped by bays, and the gate proves the '
     'cap bites by pouring forty times the power into four bays and still getting four. '
     '(E) RULE 29: all six of this lane\'s text items were voted down ("boring asf"), so the '
     'cook is a THING DRAWN -- the tile is a real PNG, one thing on it is lit and it is a '
     'small thing, and every unjudged item this lane holds is an image and not a page.', True),
    ('FULL SHELVES', ['node', 'gates/full_shelves_gate.js'],
     'THE-DAY-AFTER-THE-MONEY-DIES (board row [full shelves], 9/21). RULE 12 FIRST AND '
     'THE FIRST HALF OF THE ROW WAS ALREADY TRUE: driven through the game\'s own '
     'payday.buy(), a purchase is a CONVERT, so the battery is DESTROYED to make the '
     'good and the supply falls with every sale. Measured -- stock the valley the way the '
     'game stocks it (19 holders, 56 batteries), spend every holder down: 56 burned, '
     'BATTERIES IN THE VALLEY 0 across 19 hands, nobody minted one, the shelves still '
     'hold goods, every purchase answers CANNOT_AFFORD. The valley really can run out and '
     'it already does. WHAT WAS MISSING IS THAT NOTHING MOVES AT ALL ONCE IT HAS: there '
     'was no fallback, so the game stands still with full shelves forever. '
     'engine/bohemia_aftermoney.js is the second half, and it ROUTES rather than '
     'reinventing -- swapping is bohemia_barter and owing is bohemia_lend, both already '
     'shipped by this lane with no caller for this, and the gate proves it by taking each '
     'module away and watching the way it carried disappear. HOLDS THREE THINGS. (A) *** '
     'YOUR EMPTY POCKET IS NOT A DEAD CURRENCY *** -- an empty purse and an empty valley '
     'both say CANNOT_AFFORD from inside a shop and the rules only change for one; if '
     'being broke flipped the world into barter the game would announce the collapse of '
     'money on a player\'s first bad afternoon. state() has three answers and all three '
     'are proved reachable and distinct. (B) AN UNSTOCKED SAVE IS NOT A COLLAPSED ONE: '
     'zero batteries across ZERO hands is a game nobody has counted, and calling it DEAD '
     'would fire the beat on the first frame -- the same reading that made the night card '
     'say 0 on the first night. (C) PRICES STAY HIS: a dead currency does not re-tariff '
     'the valley, it stops the price meaning anything, and the module invents no number. '
     'Plus the posted SCHEDULE OF PRICES, built from the goods the economy actually has, '
     'every line his ONE, which does not know nobody can pay it. Red two ways: let being '
     'broke mean the money is dead -> 3; call an uncounted save collapsed -> 3.', True),
    ('SUBURB WALLS', ['node', 'gates/suburb_walls_gate.js'],
     'THE-WALLED-SUBURB-NUMBERS (board row [suburb walls], 9/21). The row asked for three '
     'numbers to be put in the table the walled-suburb law reads. RULE 12 FIRST, AND THE '
     'TABLE IS READ AGAINST THE WRONG SCALE, so no number put in it could have worked: '
     'the plot generator decides with `quality >= wallThreshold` where the default is 1 '
     'on a 0..4 integer scale, and the overmap hands it a 0..1 FLOAT. Measured live '
     'through the real bridge on a real map: estate 43/43 walled, gated 30/30 walled, '
     'trailer 0/15 (forced open, correct), and *** ORDINARY SUBURB 0 OF 2,558 *** -- the '
     'only district that asks the threshold, failing every time, in a city whose building '
     'code (Clark County UDC 30.64.020) makes the perimeter wall MANDATORY on a '
     'subdivision. The law was locked 7/14 and has never once fired. AND TWO OF THE '
     'ROW\'S THREE DECIDED NUMBERS NAME THINGS THAT CANNOT HAPPEN: "walls itself when '
     'quality is in the top half" contradicts Paolo\'s own 8/1 bank law that a wall '
     'signals nothing and a GATE is what is bought, and measured it would still leave 586 '
     'of 2,558 tracts without a wall the code requires; "one rear service gate for tracts '
     'over twelve cells" names a tract the valley never builds -- 1,774 tracts, sizes 1, 2 '
     'and 4, THE LARGEST IS FOUR. engine/bohemia_tract.js is the replacement table, every '
     'number DERIVED FROM THE MAP: the wall threshold is the valley\'s own quality floor '
     '(so all 2,558 are walled), the gate stays the map\'s call and never quality\'s '
     '(coarse buckets cannot express a 2.8% minority), the rear service gate is pinned to '
     'the biggest tract that exists and needs a second street, and the wall tiles come '
     'from his twelve approved picks spread across the measured range. Checked '
     'behaviourally, not by grep: hand it a different valley and every number moves. '
     'Plus the covenant violation notice a walled tract comes with, which refuses to '
     'issue without the covenant section it is enforcing. Red two ways: type the wall '
     'threshold -> 3; let quality decide the gate -> 1. The live-generator check is '
     'written so it passes when somebody FIXES the scale bug, never only while it '
     'stands.', True),
    ...
    ('FOLD CARRIES', ['node', 'gates/fold_carries_gate.js'],
     ...
     'of thirteen rows and fold() NEVER CONSULTS IT. fold(ledger, memory) joins the two '
     'halves, names the one it is missing, and hands both through untouched -- driven '
     'with a full ledger every field comes out identical, debt included, and debt is a '
     'row the table marks RULED and DIES. That is not a bug in fold(): the ruled rows are '
     'executed by whoever owns the field (standing.inherit decays, owedFold takes the '
     'rent book, [debt carried] kills the bill). WHAT WAS MISSING IS AN OWNER FOR THE '
     'MATERIAL ROWS. Eight rows are ruled:false reading `carries: whole` -- territory, '
     'builds, economyCapacity, invest, karma, virtues, family, wounds -- so every acre '
     'and every building crossed whole and THE HANDOFF COST NOTHING. The table says it '
     'itself on the builds row: "a hard ratchet today. Buildings turn over in decades and '
     'the century rule asks for visibly poorer, which is arithmetically impossible while '
     'this only climbs." DYNASTY\'s carry list (banks/, 9/5) rules the shape and needs no '
     'number: THE HOUSE is "held, not owned forever, and it can be lost", THE WALLS "come '
     'down if nobody keeps them up". A rate would be a number nobody ruled; A CONDITION is '
     'not, and the game already publishes the most visible one there is -- a circuit goes '
     'dark when the rent on that ground went unpaid ([held ground], 9/5), stays dark, and '
     'rides its own save key. SO THE HEIR KEEPS WHAT IS STILL LIT AND LOSES WHAT WENT '
     'DARK, which wires the generation to the rent you did or did not pay. THE STREET IT '
     'FRONTS, NEVER ITS OWN CELL: measured on the real map, of 620 buildable desert cells '
     'ZERO carry a circuit of their own (you build on sand, circuits are street runs) and '
     '477 front one at distance 1 -- asking the plot would have answered null every time '
     'and shipped correct and inert, the same trap [water lifted] paid for. ON THE WALKED '
     'SURFACE, through the game\'s own ctFold(): builds placed one per distinct live '
     'circuit, half the circuits doused, and the fold keeps the lit ones and removes the '
     'dark ones, with the building count really falling -- BOTH DIRECTIONS IN ONE FOLD, '
     'because a test that only shows buildings disappearing is equally consistent with '
     'demolish-everything, which would be worse than the bug. Red four ways: take '
     'everything -> 5; take nothing -> 5; say it and never demolish -> 3 (31 -> 31, rule '
     '14d exactly); ask the plot\'s own cell again -> 3. TWO WIRING BUGS THE PROBE CAUGHT '
     'BEFORE ANY OF THAT: the first cut walked CE.spans(), but build() writes into '
     'edits.CELLS and spans() is only the big ones, so it saw 0 of 6 real builds; and '
     'demolish() writes `desert` into a cell rather than deleting the key, so counting '
     'keys counts rubble as buildings. Both now use CityEdit\'s own classifier. It takes '
     'nothing it cannot judge, nothing when the ruling is asked and comes back `whole`, '
     'and nothing in the fields other lanes own (karma, virtues, family, wounds).', True),
    ('BATTERY WORTH', ['node', 'gates/battery_worth_gate.js'],
     'YOU-ARE-BUYING-THE-CONTAINER (board row [battery worth], 9/15). THE ROW ASKED FOR '
     'A DENOMINATION LADDER AND THE RECORD IT WAS HARVESTED FROM REFUSES ONE (rule 12): '
     'BOHEMIA_ECONOMY_DAY_8 section 4b says "EVERYTHING COSTS ONE already removed the '
     'denominations of price... if he ever wants physical denominations make them '
     'BODY-SCALE, NOT NUMERIC: a cell in a pocket, a car battery two people carry. Not '
     'one, five and twenty five." What that record puts in its place is the finding it '
     'calls the one that proves us wrong: "WE HAVE BEEN ASKING WHAT A BATTERY IS WORTH. '
     'THE REAL QUESTION IS HOW MANY BATTERIES EXIST, BECAUSE THE VALLEY CAN MAKE CHARGE '
     'ALL DAY AND CANNOT MAKE A SINGLE CELL." NOTHING IN THIS GAME HAD EVER ASKED IT, and '
     'until [every pocket] (9/14) nothing could -- there was one purse, and the sum of one '
     'purse is not a money supply. MEASURED: ten days of work take the valley 0 -> 10 '
     'batteries and the purse own flow calls all ten `source`, its word for made-from-'
     'nothing. A DAY OF WORK MINTS A CELL, without bound, which is exactly the infinite '
     'money the record warns about. SO THE COUNT SHIPS AND THE FIX DOES NOT, ON PURPOSE: '
     ...
     'that number, and shipping an empty pool means a day of work pays nothing on day one, '
     'which breaks the loop [rice clock] exists to close -- this lane broke the first bag '
     'of rice once already this round and will not do it again to make a mechanism fire. '
     'The stock is [PENDING Paolo]; when it lands this is an hour of work. ON THE WALKED '
     'SURFACE: the card he reads every night says BATTERIES IN THE VALLEY: N across 15 '
     'hands, and whether more exist than last night; ten days of work really makes ten; '
     'paying rent moves them and makes none. ALSO NAMED SO NOBODY HUNTS IT: the 267x unit '
     'bug is real and unreachable -- `power` is priced at one battery with unit kWh and one '
     'kWh is 267 AA cells of energy, but power is on NO shelf (fortress water/food/meds/'
     'fuel, town water/food/meds, camp water/food) and nothing converts kWh into batteries. '
     'Red three ways, mutation-tested: count only the player instead of the valley -> 5; '
     'drop the line off the night card -> 1; let made() count transfers as minting -> 1. '
     'THAT LAST ONE PASSED AT FIRST, because nothing had moved yet when the check asked, '
     'so the check now moves cells between holders BEFORE asking -- a check that cannot '
     'tell made from moved is the broken one, and that is the whole claim of the row.', True),
    ('TWO PRICES', ['node', 'gates/two_prices_gate.js'],
     'ONE-PRICE-IN-THE-WHOLE-VALLEY (board row [two prices], RE-AIMED by PAOLO 9/15 '
     'ruling 1). HIS WORDS: "THE SURCHARGE IS DEAD. EVERYTHING COSTS ONE is his pillar '
     'and it holds everywhere for everyone. The spread lives in ACCESS and DISTANCE... '
     'never a number above one." This lane built the stranger surcharge on 9/15, it made '
     'the first bag of rice cost TWO DAYS OF WORK at the market two blocks from the bed, '
     'and it was taken back out (aa3ca379). What ships instead is ACCESS, from ruling 8 of '
     'the nine defaults (9/13): "A TRADER WILL REFUSE MONEY AND ASK FOR GOODS: YES, AT A '
     'CAMP, TO A STRANGER... a camp shelf to a stranger trades goods for goods; a counted '
     'person pays one." BARTER_ONLY had been in bohemia_payday since it was written and '
     'HAD NEVER RUN, because PRICE_SOURCE is the hard-coded string economy; this is the '
     'door it was built for, opened where his ruling says and nowhere else. THE SWAP ITSELF '
     'IS NOT BUILT, ON PURPOSE, AND THAT IS MEASURED NOT ASSUMED: the first cut really did '
     'trade through the purse atomic convert() and MOVED NOTHING -- applied:true, goods '
     'delta 0, battery delta 0 -- because the purse has ONE goods pocket, so one good for '
     'one good takes a resource and hands a resource straight back. A transaction that '
     'reports success and changes no balance is rule 14(d) exactly, so the REFUSAL ships and '
     'the swap waits on goods the purse can tell apart. ON THE REAL MAP, reading the game '
     'own mktShelf() and mktBuy() at EVERY market in the valley: not one price anywhere is '
     'above one; the camp (Colorful) refuses a stranger with BARTER_ONLY and moves nothing, '
     'battery 0 goods 0; belong to them and the same shelf sells at one battery. AND THE '
     'TUTORIAL TRIPWIRE: the market he wakes beside is the Church seat, a TOWN, and the '
     'first bag of rice still costs ONE battery -- that check is the guard on the exact '
     'regression this row shipped and reverted. Red three ways, mutation-tested: let a town '
     'refuse a stranger too -> 3, INCLUDING the rice check reading battery 0 goods 0; make '
     'the camp never refuse -> 3; let a price above one onto a shelf -> 6. Nothing is typed: '
     'the stranger rung is read off his ladder and a behavioural check moves the rung and '
     'watches the refusal move with it.', True),
    ('EVERY POCKET', ['node', 'gates/every_pocket_gate.js'],
     'THERE-IS-EXACTLY-ONE-PURSE-IN-THE-WHOLE-GAME (board row [every pocket], 9/14). '
     'MEASURED FIRST (rule 12) and the board\'s premise was half wrong in the direction '
     'that mattered. The row said the swap primitive "is built, works and is half-called". '
     'There is no swap: NO FUNCTION ANYWHERE IN engine/ TAKES TWO PURSES, transferOut and '
     'transferIn are two one-legged posts that were never joined, and transferIn HAD ZERO '
     'CALLERS IN THE GAME -- not half-called, the receiving half had never been called '
     'once. BohemiaPurse.create() has exactly one live call site, purseGet(), so the '
     'player\'s balance WAS the money supply of Las Vegas. Driven with the city\'s own '
     'rent-night call over one week: the ledger says 10 batteries moved-to-another-holder '
     'and 0 are in anybody\'s hands, supply 9 -> 0. EVERY BATTERY THE PLAYER PAID WAS '
     'BEING DESTROYED, while the code at the road site said out loud "a transfer moved to '
     'another holder. The crew HAS the cut." AND THE GAME ALREADY NAMED WHO SHOULD HAVE '
     'RECEIVED at all four payment sites, in the ref: the crew, r.faction, r.who, the '
     'person\'s id. So engine/bohemia_pockets.js is the book of holders and hand() is the '
     'one way a battery moves between any two of them -- transferOut on one purse and '
     'transferIn on the other, ATOMIC, unwinding leg one the way convert() has inside one '
     'purse since 7/31. WHO HOLDS ONE IS ECONOMY Q40\'s ruling, TWO POCKETS AND A '
     'WAREHOUSE: the player plus a treasury for each of the 14 factions that can hold '
     'ground (derived off the faction graph, never typed), and NO POCKET FOR A SHOP, '
     'because a shop holds goods in ledger.stocks and a ledger per person is 28,844 of '
     'them. A lender, a crew or a person you wronged gets one THE MOMENT THEY ARE HANDED '
     'SOMETHING, which is a handful a playthrough and beats destroying the battery. '
     'Treasuries ship EMPTY: what a faction starts with is a ruling. NO SAVE KEY ON '
     'PURPOSE -- the player\'s own purse is memory-only, and treasuries that outlived it '
     'would mint batteries across a reload. ON THE WALKED SURFACE, driving the game\'s own '
     'blockRent() and loanNight(): 15 holders from the first frame, three rent nights take '
     'him 40 -> 37 and leave the Mob holding 3, a loan repayment really reaches CHURCH, '
     'and the supply is 40 before and 40 after. THE DEMO IS REPORTED, NEVER CHECKED: rule '
     '14(a) reserves the cutter for THE RUN, and a check demanding this in a demo the lane '
     'may not cut would be red through nobody\'s fault. Red four ways, all mutation-tested: '
     'drop the receiving leg -> 6; delete the unwind -> 3; revert one city site to a bare '
     'transferOut -> 5 (including the supply going 40 -> 37 on the real surface); type the '
     'faction list into the module -> 1. THAT LAST ONE PASSED AT FIRST and is why the check '
     'is behavioural: a source scan cannot see a typed list, because strings must be '
     'stripped for the mention-vs-use checks and a typed list lives in strings. It now adds '
     'a fifteenth faction to the graph and watches the module find it.', True),
    ('BACK OF HOUSE', ['node', 'gates/back_of_house_gate.js'],
     'THE-CASINO-HAS-NO-BACK (board row [back of house], 9/13). Harvested from ECONOMY '
     'round 6: "our own economy module has said since 7/19 that downtown matters for its '
     'deep casino/resort dry stores, and there is no dry store, no laundry, no boiler, no '
     'loading dock in any building in this game. Every job the city offers happens in a '
     'room that does not exist... Needs COOK for the rooms." MEASURED FIRST AND THE ROW IS '
     'HALF WRONG IN THE MOST USEFUL WAY: the rooms EXIST. A casino is zone `leisure` and '
     'already lays out concourse, counter, kitchen, locker, restroom and service; a shop '
     'already gets a stockroom; a warehouse already gets a dock; bohemia_furnish already '
     'fills a stockroom wall to wall with racking and pallets; you can already walk in. So '
     '"needs COOK" is NOT TRUE and front-page rule 12 says measure a named blocker rather '
     'than wait on it -- the rooms are generated and furnished by CODE, and not one piece '
     'of art was needed. WHAT WAS REALLY MISSING WAS TWO THINGS. ONE: the one room the '
     'economy asks for BY NAME was the one room it never got -- leisure was the goods-heavy '
     'zone with no store in its list, so the dry store the economy is built around was the '
     'room nobody laid out. It has one now, and it is the EXISTING stockroom, already '
     'furnished (REUSE-FIRST). TWO, and this is the whole row: NOTHING THAT PAYS YOU COULD '
     'SEE ANY ROOM AT ALL. YIELD is two flat numbers and bohemia_work did not contain the '
     'word room, so a sweep through the back of a casino paid exactly what a sweep across a '
     'car park paid, and every interior in the valley -- built, furnished, walkable -- was '
     'invisible to the only thing that rewards you for being in it. NOW THE ROOM DECIDES, '
     'AND IT WINS BOTH WAYS: a dry store on a nothing block is a SITE, a casino concourse '
     'on a job district is a SWEEP. NO NEW NUMBER IS BORN -- site against scav is the '
     'economy\'s own pair, already 3.0 against 1.2, already untuned and already his -- and '
     'WHICH ROOMS HOLD GOODS IS DERIVED FROM THE FURNISHER, never listed, so emptying the '
     'furniture out of a room stops it holding goods and a new room gets the right answer '
     'for free. The button names the room. Driven on the walked surface AND the demo by '
     ...
     'as a store -> 5.', True),
    ('SOMEONE LENDS', ['node', 'gates/someone_lends_gate.js'],
     'WE-BUILT-THE-COURTHOUSE-AND-NEVER-ISSUED-A-LOAN (board row [someone lends], 9/13). '
     'Harvested from ECONOMY round 9: "Everything courtless credit needs is LIVE... Nothing '
     'in the game ever lends anybody anything, so the enforcement machine has never had a '
     'debt to enforce. Let a person or a faction lend you batteries, on a handshake, '
     'remembered by the machine that already exists; the debt names its lender and the '
     'street finds out if you do not pay." MEASURED AND THE ROW WAS RIGHT: zero hits for '
     'lend/loan/borrow in engine/ that are not prose, and the only two that read like '
     'credit are BARKS -- people in this game TALK about lending and nobody lends. '
     'A LOAN IS NOT A NEGATIVE BALANCE, which is why the purse never had to bend: its rule '
     'that balances never go negative was never in the way, because a real loan is TWO '
     'facts, the batteries really arriving through the purse\'s own writer and an '
     'obligation recorded beside them naming who it is to. WHO LENDS IS NOT A NUMBER THIS '
     'LANE PICKED: a lender is somebody who gives before you have earned it, and '
     'bohemia_favour\'s own `owes` flag already says who that is, so no rung and no '
     'standing floor is typed -- 4 outfits lend and 12 will not, off the game\'s own table. '
     'ONE BATTERY ON A HANDSHAKE (his 8/15 ONE in his 9/4 BATTERY) COMING BACK ONE A NIGHT, '
     'which is the shape the night already has for rent, so nothing new was invented to '
     'make a loan bite. IT CAN CLOSE: the account is deleted at zero, because an interval '
     'that cannot close is a sentence and not a relationship (THE DEBT GETS CALLED IN 8/18 '
     'rule 2), and INTEREST SHIPS EMPTY because a rate is exactly the thing that makes it '
     'uncloseable. It lands in [debt carried]\'s one book as a third kind, so it is named '
     'on the nightfall card, dies at the fold and leaves its lender standing, with no part '
     'of that machinery knowing what a loan is. A NIGHT YOU GO SHORT IS PUBLISHED AS A DEED '
     'and really is witnessed and retold. MEASURED RATHER THAN ASSUMED: DEED_WEIGHT is NOT '
     'empty-waiting-on-a-ruling as this lane first assumed -- it has 83 rows, all '
     'q:<quest>:<stage>@<FACTION> derived from the authored quest corpus, and NOT ONE of '
     'the five city-published deed kinds is in it (claim:met, claim:refused, commit, favour '
     'and this one), so every act the walked city publishes weighs nothing. One gap for all '
     'five, ROUTED, never patched with a number invented here. Red six ways: make the debt '
     'never close -> 3; let anybody lend -> 1; take the whole lot in one night -> 3; never '
     'publish the deed -> 1; hand over nothing -> 1; keep it out of the one book -> 3. '
     'AND THE GATE ITSELF WAS FIXED TWICE BY THOSE MUTATIONS: it first re-implemented the '
     'button\'s own two lines instead of pressing the button, so deleting the credit left '
     'it GREEN -- a gate that re-implements the thing it is testing cannot see it break; it '
     'now stands next to somebody from a lending outfit and clicks.', True),
    ...
    ('VALLEY RUNS OUT',['node', 'gates/valley_runs_out_gate.js'],
     ...
     'simulator and a place that is running out is the whole point -- so make it visible day '
     'by day and make the day it empties a BEAT rather than a silent zero. THE BUG '
     'UNDERNEATH IT WAS THE WHOLE PREMISE: mktAdvanceDay read "if(!MKT_LEDGER) return null" '
     'with the note "never censused = never traded = nothing to age", which is true of a '
     'SHOP and false of a VALLEY. Measured: the ledger does not exist at boot, so a player '
     'who had not walked into a market had a world that never ate anything, and the first '
     'time he did the ledger was born FULL and the countdown started THEN. A place that only '
     'starts running out when somebody checks is not running out. EVERY NUMBER IS READ, NONE '
     'IS TYPED -- bohemia_economy has computed daysLeft since it was written and nothing '
     'outside a market card ever asked; food goes 8.4 days to 0 across exactly TEN DAYS off '
     'his own stocks and needs, scheduled by nobody, and check 1 holds the readout clear of '
     'any typed day, stock, rate or threshold. The reckoning card counts down every night '
     'and says "THE FOOD IS GONE" once, on the night it happens, then moves to whatever is '
     'scarcest next', True),
    ...
    ('THE FOUR THINGS', ['node', 'gates/the_cut_is_the_four_things_gate.js'],
     ...
     '"I am really trying to push this demo out... I DON\'T NEED THE WHOLE WORLD, I don\'t need '
     'everything." MEASURED ON THE SERVED CUT AT 390x844 BEFORE ANYTHING WAS STRIPPED: TWENTY-SEVEN '
     'named boxes on his first screen, fourteen of them things rule 18g names for the bin -- MUSIC, '
     'SAVE, HUMAN MODE, SUBURB - ON FOOT, DAY 1 - 06:00 twice, and a left rail of six chips '
     '(STANDING, BUILD HERE, SCAVENGE, BIKE, SLEEP, MARKET) plus a line of prose with no mouth. '
     'AFTER: FOURTEEN with the drawn phone counted ONCE (it is a 46x62 object made of six named boxes, '
     'and counting the parts of one object as clutter would punish the phone-on-the-street fix this '
     'lane asked UI for); both taken on a game the driver really opened (the first pair read 24 -> 10 '
     'through a loading screen the driver never pressed BEGIN on). The cut subtracts from the DEMO '
     'SIDE ONLY, like the builder drawer and the cold '
     'open, so the alpha keeps every control with its code and its gates -- and this gate opens the '
     'alpha afterwards and checks all eight are still there, because a cut that deletes a feature '
     'is not a cut. THE HALF THAT MATTERS MORE IS REACHABILITY, asked with a real finger on the '
     'stripped cut and never off the source: the pad still walks him (12 presses, 52 cells), NOTES '
     'still opens and holds words and folds again on its own CLOSE, the drawn phone still catches '
     'the point and opens on one touch, and the fight still starts although the tab strip it '
     'reaches through is hidden. Rule 14h: every one of those requires the thing to still be OPEN '
     'with its words arrived, never just "something changed". MUTATIONS, three, each caught and '
     'each restored: let the left rail back -> 3 red (and the box count rises by seven); strip the '
     'walk pad too -> 2 red and twelve presses move him 0 cells; let NOTES slide to x=16 -> 1 red.', False),
    ...
    ('BEAT ON A PHONE',['python3', 'gates/beat_survives_a_phone_gate.py'],
     "A LAW WITHOUT A MACHINE GATE IS NOT ENFORCED, and the 120 BPM law -- this "
     "lane's own law -- had no machine watching it on a phone. BEAT FIRST exists "
     "and runs UNTHROTTLED, so it has always asked about a machine several times "
     "faster than the one he holds; PLUMBER found that class of mistake fleet-wide "
     "(f90a810) and measured the cost: 31.6% of beats late on a phone-shaped CPU, "
     "8.3% swallowed whole, one beat in twelve never happening. MEASURED HERE by "
     "wrapping MUS.playStep, so the only countable thing is a step really handed to "
     "Web Audio: on a phone only 42 of 160 steps in twenty seconds were booked, "
     "26%, and even on this fast box 128 of 160. The cause was the horizon, 0.12 s "
     "on a thread that stalls for seconds; it is now ONE BEAT, 4*stepDur(), the "
     "engine's own unit and no new number. This gate throttles and PROVES the "
     "throttle inside the run with a WARMED yardstick (the first version ran the "
     "busy loop cold then warm and reported 0.51x with no throttle applied -- it "
     "was timing the JIT). It holds BOTH sides, because a coverage floor alone is "
     "satisfiable by making the game unresponsive: a booked step cannot be "
     "un-booked, so the horizon IS how long the music takes to obey a change, and "
     "the committed music must stay inside one beat. Its on-time claim is a RATCHET "
     "at half the measured before-value rather than a tolerance picked off one "
     "lucky run, which is the mistake it caught me making. Mutation puts the old "
     "AND ITS COVERAGE VERDICT IS PAIRED, because an absolute 40% floor set off two runs then "
     "failed a good build when four legitimate 4x runs read 53.1, 50.0, 48.8 and 33.8 "
     "percent: COVERAGE ON A THROTTLED BOX IS PARTLY A MEASUREMENT OF THE BOX. It runs the "
     "same window with the shipped horizon and then with the old 0.12 s one, minutes apart "
     "on one machine, and holds the RATIO (2.1x, 1.8x, 4.5x measured; floor 1.4x), printing "
     "every absolute number and asserting none -- the suite's own rule, pair every "
     "before/after inside one window or you measure the hour. LOAD-SENSITIVE "
     "BY NATURE AND HANDLED, NOT HIDDEN: it went RED in the pack and GREEN alone on "
     "its first suite run, because a gate that THROTTLES the CPU is applying a 4x "
     "floor to an already-loaded box. A GATE THAT ASKS FOR A CPU RATE MUST CHECK THE "
     "RATE IT GOT, so when the measured ratio overshoots the asked rate by more than "
     "half again the coverage claim reports NOT APPLICABLE with both numbers instead "
     "of reading red, and the mutation reports INCONCLUSIVE rather than a pass, "
     "because an overloaded box collapses coverage on its own and would fake it. The "
     "floor itself is untouched", True),
    ('COOKED SOUNDS',['node', 'gates/cooked_sounds_gate.js'],
     "Row [cook sounds], rule 22 (Paolo 9/21): 'I will enter the sound chat and it is "
     "not even making fucking sounds. It is coding and checking whether the sounds are "
     "broken or not... I need to be seeing them cooking up more, every time, not never.' "
     "THREE COOKED SOUNDS HE CAN PLAY, held to the ten rules of this lane's own school "
     "page: a footstep that lands ON the beat (noise-first, because 51 of the 65 shipped "
     "sounds measured as near-pure tones and a real footfall is not a tone at all), the "
     "same step LOSING CONTACT at ANIMATION's own ground stations, and the phone's "
     "two-tone broadcast over a carrier that outlives it. It reads engine/"
     "bohemia_horror_sounds.js, the one body the vote page also plays from, so no number "
     "is typed twice. *** AND IT IS DELIBERATELY NOT THE ROUND'S OUTPUT: the output is "
     "three sounds in the vote tab, and this only stops a later round undoing them. *** "
     "THE BAND CLAIM COST FIVE ATTEMPTS AND EVERY ONE OF THEM BLAMED A SOUND THAT WAS "
     "CORRECTLY BUILT. A single corner frequency is not one quantity across an impact "
     "with a low thump, a hiss bed and a tone over a carrier: the -20 dB point rides on "
     "noise scatter (a real roll-off measured as reaching Nyquist), the -3 dB point rides "
     "on whichever peak dominates (a footstep measured 108 Hz). Twice I changed a good "
     "sound to satisfy the bad ruler, once breaking its band and once filtering the life "
     "out of it, flatness 0.253 down to 0.0095, which is exactly the near-tone the cook "
     "exists to replace. The claim is ENERGY now, which is shape-independent, the split "
     "is four cascaded poles so the measure is not its own leak, and the 5 percent and 1 "
     "percent bars sit an order of magnitude clear of the readings (0.54, 0.63 and 0.03 "
     "percent) instead of inside their spread. The school page's rule 4 was corrected to "
     "match, because the record was wrong, not just the code. Mutation replaces all "
     "three recipes with a bare sine before any reading and 8 claims go red; it reports "
     "them as claims rather than dying, because a checker that crashes does not say "
     "which claim broke", True),
    ('ANALOG HORROR SOUND',['node', 'gates/analog_horror_sound_gate.js'],
     "Rule 20 (Paolo 9/20, LOCKED): 'everything that's a sound has to be thought about "
     "as analog horror.' Row [analog horror sound] round two, the KEEP/REDO list. IT "
     "LANDS GREEN ON A SHELF THAT BREAKS EIGHT OF THE TEN SCHOOL RULES, ON PURPOSE: a "
     "checker written to the law outright goes red on 65 shipped sounds at once and "
     "breaks the suite for twenty lanes over work nobody has been asked for, so it is a "
     "RATCHET, the pattern reference_check_gate.py already settled on here. Today's "
     "shelf is FROZEN in records/target/BOHEMIA_THE_KEEP_REDO_LIST_9_24_26.json and no "
     "sound may get duller, clip, or grow past its own speaker; any sound added from "
     "9/24 on is held to its class OUTRIGHT with no grandfathering; and every sound must "
     "carry a line in banks/BOHEMIA_WHAT_MACHINE_IS_IT_9_24_26.json, so a sound cannot "
     "enter the game without somebody saying what it came off. *** THE CLASSES COME FROM "
     "DIRECTION'S BIBLE, NOT FROM THIS LANE'S TASTE: rule 8 is DIEGETIC OR DEAD, tape "
     "damage lives only inside in-world speakers and the lens is an eye, so the school "
     "page's rule 4 cannot be asked of a footfall under your own boot. Measured: 57 of "
     "65 are heard directly, 3 are machines you can point at, 5 come through a speaker, "
     "so THE REDO LIST IS 28 SOUNDS AND NOT 65. *** It measures with the tool's own "
     "exported body rather than a copy of it, because a checker carrying its own ruler "
     "is the duplication that silenced every footstep in this game for days. AND THE "
     "PITCH RULER IS CHECKED ON SINES WE KNOW EXACTLY BEFORE IT IS USED: one bin is "
     "10.77 Hz, eighteen percent at 60 Hz, and the first cut read the generator at 54 Hz "
     "and nearly accused a hum that is not far off 60. Refined below the bin the worst "
     "error is 0.17 percent against the raw bin's 7.67, so rule 2's tolerance is 1 "
     "percent, six times the instrument's own measured error. Mutation is REAL and not "
     "calculated: the brightest sound loses half its top end and one sound loses its "
     "table line, both injected before any claim runs, and the two claims that carry the "
     "gate go red", True),
    ('FIRST SOUND',['python3', 'gates/first_sound_gate.py'],
     "[first sound], un-held under rule 18 because it is LOADING. The row's ship "
     "test in its own words: sound within one beat of the tap on a 4x phone "
     "profile, a room hum and not a jingle, covering the measured gap to the first "
     "song. THE HEARTBEAT (9/5) WAS THE RIGHT MECHANISM AND THE WRONG SOUND: two "
     "thumps at 58->34 and 50->30 Hz, so it is all sub-bass and it is rhythmic, an "
     "event repeating rather than a room. Measured on the whole shipped mix the "
     "same round, 96 percent of this game's sound energy is already below 320 Hz "
     "and 0.02 percent is above 5 kHz, which is the hiss band, empty. So a 60 Hz "
     "mains hum with broadband hiss on top of it, band-limited 100 Hz to 5 kHz "
     "(an AM broadcast, the machine it declares), starts on the same tap and DUCKS "
     "rather than stopping when the song lands. The heartbeat is untouched. "
     "*** THIS GATE DOES NOT WATCH AND WAIT, AND THAT IS THE WHOLE DESIGN. Three "
     "polling instruments were built and thrown away this round; the third failed "
     "in the way that matters, because on a 4x profile each round trip into the "
     "page costs about two seconds, so ninety samples spanned a hundred and "
     "ninety-eight seconds and the FIRST sample landed eighty seconds after the "
     "tap. It reported 'first sound 84 s after the tap', which was observation "
     "latency with a timestamp on it. So nothing is observed: the audio thread is "
     "asked what it SCHEDULED, which is true whether or not the main thread is "
     "alive to see it, and that is the reason this sound is a looping buffer. And "
     "the tap IS context time zero, measured: zero AudioContexts exist before the "
     "door, so no wall clock appears anywhere in the claim. *** The level is not "
     "chosen either: it is derived at run time from his own approved heartbeat and "
     "matched on RMS, never on peak, because a thump and a bed cannot be compared "
     "on peak. The loop seam is PAIRED against the buffer's own typical step, with "
     "no absolute tolerance, because five of this lane's thresholds have measured "
     "the box instead of the game. One claim in here was WRONG THE FIRST TIME and "
     "the sound was right: it asked for 60 Hz and measured 118, because a 100 Hz "
     "low corner takes a 60 Hz fundamental, which is what a hum heard through a "
     "transmitter does, and 2x mains is the fluorescent ballast's own pitch. It "
     "now tests what the rule says, an integer multiple of mains, and prints the "
     "multiple. Mutation removes the room's start before the door opens, never "
     "after the reading it falsifies", True),
    ('OFF THE BEAT HEARD',['python3', 'gates/off_the_beat_heard_gate.py'],
     "PAOLO 9/15: 'I don't even know how to engage in combat and when that shit "
     "starts.' The row says a swing on the beat must sound different from a swing "
     "off it, carried by SOUND not words. MEASURED inside the fight, on a real "
     "encounter, with tone/drumV/sfxAsk all wrapped so only audio the frame really "
     "asked for can be counted: the fight grades every press into four bands and "
     "PERFECT rendered 3 notes, GOOD rendered 1, and EARLY AND LATE RENDERED "
     "NOTHING. Two of four bands were silent, so the one thing the first fight "
     "exists to teach was the one thing you could not hear -- and the chain-break "
     "that might have covered it needs groove > 0 (broke(0) false, broke(3) true), "
     "which a player's first press never is. The row's FIRST claim was already true "
     "and COMBAT built it: a metronome runs every beat, ~49 drum hits in six live "
     "seconds, so nothing was rebuilt. This holds the missing branch down and it is "
     "mostly the ways this lane fools itself: the bands come from a SWEEP of the "
     "fight's own gradeOf rather than a list typed here, so it cannot go stale when "
     "COMBAT adds a grade; there is a PHONE FLOOR on the cue's pitch, because the "
     "first version of the fix put it an octave below the family's root and the "
     "family's pitch MOVES with the song -- measured live at 82.4 Hz, which is a "
     "second silence wearing the fix's clothes; and the cue must render at least the "
     "GOOD cue's energy so it cannot become the new silence, with a stated 5% peak "
     "margin because the first cut of that claim failed by 0.8% comparing two decay "
     "shapes, and a tolerance tighter than the instrument's resolution measures the "
     "instrument. Mutation control removes the branch and the bands go silent again", True),
    ...
    ('HAND TO FACE',   ['node', 'gates/the_hand_reaches_the_face_gate.js'],
     'PAOLO 9/7, killing eat, drink, smoke, cough and whistle: \'some of the DIRECTIONS look like dog shit: when it is facing north-east the hand was behind the head even though it is supposed to be in front.\' TWO PROBLEMS IN ONE COAT -- the draw-order half was fixed 9/13, and the other half was worse. ALL FIVE AIMED WITH gunT, THE GUN TARGET: the chest point pushed forward along the facing, right for a pistol held out and wrong for a hand at the mouth, and because the push follows the facing it lands somewhere different every time he turns. The head is 18 px crown to neck and the hand never came closer than 12.9. MEASURED IN THE DRAWN FRAME, not the skeleton, and that is the gate: a joint ruler scored the redo 2.9 px from the mouth on north-east, its best cell of forty, while the PICTURE showed an arm past the head with no hand near it -- both right, because facing away a hand at the mouth is behind the skull, the compositor is first-wins, and the reaching hand is never drawn. A DISTANCE BETWEEN TWO JOINTS CANNOT SEE WHAT IS IN FRONT OF WHAT. So it asks buildFrame for the part id under every drawn pixel and measures hand pixels against the face. RATCHET 9 of 40 -> 33 of 40, may only go up. THE REDO IS DELIBERATELY NOT IN THE BUILD and the gate SAYS SO instead of going red over a thing nobody shipped: it costs NECK HOLDS HEAD 15 detached frames -> 22, every new one facing east, because the forearm that reaches the mouth in profile crosses the throat, and raising the hand to clear it made BOTH numbers worse. Both halves want one missing piece: a clip that DECLARES hand-at-the-face so handOrder can order the arm-unit and the neck off a flag, the way a gun clip declares _gun -- constant per clip, so unlike the rules retired 7/26 it cannot flip mid-swing. 2 mutations caught, and the first cut of its control was VACUOUS: it asked whether any cell was away from the face, and a build with the hand welded on still scored 10 away because the back views are occluded whatever the pose does; it asks the rest phase now, 21 of 32 -> 0 of 32', True),
    ('ELDER STOOPS',   ['node', 'gates/the_elder_stoops_gate.js'],
     'PAOLO 9/21, on the ages he had just voted YES to: \'the old person looks like shit.\' CHARACTER measured why (e357361, 5c6aebb): young adult, adult and elder land WITHIN 3%% OF EACH OTHER, three ages one body, because the only dials were HEIGHT and LEG FRACTION and the elder shared its height with the young adult. AND THE RIG\'S OWN NOTE ALREADY SAID WHAT WAS MISSING -- \'old age compresses the spine, it does not narrow the shoulders\' -- while nothing in the rig could compress a spine. A NOTE THAT NAMES A MECHANISM NOBODY BUILT IS A PLAN, NOT A FEATURE. One bend point above the hips, on a squared curve so the back ROUNDS instead of the torso tipping like a plank; arms come with the shoulders. Measured across eight facings: the elder is 11.6%% shorter than the adult (was 2.4%%) and his head hangs 2.4 px out over his own hip (was 0), and the DRAWN crown sits 8 rows lower with all 8 facings drawing differently -- because a skeleton that moved while the art did not would pass every joint claim, which this lane has shipped before. THE LOAD-BEARING CLAIM IS THAT HIS LEGS DO NOT MOVE: the pivot is above the hips, and a stoop that dragged a knee is a body falling over, not an old man. MUTATION TAUGHT THE SHAPE OF ITS OWN GUARD: adding the leg joints to the bend list is a NO-OP, because the clamp makes anything below the pivot score zero travel, so the legs are protected by construction and not by a list somebody has to remember; the mutation that does move them removes the clamp, and the claim goes 0 to 32 joints, worst 10.7 px. 3 mutations caught, with a CONTROL that the young adult is untouched so a global change cannot pass by showing up worst on the elder. 0.20 was LOOKED AT: below 0.12 it reads as bad posture on a young man, above 0.28 his head passes his toes and he reads as bent double', True),
    ('SLIDE AND TURN',  ['node', 'gates/the_walk_slides_and_the_turn_takes_the_beat_gate.js'],
     'RULE 26, PAOLO 9/21, four words on the walk item: \'I want slide immedialy.\' The tape skip this lane built over three rounds was the MANAGER\'S default, not his, and it is dead. THE REASON SLIDE WAS NOT ALREADY WHAT HE HAD IS WHAT THIS GUARDS: the glide carried a handwritten FOUR-CELL ceiling written 8/23 when a step was ONE cell (its own comment said so), rule 16 made a step twenty-five, so every full step died on its first drawn frame while the chip and the source both said SLIDE and what he walked was GRID -- the whole world jumping 275 px in one frame, twice a second. A NUMBER TYPED INTO A GUARD EXPIRES SILENTLY, because a guard that refuses everything looks exactly like a feature nobody turned on. Saying the feel is SLIDE means nothing unless something checks the ground slides: measured on the PIXELS THE GAME DRAWS by stubbing performance.now() and calling render() at each of the sixty moments a sixty-frame beat lands on (this box clamps a headless page to twenty), SLIDE moves the world on 31 of 60 frames against GRID\'s 0. AND THE TURN, VOTED UP 9/21: there was no turn at all, the facing was assigned the instant the step ran. It is now set in ONE place and that place starts a turn through the eight drawings the game already ships, the short way round. THE JUMP COUNT IS PRINTED AND DECIDES NOTHING, on purpose: wiring it everywhere took it from 10 skipped points (worst 4) to 4 (worst 2), then the same gate on the same tree read 8 (worst 3), because the walk itself swings -- walls redirect him and no two sweeps turn the same corners. A FLAKY RATCHET IS WORSE THAN NO RATCHET: it fails honest work, gets switched off, and costs the gate everything else it holds. AND THE IDLE, VOTED UP 9/21, under the one law that can break it: the feet NEVER move in idle, 0 of 56 frames across eight facings, measured on the DRAWN frame because the joint ruler lied to this lane twice and the picture caught it both times; with a control that the body is not simply frozen. Two rulers for the turn were built and thrown away first, both measuring the mechanism instead of the result', True),
    ('READS FACING YOU',['node', 'gates/a_clip_reads_facing_you_gate.js'],
     'FOUND 9/24 while cooking the two clips Paolo never judged, row [judge two], BY LOOKING AT THEM FIRST -- which is the one thing the hand-at-face graveyard written the same round says this lane failed to do. Facing the camera neither clip did anything at all, and the cause is one line of the rig: spF, the spine-forward sign, is defined as ZERO on N and S, so every term written spF(d)*x vanishes on the two facings the player looks at most and a clip whose whole idea is a forward bend is left with whatever else it happened to say. MEASURED ON THE DRAWN PICTURE, worst key of the bar against the clip own rest frame, pixels changed over the body and how far the drawn centre of mass travels: pickup 126%% / 10.37 px from the side against 36%% / 0.87 px facing you; laugh 28%% / 1.28 px against 10%% / 1.00 px. A MAN PICKING SOMETHING UP, FACING YOU, MOVED LESS THAN ONE PIXEL. THE FIX PATTERN WAS ALREADY IN THE FILE (nod, drunk and the gaits all branch on headOn(d)), so this is that pattern applied and not invented: facing you a squat is spent where it shows -- the body drops, the legs fold, the arms and head go with them -- and a laugh becomes a bounce and a shoulder shake. pickup 0.87 -> 5.52 px, laugh 1.00 -> 4.22, and the side views byte-for-byte untouched, which is its own claim because a fix that trades one facing for another would pass everything else. 3 mutations caught, and the second of them CHANGED THE GATE: a single pixel floor of 35%% sat BELOW what the broken pickup already scored (36.4%%), so removing the fix left that claim green -- A FLOOR UNDER THE BUG IS NOT A FLOOR, and the floors are per clip now. Mutation C is this lane own turn bug written again, the branch wired to ONE of the two head-on facings, and the claim that asks about BOTH is what catches it. CONTROL: nod, which already had the pattern, must still read facing you, so a build where headOn branches do nothing cannot pass. AND THE REST OF THE HOLE IS PRINTED, NOT CHASED: 15 of 105 clips still move well from the side and barely at all facing you, each with its two numbers. Two were on the board; quietly rewriting forty is forty untested changes', True),
    ('ENVELOPE RAMP',  ['node', 'gates/an_envelope_ramps_slower_than_the_grid_gate.js'],
     'PAOLO 9/21, voting down the five hand clips: \'it looks like all of them Northeast and south tweaking.\' The round before proved it is NOT placement -- on those two facings the redone sheets bake BYTE-IDENTICAL to the old ones -- so it is the MOTION, and the defect generalises into a law this repo had already met once and never written down. THE BODY IS POSED ON A GRID OF POSEHOLD.keys KEYS A BAR, TWELVE, AND NOTHING BETWEEN TWO OF THEM IS EVER DRAWN. An envelope written as a smooth continuous curve can still move a joint further between two DRAWN keys than the eye reads as travel, and then the joint does not appear to move, it appears to JUMP, and to jump BACK. Two instances: eat was |sin(ph*2pi)|, TWO bites a bar, the hand crossing 13 px between two drawings and reversing at every hump; cough was max(0,sin(t*3pi))*(t<0.6), which sampled at i/12 is 0,1,0,0,0,0 -- THE WHOLE CONVULSION ONE KEY WIDE, the head moving 10.44 px on north-east and back inside a single drawing, twice a bar. Same family as the tweeze bug the file already carried in a comment, which is exactly why it came back: a note is not a gate. THE RULER IS DIRECTIONAL, not a distance: a step that goes BACK the way the last one came (flipped by more than 120 degrees) between two drawn keys. That is the shape of a jitter and it is invisible to every how-far-did-it-move ruler, which is why five rounds of distance measurements never found this. Measured over the five clips, five joints, his two facings plus east: reversals 26 -> 0 on the two he NAMED and 43 -> 5 overall, worst jump between two drawn keys 10.44 px -> 7.28 on a 98-row body, the cough spanning 2 of 12 keys -> 6. The 5 left are printed and named: four are the cough own duck and return, which is a cough, and the fifth is whistle far hand breathing, identical before and after and never mine to claim. The grid is READ off POSEHOLD and never typed, so a game that draws a different number of keys moves the ruler with it. CONTROL: the exact shape the cough bug had is pushed through the SAME walk and must still score reversals, so one ruler serves both and a gate that goes quietly blind fails its own control -- proved by mutating the reversal test to a threshold that can never fire, which the control catches while every other claim reads green. 3 mutations caught. AND WHEN HIS SECOND VOTES KILLED THESE CLIPS A THIRD TIME MID-ROUND, THE ITEM CLAIM WAS RE-AIMED AT THE RULING ITSELF rather than deleted: I had a vote page on these same five clips built, measured and registered when the graveyard ruling landed, and it was PULLED and its files deleted rather than shown to him a fourth time. This gate now holds the existence of the post-mortem (naming all three of his verdicts) and the fact that NO vote page puts those clips in front of him again -- a ruling with no machine behind it is a note. Both mutation-tested. AND A RULER BUILT AND THROWN AWAY THE SAME ROUND: a connected-component ruler that would have passed the four frames holding the redone poses out was mutation-tested first, and with the neck rule neutered the old ruler goes 15 -> 48 while the island ruler stays 0 -> 0. IT COULD NOT SEE THE DEFECT AT ALL. Deleted, ratchet untouched: A RULER THAT PASSES THE THING YOU WANT TO SHIP IS THE FIRST ONE TO MUTATION-TEST', True),
    ...
    ('ONE NUMBER BODY',['node', 'gates/one_number_for_a_body_gate.js'],
     'RULE 16, THE STEP IS A HOUSE (Paolo 9/15): bodies are drawn LARGER, and the law says the scale lives in ONE constant, NOTHING IS BAKED ONCE, a second copy anywhere is the bug. This lane owns the body pixels and the number lived in SIX places: twice in the bake that ships bodies to the street and in the w/h typed into each of the five senders (city player, city cast, the cast append, the faction cast, the run slice) -- plus a SEVENTH in the bake\'s own name, because a function called bake56 that bakes at a constant is the copy that misleads the next reader. MEASURED FIRST: the rig renders 112x112 with a 98-row standing body and the bake HALVES it to 56 before shipping, keeping 4,691 of 18,739 body pixels (25%) and a 49-row body, while 158 of 160 colours survive -- the loss is RESOLUTION, not palette -- and the city then doubles it back with Scale2x, inventing the pixels just discarded. That was not a mistake but an EXPIRED CONTRACT: 56 was the rig\'s native size when the promise was made, the rig moved to 112, COMBAT got the benefit (bake112 stops upscaling) and the city kept being handed a half-size body. Now one constant, and at 56 it is BYTE-IDENTICAL to what shipped before -- proved, not asserted, 48 baked frames (8 facings x 3 clips x bordered and borderless) compared byte for byte against origin/main, 48 identical 0 different -- so it lands safely before RUN\'s step constant exists, and growing the street\'s bodies becomes ONE edit. The load-bearing claim is BEHAVIOURAL, not a source scan (PLUMBER proved a scan cannot see a typed list once strings are stripped): it intercepts the real postMessage traffic and checks what every sender PROMISES equals what the bake actually SHIPS. The city iframe is LAZY and that nearly made it vacuous -- the first cut caught ZERO senders and scored \'0 of 0 agree\' as a pass -- so the gate installs a target and the sender count is its own claim. CONTROL: combat\'s separate 112 promise is untouched. 5 mutations, and moving the constant to 112 stays GREEN on purpose: the gate forbids a second copy, never the ruling being executed', True),
    ('HEAD SNAPS',     ['node', 'gates/the_head_snaps_back_gate.js'],
     'BEAT ONE of Paolo\'s headshot spec, "head snaps back on impact (0 to 0.08)", which defeated THREE attempts because the reason was invisible. An impulse on headTop at frame zero, the same impulse bigger, and exempting the head from the horizontal speed cap ALL measured 5.1px of head travel against the waist\'s 4.1 -- identical to each other and to no impulse at all. THE PIN, MEASURED: the head has EXACTLY ZERO freedom in the sim. Head angle minus spine angle is 0.0000 degrees on every step of every fall, kicked or not, because the GLUE LAW block in hsStep rewrites headTop off the spine frame every step, so every impulse was overwritten in the step it was applied. The sim\'s lever is the NECK, already kicked at 3.4 px/frame, but the 1.6 horizontal speed cap clips it on the first step while the waist\'s 1.2 passes untouched -- which is the whole of why the head leads instead of snapping. Lifting that cap builds beat one and KILLS beat four (arms close -13.3px against his floor of 12), the same trade the leg window offered and was refused for, so the cap is untouched. Beat one is built at the NECK JOINT, which the sim does not model at all, in hsPose on the rendered skeleton: a pulse inside his window that peaks 30%% in and returns to glued by its end. The sim is never touched, so his other three beats measure IDENTICAL on all eight facings -- verified against the previous build, not assumed -- and the RIGID LIMB LAW below it forces the neck bone back to rest length, which is why the head cannot come off (worst gap 1px, eight facings, out to 0.9 rad). The load-bearing claim is the WHIP: peak head travel inside his window over the travel at the window END, because a head that merely leads moves monotonically and scores exactly 1.00, which is what the build before this scored on all eight. Amplitude 0.45 rad was LOOKED AT, not picked off a table: 0.6 reads long-necked, 0.8 lifts the head off the collar. Prints the one facing whose own fall outruns the whip and names it as BEAT TWO\'s defect. 7 mutations caught', True),
    ('HEAD IN FRAME',  ['node', 'gates/his_head_is_in_the_frame_gate.js'],
     'Paolo 7/2/26 TRACKING CAMERA: "the whole body stays in frame for the entire fall, every knock direction, at STABLE SCALE." FOUR camera blocks -- the tracking pan, the crumple pan, the fixed-frame corpse and the ragdoll centroid -- had 28, 27, 34, 50, 53, 54, 52, 2 and 3 typed out by hand, and every one is a 56-ROW CANVAS. BAKED is 112x112 with RIG_RS 2, and a body just STANDING spans 88 rows (head 14, feet 102), so those cameras centred a 112-space body on the middle of a 56-space frame on every frame of every fall. Measured from a deterministic reset: the headshot sat at -16 to 72 for the whole fall, sixteen rows of head above the top edge, and the drawn sprite was clipped at row 0 in every single frame. HE WAS WATCHING A HEADLESS BODY, on two clips he thumbed down, and MOTION VISIBLE had been red on main on exactly this. One derivation (rigFrame) and four callers; at RIG_RS 1 it reproduces every number that was typed before, so the 56 rig is provably untouched. Carries a code claim that no camera types a 56-space number again, both edges of the frame, and a CONTROL that the camera may not move a body that already fits. 4 mutations caught, and two of them only after the first cut of the control was found VACUOUS (it compared idle, a clip none of these blocks touch)', True),
    ('YOU KILLED THIS',['node', 'gates/you_killed_this_gate.js'],
     'Paolo 9/7: "WE GOTTA RE-ANALYZE A LOT OF THESE. IF I KILLED IT I DON\'T WANT IT GONE." He thumbed 47 clips down and named why; three rounds of rig work later the reasons are measured gone, and he has to SEE that or the second verdict is worth no more than the first. JUDGE ALL now carries a ONES YOU KILLED filter and, on those 47 rows and no others, one green line saying what was wrong and what changed -- every line MEASURED from the same probe run on the rig before his three complaints were fixed and on the rig now. THE LIST IS READ FROM HIS OWN PASTE (records/BOHEMIA_CLIP_VERDICTS_9_7_26.txt), not copied into the gate, so the panel cannot drift from what he actually said. Carries the CONTROL that not one of the 56 he KEPT gets a line, and the row-height ceiling the 9/5 record paid for once (a wrapping note took a row to 200px and put four clips on a phone screen). 3 mutations caught', True),
    ...
    ('COAT ON LEGS',   ['node', 'gates/coat_tied_to_the_legs_gate.js'],
     'Paolo 9/7: "the trenchcoat, as will be the nature for any long jackets and coats, has to be done a lot better; it\'s glitching and popping out of place; tie it more to the legs, it feels like it\'s freestyling where to go." TWO defects, both measured on the real rig over all eight facings and all 24 walk buckets, never on a mannequin (a mannequin has no stride and no swinging arm and cannot show either). ONE: the skirt was a cone about the TORSO\'s centroid that never looked at a leg, so it ran in the GAP between two striding thighs -- 12.2% of leg pixels sat outside the coat on rows it covered, 23% facing away, 16 px on the worst row. Now 3.9%, worst facing 6.5%, worst row 9. TWO: the skirt\'s width came from the torso\'s extent on ONE row, and whichever arm is swinging covers part of that row and uncovers a sliver past it, so the measured hip read 4 px on one frame and 14 on the next with nothing in the body moving; facing you the coat changed 31% of its own area in a single frame against the body\'s 9.7%. A hip is the torso\'s TYPICAL width, so it is area over height now: 31% -> 17.9%. Six estimators were measured before picking that one. Carries a tent ceiling so the reach cannot balloon, and a VEST control so the ruler cannot be measuring the body. 5 mutations caught', True),
    ...
    ('ELBOW ONE WAY', ['node', 'gates/an_elbow_bends_one_way_gate.js'],
     'Paolo 9/7 thumbing the clip list: "you kind of suck at bending elbows at all, and if you do, sometimes they are going the wrong way, looking broken." Every IK clip passed bend:auto, which solved BOTH elbows each frame and kept whichever hung LOWER on screen -- gravity, not a joint, and a knife-edge comparison that gets remade every frame. MEASURED over 40,320 arm-frames: 59 snaps in 11 clips, the elbow moving up to 36px in ONE frame on a 56px body, AND ALL ELEVEN OF THOSE CLIPS ARE ONES HE THUMBED DOWN (found by the machine before the list was compared to his verdicts). The side is now decided ONCE from the arm and the facing and never recomputed: 59 -> 7 flips, 11 -> 3 clips, 36px -> 11.7px. Holds the flip floor and the rule itself through a test hook, because the first cut of that claim measured the OBSERVED side and failed two clips for doing what a real arm does when a hand sweeps across the body. 3 mutations caught', True),
    ('APPROACH TELL', ['node', 'gates/the_approach_is_the_tell_gate.js'],
     'DIRECTION judged the shipped hostiles at phone size and all three questions came back NO -- they do not read as dangerous, as Cartel, or at all: THEY ARE THE CROWD. Measured: watching, every body stood STILL on its corner, then at closeAt four TELEPORTED onto [-1,0][-1,1][1,0][0,1] and never moved again, all drawn set.idle -- since the residents started breathing the most static bodies on the street were the dangerous ones. Holds that a crew CLOSES THE GROUND ITSELF (measured as the gap they open on their own corner, because the first version of that claim was satisfied by the players own legs and passed with the approach deleted), never teleports, stays inside a leash shorter than closeAt so the crew cell stays the only answer to where the crew is, and that a moving body blits a WALK frame. 3 mutations caught', True),
    ...
    ('BEAT BUDGET',    ['node', 'gates/beat_budget_gate.js'],
     ...
     'at all and both answer "how much", never "of what". A frame rate tells you the bill; a '
     'sampling profile tells you who ran it up. WHAT THE FIRST RUN FOUND: walking the street '
     'spends 229 ms of every beat, and its five most expensive systems are (program), canvas '
     'blits, the DANGER+CREWS system at 7.3% (the biggest pure-JS cost on the street), canvas '
     'fills and the map grid. A FIGHT SPENDS 497 OF EVERY 500 MS -- 99.5% of the beat, no '
     'headroom at all, which is what a 17 fps fight looks like from the inside -- and 61% of '
     'everything it does is ONE CALL, drawImage. The fight is not thinking too hard, it is '
     'blitting too much. AND THE FIGHT IS ANIMATING BEHIND A HIDDEN PANEL BEFORE ANY FIGHT '
     'HAPPENS: the combat frame is created at boot, sits on a panel with display:none in a box '
     'measuring ZERO BY ZERO, and runs about 60 frames a second with ~900 drawImage calls a '
     'second into it, costing 3% of a core and 15 ms of every beat to draw something nobody '
     'can see. Found because a walk profile of a session that had never entered a fight '
     'contained drawField, a fight function. '
     'IT MEASURES A FIGHT SOMEBODY IS PLAYING, AND THAT WAS NOT ALWAYS TRUE (9/7). It used to '
     'wait for the cover camera to settle and profile what was left, which is not a fight, it is '
     'an idle screen with a fight on it -- and this lane published a flattering number off it: '
     '"a settled fighting beat is under 400 ms" when a DRIVEN fight was 347 to 428, median 386.8. '
     'The cover zoom eases 10% a frame toward a target set by how far the enemies are, so while '
     'anybody plays the camera is almost never still (309 and 599 distinct zooms in 28 seconds) '
     'and a camera-keyed cache holds 43% to 75% of frames rather than all of them. So THREE fight '
     'windows are taken now -- the opening, NOBODY PLAYING (labelled the ceiling), and BEING '
     'PLAYED (the controls driven, Chromium\'s own TaskDuration read so raster counts) -- and two '
     'checks stop the driven window quietly becoming another quiet one: it must produce more than '
     'twenty distinct camera zooms and more than five thousand samples, so a drive that stopped '
     'driving fails instead of passing. A gate that can only see a quiet fight flatters every '
     'change anybody ever makes to the fight, forever. '
     'THERE IS STILL NO BUDGET LINE FOR THE FIGHT AND THAT IS '
     'NOT AN OVERSIGHT: at 99.5% of the beat any ceiling is either above 100% and can never '
     'fail, or below today and red on arrival, and a gate red on arrival gets switched off by '
     'the next session that meets it -- so the number is PRINTED every run until the fight has '
     'headroom and a real line can be set. Saying "we could not hold this one yet" out loud '
     'beats a line that looks like coverage and is not. Two independent counters are compared '
     'every run (the sampler\'s own idle share and Chromium\'s TaskDuration, 0 to 1 point '
     'apart) because one counter for one fact is a number nobody can check, and every ceiling '
     'is paired with a floor so a profile that sampled nothing fails instead of passing. '
     'Scaled by the same CPU yardstick the speed gate uses. IT FIXES NOTHING AND CANNOT: every '
    ...
    ('DERIVED FRESH', ['node', 'gates/derived_freshness_gate.js'],
     ...
     'gate: derived_freshness_gate" -- this is that file, and until it existed the law was '
     'advertised and unenforced, the exact shape law_index_gate was built to make impossible. The '
     'law came from SIX incidents in one day, each found by a different lane, none of whom saw it '
     'as the same bug. IT FOUND FIVE MORE ON ITS FIRST RUN, three of them in files that SHIP: '
     'BOHEMIA_RUN_CURRENT.html is +938/-55 against what its own builder produces now (a 22 MB '
     'shipped slice); BOHEMIA_CURRENT_SLICE.html is +107/-11 and still carries the OLD quest canon, '
     '"SIDE QUEST S01 THE METER READER", where the source now says "ACT ONE ASK A01 THE KILLING '
     'SUMMER"; BOHEMIA_MAP_CURRENT.html is +92/-7 with engine md5 stamps that no longer match the '
     'files they name; the 8/15 surface audit says 6 things are on the walked surface when the '
     'truth is 22; and the reachability census is +20/-11. Nobody did anything wrong on purpose, '
     'which is the whole point: a bake is correct the day it is made and rots silently after. '
     'GOOD NEWS IN THE SAME RUN: 37 of 42 derived files re-derive BYTE FOR BYTE, the alpha Paolo '
     'taps among them. HOW IT IS SAFE: it regenerates in a throwaway git worktree in the scratch '
     'directory -- HEAD overlaid with the working tree\'s own changes, so it checks what is about '
     'to be PUSHED -- and the real checkout is never written to, proven by comparing git status '
     'before and after. The obvious "regenerate in place and git checkout -- to restore" would eat '
     'an uncommitted change the first time two lanes worked at once. DISCOVERY IS BY RUNNING, NOT '
     'BY READING: makers come from the "Generated by" headers the law asks for, but their OUTPUTS '
     'come from which files the run TOUCHED, because the file most likely to be stale is the one '
     'whose header nobody wrote -- and that paid immediately, since the 92-line-stale map slice '
     'declares no maker at all. ITS OWN FLOOR CAUGHT ITS FIRST MECHANISM: asking git what CHANGED '
     'made the 37 correct files invisible (a maker whose output is right leaves the tree clean), so '
     'it reported "1 of 6" where the truth was 37 of 42. Writes are found by MTIME now. RED ON: new '
     'drift not on the frozen list, a frozen entry that re-derives clean again (the list only '
     'shrinks), a pinned maker that is missing or crashes, an exemption for a maker nobody has, and '
     'an empty sweep of makers, comparisons or bytes. Five teeth proven to bite with throwaway '
     'edits, including a derived file made stale only in the working tree. 16 s, 9 passed / 0 '
     'failed. records/BOHEMIA_DERIVED_FRESHNESS.json lists every file and what it re-derives to',
     True),
    ...
    ('BUILD SIZE',     ['node', 'gates/build_size_gate.js'],
     ...
     'and a gate to hold it. WHAT THE FIRST RUN FOUND: the site we publish is 235.53 MB in '
     '642 files, and 73.00 MB of it in 445 FILES IS REACHABLE FROM NEITHER ENTRY POINT -- '
     'not a tab, not an iframe, not a link, not a fetch. A third of what we serve to the '
     'open internet cannot be opened from the game; it is old judge pages, galleries and '
     'proofs sitting in folders Pages publishes wholesale. IT DOES NOT RE-ASSERT THE TOTAL: '
     'PAGES PUBLISH has held the 260 MB ceiling on that number since 8/6, tied to the build '
     'timeout, and a second ceiling on one number at a different value is the drift this repo '
     'keeps paying for -- one fact, one owner. This gate reports the total, checks its own '
     'count agrees with the neighbour\'s, and asserts only what nothing else looks at. '
     'And the biggest single block in '
     'BOTH shipped files is COMBAT_B64, 1.73 MB of inline base64 downloaded before the '
     'first frame by everybody who opens either link, fight or no fight, while four other '
     'tabs in the same file already load their page from a sibling and pay none of it. '
     'IT ALSO CORRECTED THE ROW\'S OWN NUMBER: the alpha is 4.59 MB, not the 11 MB the '
     'board says, because the 8/2 payload-wall work already moved 35.76 MB of inlined city '
     'out to a sibling. A ONE-WAY RATCHET, DOWN ONLY, WHICH IS THE OPPOSITE OF THE CALL '
     'MADE FOR SPEED AND DELIBERATELY SO: a frame rate swings 40% between runs of an '
     'unchanged build, so a one-way ratchet there pins the budget to the luckiest afternoon '
     'and goes red on a game nobody touched; a byte count has no spread at all, so there is '
     'no headroom to leave and nothing to excuse. Every ceiling is paired with a floor so '
     'an empty scan fails instead of passing: a ceiling is trivially met by a sweep whose '
     'pattern stopped matching. IT REMOVES NOTHING AND CANNOT -- this lane may not touch '
     'slices/ content, so the record beside it is a hand-off list, and the row\'s "nothing '
     'removed without a record of what it was" is satisfied by that record existing FIRST',
     False),
    ...
    ('WEIGHTS SHAPE',  ['node', 'gates/weights_shape_gate.js'],
     ...
     "worth about five kindnesses, and that it is remembered longer and travels further. *** MEASURED "
     "FIRST AND THE 82-ROW DEED TABLE IS NOT A TABLE OF DEEDS: *** it is 83 rows now, and ALL 83 ARE "
     "SHAPED q:<quest>:<stage>@<FACTION>. Not one is a thing a person does on the street, and NOT ONE "
     "OF THE SEVEN KINDS THE STREET ACTUALLY PUBLISHES IS IN IT -- the standing organ calls that "
     "'unruled deed = weightless' in its own words, so the whole reputation machine is built, "
     "working, and grading nothing. *** AND THE CONTROL COST A TRY, AGAIN, AND AGAIN IT WAS THE "
     "INSTRUMENT: *** witness() takes EIGHT arguments and the first cut passed six, so `where` was "
     "undefined, nobody witnessed and it read zero either way. With the right call: no row gives 0 "
     "now and 0 at twenty-one days; a row of -2.5 gives -2.50, then -1.95, and the rung reads COLD. "
     "THE SHAPE: his ratio is the only number and the unit is ARGUED FROM HIS OWN RUNG LADDER (at 0.5 "
     "one kindness does not move a rung and one betrayal does); five kindnesses cancel one betrayal "
     "to the decimal; everything else weighs zero with a reason written on each one. TWO OF HIS FIVE "
     "HAVE NO DEED KIND AT ALL (stealing, and helping somebody who needed it) and are NAMED rather "
     "than given an invented one. 'Remembered longer' is FREE off the weight and this file computes "
     "no fade; 'travels further' is FALSE AND BACKWARDS, because the worst thing a person can do is "
     "not in the loudness table while throwing in with an outfit is the loudest thing on the street, "
     "and the proposal for it is carried and applied by nothing. RULE 18 HELD BY THE GATE: nothing "
     "calls it, the city's loudness table is untouched, the engine table still ships EMPTY, and 0 of "
     "his 83 rows are overwritten. Mutations, each put back: drop the ratio to two and invent a kind "
     "for stealing, 8 red; have the city mention the shape, 1 red.", False),
    ...
    ('COMBAT FLOOR', ['node', 'gates/combat_floor_gate.js'],
     'VAMILY [combat floor] / THE-AERIAL-VIEW-IS-THE-COMBAT-FLOOR: the zoomed-out city, '
     'handed to COMBAT as ground. His 9/4 tile law 3b is LOCKED and is the whole spec -- '
     '"the size of the ground changes but the player is the same size ... war is '
     'spilling in the streets type shit" -- and the law answers the HOW itself: '
     ...
     'this file draws NO city: the surface registers its own renderCity as a painter, the '
     'floor points it at a block, copies the frame out and puts the camera back. A second '
     'renderer would be byte-different from the streets he walked to get there, which is '
     'what ONE SEED forbids. A2 holds the ruling AS A RULE: a tile at or above the 112px '
     'sprite is REFUSED BY NAME, because a floor that zooms IN is not this ruling with a '
     'different number, it is the opposite of it. A3 states the picture as a NUMBER -- a '
     'figure stands 6.22 blocks wide at the city\'s own 18px tile, which is "figures on a '
     'war map" measured rather than asserted. B2 proves it is the city and not a picture: '
     'the same block twice is byte-identical while a block thirty away is not. B3 proves '
     'the camera comes back EXACTLY, because a camera left pointing at somebody else\'s '
     'fight outlives the frame that caused it. B4 states the UNIT out loud -- a cell here '
     'is a BLOCK, not a house (the law\'s other clause governs the house-scale board), so '
     '2,524 of 2,601 cells coming back solid is correct for blocks and would be nonsense '
     'for houses. MEASURED GREEN 13/0 on the walked surface and in the cut demo. '
     'Mutation-tested three ways: drop the scale rule (A2 red), leave the camera where '
     'the fight put it (B3 red), paint a flat fill instead of the city (B2 red, 1 colour). '
     'B3 DID NOT CATCH ITS MUTATION THE FIRST TIME and that is worth the line: the leg '
     'captured the camera as it happened to sit -- already on the walked cell at the '
     'default tile -- so deleting the restore changed nothing. It parks the camera '
     'somewhere distinctive first now, and asserts the park really was somewhere the '
     'floor had to move away from, or the leg is grading a claim against itself.', False),
    ('WHERE PEOPLE LIVE', ['node', 'gates/where_people_live_gate.js'],
     'VAMILY [more people] / POPULATION-DEFAULT, playtest dispatch item 5: "THE CITY '
     'SEEMS DEAD ASF AND I DONT LIKE THIS BEING THE DEFAULT I KNOW WE HAVE A SLIDER AND '
     'SHIT BUT YEAH MAN." MEASURED FIRST ON THE RUNNING DEMO, standing where he wakes, '
     'bodies actually blitted every hour from 05:00 to 23:00: ONE, all day -- his sentence '
     'confirmed. And it is NOT the count: 20 people live in that neighbourhood, the nearest '
     'is 64 cells away (seven screens), and the overmap cell he is standing on carries '
     '4,188 doorway cells with the NEAREST FRONT DOOR FOURTEEN CELLS AWAY. The suburb was '
     'full of houses and the game seated its residents on open ground with no relation to '
     'any of them, because homesIn() scattered evenly over all sixteen overmap cells while '
     'surveyNeighbourhood() had already worked out which six were suburb and threw the '
     'answer away -- measured headless, NINETEEN OF TWENTY loner residents were living off '
     'residential ground. A2 is the leg the job exists for: nobody sleeps on the freeway. '
     'A3 IS THE LINE THIS LANE MUST NOT CROSS -- the module carries a standing 8/28 warning '
     'that the next session must not turn the dial and call the job done, so A3 pins the '
     'dial at LANDMARK.story and the census at the ruled 297: everything here moved WHERE, '
     'never how many. A5 proves the doorstep pass FALLS BACK, so a district whose art has '
     'no doors seats exactly as many people as before, and A5b proves two passes are a '
     'preference and never a second helping. A6 IS A FINDING, NOT A CHECK: summing the '
     'module\'s own dialled heads gives 5,940 people on the surface he walks against the '
     '~69,000 its own comment claims for the same setting (that number was read off '
     'agents.js\'s plot model, a DIFFERENT population model about fourteen times bigger), '
     'so the default was set to 20 believing it bought the GDD\'s valley and the top of his '
     'slider is 9,504. Which number the valley is stays [PENDING Paolo]; A6 only stops the '
     'two drifting again unnoticed. MEASURED GREEN 15/0 on the walked surface and in the '
     'cut demo. Mutation-tested three ways: scatter over the whole square again (A2 red at '
     '1/20 and 3/20, B1 and C1 red at 54/61), stop asking for a doorstep (B2 red, 44 of 61 '
     'at a door falls to 7), and answer the doorstep question with a flat yes (B4 red at '
     '1,849 yes / 0 no).', False),
    ('PEOPLE GATHER', ['node', 'gates/people_gather_gate.js'],
     'VAMILY [more people] / POPULATION-DEFAULT round 2, "A BEARING IS A PLACE". Round 1 '
     '(AN ADDRESS IS A FRONT DOOR) put residents on residential ground at their own front '
     'doors and a walk still met nobody. MEASURED on the running demo across the 61 people '
     'in the nine neighbourhoods around where he wakes: AT HOME nearest neighbour 1 cell '
     'and 4 share a screen; OUT, at the busiest hour, nearest neighbour 14 cells and 3 '
     'share a screen. THE DAY IS WHAT EMPTIES THE STREET -- 41 of the 61 DO leave home at '
     '10:00 (this lane\'s own previous handoff said they never left the doorstep, off a '
     'stale comment and an out=0 that was only ever counted among the ONE body being '
     'drawn), and going out FOURTEEN-TIMED the gap between them: sixty-one private rays, '
     'sixty-one private destinations, none of them near anybody else\'s. The population '
     'module had already said what to do, on the workDir field: "a bearing and a distance, '
     'not a district name -- NAMING THE WORKPLACE IS THE SURFACE\'S JOB because only the '
     'surface knows what is actually there." The surface resolved the bearing to the most '
     'open cell on the ray, which is a COORDINATE; a place is a thing other people also go '
     'to. NOTHING HERE IS A NEW NUMBER: how many places a neighbourhood gets is the '
     'population module\'s own HEADS.cluster, the size it already calls a settlement (A1 '
     'holds that line); where they are is pplOpenness and pplDoorstep, the two instruments '
     'the surface already had; and a faction seat beats a street corner because BB-TURF put '
     'real markets on the map and the walked surface had never asked where they were. A2 '
     'proves the old ray still answers where there is no place, so ground the art has not '
     'filled in behaves exactly as before. HEADLINE, same protocol before and after (four '
     '400-step walks from the wake cell at 06:00, 10:00 and 18:00, counting only people NOT '
     'already on the glass): 0 of 12 walks met anybody BEFORE, 3 of 12 AFTER, median first '
     'meeting at step 157. Three of twelve is not "he meets people without trying" and the '
     'row stays OPEN. AND SENDING EVERYBODY TO A PLACE WAS TOO MUCH, WHICH ANOTHER LANE\'S '
     'GATE SAID FIRST: alive_gate went red on "it can happen inside a block", its closest '
     'meeting across 32 walks going from within 60 steps to 198 -- gathering everybody makes '
     'the ground BETWEEN places emptier, and the lone body you used to bump into is now '
     'standing in a square somewhere else. WHOSE DAY HAPPENS AT A PLACE IS ALREADY WRITTEN '
     'in bohemia_agents.js\'s four days and is not a ratio anybody chose: a WORKER goes to '
     'an off-block site job and a KEEPER runs one daily errand, so their day happens '
     'somewhere; a SCAV runs a subsistence sweep (two scavengers on one corner are '
     'competing) and a WATCH stands a post, and both are solitary by definition. The '
     'FAVOURITE spot stays a place for everybody, because Shadows of Doubt\'s favourite bar '
     'is a BAR. Measured after: workers and keepers 6 on a screenful and 1 cell apart, '
     'scavs and lookouts 2 on a screenful and 25 cells apart, the valley at home 4 and 1. '
     'And on alive_gate\'s own 32 walks -- a bigger, older sample than anything this round '
     'built -- 2 of 32 met a stranger BEFORE and 9 of 32 AFTER, closest meeting 198 steps '
     'back down to 58. BETTER THAN EITHER VERSION ON ITS OWN, and it came out of taking '
     'another lane\'s red seriously instead of arguing with it. MEASURED GREEN 14/0 on the '
     'walked surface and in the cut demo. Mutation-tested six ways: delete the places (B1 red, 4 share a screen instead of 13, and B4 red), '
     'delete the spacing rule (B3 red, closest pair anywhere 12 instead of 20), stop '
     'preferring frontage (B2 red, 0 of 12 places on a door), and ignore the person\'s '
     'bearing (A3 and B1b red, 8 bearings collapsing to 3 places), send the solitary trades '
     'to a place too (B1a red, 36 scavs and lookouts 1 cell apart), and send nobody\'s '
     'working day to a place (B1, B1b and B4 red, peak 3 bodies and only at 18:00). FOUR OF '
     'THOSE LEGS HAD '
     'TO BE REWRITTEN BECAUSE THEY WERE ACCIDENTALLY CORRECT: B3 asked the nine '
     'neighbourhoods round the wake cell (1-2 places each, far apart anyway), then the '
     'single busiest settlement (still green with the rule off, because the closest pair in '
     'the valley is not in the biggest settlement), before asking the minimum over all 704 '
     'places in 133 settlements; and B1b counted distinct CELLS, which cannot fail because '
     'the OCCUPANCY LAW rings a crowd around its spot, before it counted distinct PLACES. '
     'A LEG THAT ASKS THE CASE THAT SOUNDS HARDEST IS STILL GUESSING. And B1 itself averaged '
     'the whole valley and demanded that going out tighten EVERYBODY, so it went red the '
     'moment the solitary trades were let walk alone -- the change that took alive_gate from '
     '2 meetings to 9. AN AVERAGE OVER TWO POPULATIONS THAT ARE MEANT TO DIFFER MEASURES '
     'NEITHER OF THEM; it is two legs now and both directions of the split are '
     'mutation-proved.', False),
    ('MAP KNOWS PEOPLE', ['node', 'gates/map_knows_people_gate.js'],
     'VAMILY [more people] / POPULATION-DEFAULT round 3. Rounds 1 and 2 put residents at '
     'their front doors and gave a neighbourhood places to gather, and a walk still only '
     'meets somebody 9 times in 32 -- a crowd is real when you are standing in it and '
     'invisible when you are not. What was left was FINDING. MEASURED FIRST, AND THE MAP '
     'HAD NEVER ASKED: renderCity() does not call headsAt(), census() or pplPeople() once, '
     'so you could open the map of the whole valley and nothing on it said a single person '
     'lived anywhere -- the exact sentence this file already carries about factions ("you '
     'could open the map of the whole valley and nothing on it said that anybody held any '
     'of it"), with people the half nobody came back for. AND THE NUMBER THAT MAKES IT '
     'WORTH DRAWING: HE WAKES ONE KILOMETRE FROM A SETTLEMENT OF 220 PEOPLE, eight overmap '
     'cells, a twenty-minute walk on the roads BB-ROADS-ARE-FAST made fast, and the game '
     'had no way to tell him. One mark per settlement, thirteen on a 96x96 valley, off a '
     'flat array built once with the valley exactly as turfGrid() is. The rules are the '
     'map\'s own, taken from the territory pass rather than invented: MAP LAW nothing is '
     'placed (A1), NOT A HUD PIN so it is never on the walking screen (B5), no fill over '
     'the ground, and no faction hue because COLOUR IS TERRITORY reserves those for Paolo. '
     'TWO THINGS THE REAL SCREEN KILLED AND NEITHER WAS VISIBLE FROM THE CODE: the first '
     'cut also marked all 139 neighbourhoods holding anybody, which is clutter over the '
     'streets the map is for AND a small pale diamond indistinguishable from the gold '
     'diamonds the map already draws for every landmark -- a mark that reads as a different '
     'mark is worse than no mark; and it measured 2.41:1 against its own ground, under the '
     '3:1 floor for a graphic, in the same round EYES measured 38 pieces of text too faint '
     'to read. It is 10.5:1 now. MEASURED GREEN 12/0 on the walked surface and in the cut '
     'demo. Mutation-tested three ways: mark every household instead of the settlements '
     '(B2b red, 69 drawn against 6), draw the mark faint (B3 red at 1.58:1), and return no '
     'grid at all (B1, B2, B2b, B3, B4 and C1 red). B2b HAD TO BE ADDED AFTER THE FIRST '
     'MUTATION RUN: loosening the mark back to every household went UNCAUGHT by every other '
     'leg, because "is it drawn" and "is it bright" say nothing about WHAT is drawn.', False),
    ('HEAR THE CROWD', ['node', 'gates/hear_the_crowd_gate.js'],
     'VAMILY [more people] / POPULATION-DEFAULT round 4. Round 3 put the crowds on the MAP, '
     'which is a thing he has to think to open; NOTHING HAS EVER TOLD HIM A CROWD IS THERE '
     'WHILE HE IS WALKING, and the walked status line has been the hard-coded string '
     '"walking your own block." since it was written. The population module\'s own phrase '
     'for what this should feel like is "you hear a settlement before you see it". THE '
     'RANGE IS DERIVED, NOT PICKED (A1), because this lane may not invent numbers about '
     ...
     'cells, BohemiaStanding) -- which makes the phrase literally true, heard further than '
     'seen. MEASURED BEFORE IT WAS BUILT, because a mechanism that is right and never '
     'happens is not a deliverable: the biggest crowd standing at one place anywhere is 18 '
     'people, buying it 38 cells or about four screens; at the cell he wakes on it says '
     'NOTHING at any hour, correctly, because the nearest place is 97 cells off and holds '
     'eight; and of four 400-step walks from there at 10:00, two come within earshot, first '
     'at step 91 and 156. HE HAS TO WALK TOWARD IT, and B4 is the leg that keeps that '
     'honest: THIS IS NOT A COMPASS. A3 holds that the one you hear is the LOUDEST and not '
     'the nearest, because a big crowd further off carries over a pair of voices next door. '
     'B3 holds that the day has a shape in the ear -- quiet at 02:00 and 22:00, heard at '
     '10:00 and 18:00 -- so the street is a valley and not a spawner. WORDS, NOT AN ARROW, '
     'which is this repo\'s own ruling from the address gate ("Bohemia is a city whose '
     'phones do not work, so a compass that always knows where everybody is would be the '
     'strangest object in it"); the line is an attempt, draft:true. B5 states the cost the '
     'way this lane\'s own [draw budget] row demands: 0.044 ms when it recomputes, 0.0002 '
     'ms cached, 0.01% of a 500 ms beat, cached per cell per ten minutes of game time. '
     'MEASURED GREEN 9/0 in the cut demo. Mutation-tested three ways: make earshot infinite '
     'so it becomes a compass (A1 and B4 red, it speaks at the wake cell at three hours), '
     'compute the line and never write it to the glass (A2 and B2 red -- the authored-but-'
     'unread disease), and hear people where they SLEEP rather than where they are (B1, B2 '
     'and B3 red, the whole day goes quiet).', False),
    ('ON THE WAY', ['node', 'gates/on_the_way_gate.js'],
     'VAMILY [more people] / POPULATION-DEFAULT round 5. Rounds 1-4 put people in the '
     'houses, gathered their days at places, marked the crowds on the map and made them '
     'audible. THEY STILL TELEPORTED. Measured on the demo minute by minute through one '
     'morning, with a jump defined as MOVING FASTER THAN THE WORLD WALKS (MIN_PER_CELL, '
     'the surface\'s own 0.084 minutes a cell, about twelve cells a minute): 34 journeys '
     'finished in under a minute, THE BIGGEST 477 CELLS -- half a kilometre in sixty '
     'seconds -- and NOBODY was ever visibly on the way, at any hour, all day. The '
     'schedule already has rush hours (16 of 61 change place at 08:00, 14 at 15:00, '
     'TWENTY-EIGHT at 17:00, 16 at 19:00) and not one of those journeys had ever happened '
     'on a street. NOTHING HERE IS A NUMBER THIS LANE PICKED (A1, A2): a journey takes '
     'distance x MIN_PER_CELL, the same cost the PLAYER pays to cross a cell, and when '
     'they set off is the schedule\'s own block boundary with the block before it saying '
     'where from. B3 holds that a walker stands on ground a person can stand on -- the '
     'straight line between two places runs through buildings, a two-cell nudge was not '
     'enough (68 journeys still snapped), and the fix is to step BACK ALONG THEIR OWN '
     'ROUTE, which is what anybody going round a building does; a body inside a wall '
     'would be worse than a body that teleports. *** AND B4 IS THE HONEST HEADLINE: THIS '
     'DOES NOT MOVE THE SHIP TEST. *** Twelve walks from the wake cell at 08:00, 12:00 '
     'and 17:00, before and after: THREE OF TWELVE MET SOMEBODY, BOTH TIMES. What changed '
     'is that seven people are visibly walking at 08:30 where the answer used to be an '
     'empty list at every hour, and teleports fell from 34 to 26 with the biggest from 477 '
     'cells to 400. A DEFECT IN THE WORLD\'S HONESTY FIXED, NOT A MEETING RATE IMPROVED, '
     'and a gate that let those two be confused would be doing the confusing. B5 states '
     'the cost, because pplAt runs for every drawn body every frame: 0.07 ms to place all '
     '61 at rush hour, 0.01% of a beat. MEASURED GREEN 8/0 in the cut demo. '
     'Mutation-tested two ways: remove the commute so they teleport again (B1 red at 34 '
     'and 477, B2 red at a peak of 0), and let walkers stand inside walls (B3 red, 9 hours '
     'with a body off walkable ground). AND THE PROBE ITSELF HAD TO BE FIXED FIRST: its '
     'first cut called anything over TWO cells a minute a teleport, which is six times '
    ...
    ('THE FACE ON THE MAP IS HIS', ['node', 'gates/the_face_on_the_map_is_his_gate.js'],
     'LIFE + CITY, row [see me], un-held under rule 18f because it is PAOLO 9/22 asking: "why '
     'do I not see my person on this map? ... what are the peach colored dots". TWO THINGS, AND '
     'THE FIRST IS NOT WHAT THE ROW SAID. The mark was ALREADY the loudest thing on the glass -- '
     'that was [white rings] and its numbers still hold, 86 bright pixels against a town\'s 74 -- '
     'so "I do not see my person" was never about finding it. WHAT IT CARRIED IN THE MIDDLE WAS A '
     'FLAT DISC OF SKIN COLOUR, and a brown dot is not a person. *** AND THE ROW ASKED FOR THE '
     'WRONG FACE: it says faceFor on his id, faceFor ROLLS a face out of a hash, and the head the '
     'pad wears is buildSpec(), the face he BUILT. *** Leg C measures that they are different '
     'people (his own bake 10,584 bytes, the rolled one 8,944, not equal), so the row\'s wording '
     'would have put a stranger on his map wearing his own mark: the ONE ID ONE WHOLE PERSON '
     'defect. He is asked for by the reserved id "you" and the bridge hands back his own spec '
     'through the same ramp, renderFace, packIdx and decoder; nothing new draws a face. AND THE '
     'SIZE IS ARITHMETIC, NOT TASTE: the white annulus is what makes him bright, a face at 0.66 '
     'of the head eats it, and at TW=48 the first cut went 684 -> 533 bright pixels and STILL '
     'PASSED only because no town happened to be on screen there -- a pass that depends on what '
     'happens to be on screen is not a pass. The head now grows by 1/sqrt(1-f*f), which puts the '
     'annulus back exactly; measured after, he goes 260 -> 375 against a town\'s 236 at TW=30 and '
     '684 -> 935 at TW=48, so he got LOUDER, and inside his head there are 30 and 53 colours '
     'where a disc is 1. Under a 14 px face he stays a clean disc rather than four muddy pixels. '
    ...
    ('NO YARD IS SEALED', ['node', 'gates/no_yard_is_sealed_gate.js'],
     ...
     'RUN 930e2bf3 handed this lane "45 doorsteps with 0 straight walkable ways out; the '
     'suburb generator seals yards behind one-cell walls", and the suburb generator is this '
     'lane\'s. The 45/0 number is a RAY cast in eight directions across 140 cells, and a ray '
     'reports LINE OF SIGHT: on a suburb block full of houses the street almost never is in '
     'line of sight, which is what a suburb IS, not a defect. MEASURED ON THE CUT, both '
     'instruments side by side from his own doorsteps: STRAIGHT ways out 7 of 68, WALKABLE '
     'ways out 68 OF 68, proved sealed 0. Then the whole block he wakes on -- every walkable '
     'cell touching one a body cannot stand on, a superset of every real doorstep there is: '
     '1,857 doorstep-shaped cells, 0 that one flood out of his own door misses, 0 sealed, 0 '
     'unknown. THE SUBURB GENERATOR DOES NOT SEAL YARDS. Leg B runs one flood first and then '
     'spends the unbounded three-state test only on what that flood missed, so the honest '
     'instrument runs where it matters instead of 16,384 times. A GATE THAT ONLY EVER SAYS '
     'ZERO IS NOT A GATE, so LEG A SEALS A YARD ON PURPOSE on worlds with known answers: a '
     'yard fenced on all four sides is PROVED SEPARATE in 119 cells looked at; one cell of '
     'side gate and it walks out; a way out that is only a diagonal squeeze is still a way '
     'out; an endless wall in an endless world comes back UNKNOWN, never SEALED; and A4 '
     'holds the finding itself, that a RAY calls the yard WITH the gate in it sealed too. '
     'The first cut of leg A\'s own world was not actually sealed (open at the top either '
     'side of the house) and the gate said SAME GROUND and was right, which is why the world '
     'is built the way it is. Mutation: wall a ring round a real yard on the block and B3 '
     'reds. Fourth instrument-not-world finding from this lane in a row, after 9/15, 9/16 '
     'and 9/20.', True),
    ...
    ('A CROWD TO WALK INTO', ['node', 'gates/a_crowd_to_walk_into_gate.js'],
     ...
     'somebody, but the most he ever saw AT ONCE was 2, because one borrowed body per '
     'screenful is exactly what that lattice promises. AND THE FIRST MEASUREMENT SAID THE '
     'CROWDS WERE ALREADY BUILT: standing on residential ground anywhere the peak was 2 (86 '
     'of 160 standings saw nobody), while standing ON a gathering place it was 17, 14, 11, '
     '10, 9, 8, 8, 8. Round 2\'s places work -- HE JUST NEVER ENDS UP ON ONE, because a place '
     'is a single cell in a valley of millions. TWO CUTS THEN FAILED THE SAME WAY AND THE '
     'WALK CAUGHT BOTH: filling the nearest real place changed nothing (the places are a '
     'hundred cells off and the walking budget buys fifty-nine), and dropping the budget '
     'doubled the crowd standing at a place from 17 to 37 while a walk still met 0 crowds in '
     '16. A MECHANISM THAT IS RIGHT AND NEVER HAPPENS IS NOT A DELIVERABLE, and standing on '
     'sampled ground is not how anybody plays. WHAT SHIPPED: the crowd forms on the best '
     'FRONTAGE within TWO SCREENFULS of him -- a screenful being the repo\'s own 2 x '
     'SEE_RANGE + 1, the same unit the floor lattice uses -- and never inside one SEE_RANGE, '
     'so nobody forms up inside his own view; how many make a crowd is the module\'s own '
     'HEADS.cluster, capped at half the borrowed people so the floor survives. AFTER: walks '
     'that met a crowd 0 of 16 -> 5 of 16, biggest group seen 3 -> 14, walks that met '
     'anybody still 16 of 16, empty standings unchanged at 65 of 120. MEASURED GREEN 7/0 in '
     'the cut demo, AND IT WALKS, because two cuts of this round passed a standing test while '
     'a walk met nothing. Mutation-tested two ways: remove the knot (B1 and B2 red, 0 of 16, '
     'biggest 3), and let the crowd stand anywhere within the walking budget (A2, B1, B2 red, '
     'reproducing the failed cut exactly). B4 holds the other end: no crowd stands at three '
     'in the morning, because it can still only borrow people the schedule already has out.', False),
    ('THE ONE THAT IS YOU', ['node', 'gates/the_one_that_is_you_gate.js'],
     'VAMILY [white rings] / WHAT-ARE-THE-FOUR-WHITE-RINGS, from HIS OWN FRAME 9/8: four '
     'white rings on the city at once and one of them is supposed to be him. THEY WERE THIS '
     'LANE\'S OWN -- three are the SETTLEMENT RINGS shipped in round 3 of [more people], '
     'whose comment promises "NOT A HUD PIN, it does not follow him". They keep that promise '
     'and still caused this, because nobody checked them against the mark they sit beside. '
     'MEASURED ON THE ALPHA BEFORE THE FIX, at the zoom his frame was taken at: 4 settlement '
     'rings at radius 10.2 px and the mark that means YOU at radius 5.0 -- THE MARKS FOR "A '
     'TOWN IS HERE" WERE TWICE THE SIZE OF THE MARK FOR "YOU ARE HERE", and worse with zoom, '
     'because his mark was a FIXED 5 px at every zoom while the rings scale with the tile. '
     'THE FIX IS SHAPE, NOT BRIGHTNESS: dimming the rings would fight round 3\'s measured '
     'contrast floor, so the rings are untouched (A1) and the player becomes a PIN -- a stem '
     'and a foot on the ground, his own colour in the middle where a ring carries a dark '
     'hole. AFTER: 86 bright pixels against a town\'s 74, 260 against 236, and 212 apart in '
     'rgb at the centre. THIS GATE MEASURES RENDERED PIXELS, not the source, because two '
     'marks can be written differently and still look identical on the glass -- and looking '
     'identical on the glass is the whole complaint. ITS OWN FIRST CUT COULD NOT FAIL: it '
     'computed the player\'s size from its own copy of the drawing formula, so the mutation '
     'run put the old 5 px disc back and the size legs never moved; they count bright pixels '
     'in an identical box round each mark now. AND THE HONEST MEASUREMENT THEN CAUGHT THE '
     'FIX -- by height the pin looked done, by pixels it was still 106 against a town\'s 236, '
     'because a town is a FILLED disc and a pin was a thin ring on a stem. HEIGHT IS NOT '
     'LOUDNESS. MEASURED GREEN 7/0. Mutation-tested two ways: restore the fixed 5 px disc '
     '(A2, B1, B3 red, B3 printing 52 -> 52 -> 52, the old defect exactly), and give him a '
     'town\'s dark centre (B2 red at 0 apart in rgb). THE STANDING NOTE: A MARK IS ONLY EVER '
     'RIGHT NEXT TO THE OTHER MARKS -- round 3 measured its ring against the GROUND, cleared '
     'the floor, and shipped something correct in isolation that broke the one thing on the '
     'screen a player must never lose.', False),
    ('THE TAP PICKS WHAT YOU SEE', ['node', 'gates/tap_picks_gate.js'],
     'VAMILY [tap picks] / THE-TAP-PICKS-THE-TILE-BELOW, from PAOLO\'S OWN FRAME 9/8: "when I '
     'click a tile in city builder mode it\'s not the tile it\'s sitting on, it\'s like below, '
     'very awkward." THE ROUTING RULING NAMED TWO CAUSES AND BOTH WERE WRONG -- it cited a '
     'draw-time LIFT of 1.6, which is FORCE_POVERTY_LIFT, an economics constant, and page '
     'pixels not being scaled to canvas pixels, which toCv() has always done -- so this gate '
     'measures instead of inheriting. MEASURED WITH DRIVEN TAPS BEFORE THE FIX: tapping a '
     'tile\'s GROUND CENTRE 25 of 25 exact at every zoom, tapping THE BUILDING YOU CAN SEE 0 '
     'of 38. The picker inverted the ground plane, which is exact arithmetic, and the eye is '
     'on the ART, which is painted above its own footprint; the two agree only on flat ground, '
     'so a road always worked and a tower never did. AND THE PRISM PATH WAS A DEAD END: the '
     'first cut taught the picker about prism(), the procedural block fallback, and measured '
     'ZERO PRISM CALLS AT EVERY ZOOM -- the valley is painted with baked hero art, so that fix '
     'would have gone green on a body the renderer never puts on the glass. THE FIX: the '
     'renderer records the rectangle it blits and the picker asks the art\'s own pixels, '
     'taking the tile painted LAST. THE PART THAT TOOK FOUR ATTEMPTS: a ROAD plate is opaque '
     'well outside its own diamond, so "whose paint did you touch" handed a tap on open tarmac '
     'to the plot in front (caught by looking at a screenshot, not by a number). Only art that '
     'STANDS UP may take a tap off the ground, and THE CUT POINT WAS READ OFF THE ART: measured '
     'at TW=48, ground runs 1.05 to 1.23 tiles of opaque height (desert, wash, rail, water, '
     'freeway, farm, suburb, arterial) and buildings start at 1.54 (school, strip, commercial, '
     'chapel, apartment, campus, courthouse, resort), with NOTHING IN THE GAP -- so the number '
     'inside it changes nothing, which is what separates a measured threshold from a tuned one. '
     'MEASURED GREEN 6/0 on the alpha at 390x844: buildings 38 of 40 across three zooms, open '
     'ground 36 of 36. ONLY A DRIVEN TAP COUNTS (the pad: an in-page hit test said 12 of 12 '
     'while real taps landed 2 of 11). Mutation-tested two ways, each caught by a different '
     'leg: restore the ground-plane picker (B1 and B3 red, buildings 0 of 40), and drop the '
     'standing-up rule so flat art steals taps (A2 and B2 red, open ground 0 of 36).', False),
    ('THE STREET IS NEVER EMPTY', ['node', 'gates/never_empty_gate.js'],
     'VAMILY [more people] POPULATION-DEFAULT round 7, and the first round of that row with '
     'a RULING behind it. The coordinator decided 9/7 that the ship test is met by DENSITY, '
     'not headcount: the valley keeps its canon count and the mechanism places people where '
     'the player IS, spawned near and released far, with the default that the street a player '
     'is on is never empty in daylight. MEASURED FIRST, standing in FORTY residential places '
     'across the valley at three daylight hours: 116 of 120 standings with NOBODY in sight '
     '(96.7%), the most anyone ever saw was ONE, median ZERO. AND THE NUMBER THAT ENDED THE '
     'PLACEMENT ERA: asked of the schedule at 10:00, 763 of 1,140 people ARE OUTSIDE AND YOU '
     'STILL SEE NOBODY -- sight covers 361 cells of a 12,800,000-cell window, so a standing '
     'contains somebody 2% of the time, and the measurement found 4 in 120. Round 6 divided '
     'and found placement could not close a 10.7x gap; this is the same division one layer '
     'down. AFTER: 44 of 120 (36.7%), median 1. A NEAR FIELD IS A SPAWNER UNLESS SOMETHING '
     'STOPS IT BEING ONE, so four legs are about cheating rather than the feature: B3 every '
     'borrowed body is somebody their own schedule already had outdoors (checked at 10:00 AND '
     '03:00 so a quiet world cannot pass it by accident), B4 the census does not move, B5 '
     'nobody is in two places, B6 the OCCUPANCY LAW with a borrowed/local breakdown. The '
     'lattice spacing is 2 x SEE_RANGE + 1 read from BohemiaStanding, not typed (A1), and the '
     'one real budget -- five minutes\' walk at MIN_PER_CELL -- is NAMED as a budget (A2). '
     'MEASURED GREEN 11/0 in the cut demo. Mutation-tested three ways: switch the near field '
     'off (B1/B2/B5 red, B1 prints 116 of 120 again), let it borrow indoor people (B3 and B3b '
     'red, 26 bodies at three in the morning), stop looking at the locals (STAYED GREEN, which '
     'is what deleted that code). B3\'s first cut demanded an EMPTY street at 03:00 and caught '
     'the night watchman -- the world was right and the leg was wrong.', False),
    ('BUILD DEFAULT', ['node', 'gates/build_default_gate.js'],
     'VAMILY [build default] / THE-FIRST-BUILDING-A-PLAYER-PLACES-IS-AN-AIRBASE, harvested '
     'from ECONOMY round 4 by PLAYING the game rather than grepping it: "59 district types '
     'in ALPHABETICAL ORDER, no default, no grouping, no placeholder, so the browser picks '
     'the first and BUILD places an airbase. This is the worst first-hour moment in the '
     'game." MEASURED BEFORE TOUCHING IT on the cut demo: 59 options, ZERO groups, the '
     'select reads AIRBASE, first three labels AIRBASE / AIRPORT / APARTMENT. Every word '
     'true. AND THE DEFECT WAS IN FOUR PLACES, NOT ONE: the price line, the affordability '
     'check and the build itself all fell back to buildableTypes()[0] when the panel was '
     'shut, and [0] is alphabetical -- the same wrong answer in four separate copies, now '
     'one cbDefaultType() (A3, B4). WHAT THE REPO ACTUALLY KNOWS ABOUT THESE 59 THINGS IS '
     'ONE FACT: BohemiaPopulation.RESIDENTIAL names the six that are homes. Nothing says '
     'what any other district is FOR, and WHICH TYPES MAKE POWER OR CLOUT IS [PENDING '
     'Paolo] by this lane\'s own state line, so A2 holds the grouping to exactly as deep '
     'as the repo\'s knowledge -- HOMES and EVERYTHING ELSE, with NO invented POWER group. '
     'B1b is the leg that had to be added after the first mutation run: grouping ALONE '
     'already stops the airbase (the first home alphabetically is APARTMENT), so deleting '
     'the explicit default left every leg green -- which makes SUBURB a DECISION rather '
     'than a side effect, and the decision is that the sensible first building is the '
     'ground he wakes standing on. A DECISION NO LEG STATES IS A DECISION THAT DRIFTS. AND '
     'A FINDING ON THE WAY: every home holds EXACTLY 2.2 -- an apartment houses what a '
     'trailer houses -- which is HOUSEHOLD_MEAN applied flat, is a [PENDING Paolo] this '
     'lane raised before, and is why the group label carries the number ONCE instead of '
     'six identical numbers pretending to be a choice. MEASURED GREEN 9/0 in the cut demo. '
     'Mutation-tested three ways: remove the explicit default (B1b red, it falls to '
     'APARTMENT), flatten the two groups back into one alphabetical list (A2, B2 and B3 '
     'red, first three back to AIRBASE / AIRPORT / APARTMENT), and put the fallback back '
     'to buildableTypes()[0] (B4 red at "airbase").', False),
