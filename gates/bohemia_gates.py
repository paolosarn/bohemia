...
GATES = [
    ...
    ('RF4 TEARDOWN',   ['node', 'gates/rf4_teardown_gate.js'],
     "Paolo 8/17: \"For combat, I completely just want to. I really need you to re-create rogue fable four holy shit please.\" And 8/16, LOCKED: \"the reference lab and the combat chats are going to be working together to make a live recreation of Rogue Fable 4 for our game bar none. idc if its a rip off.\" That law puts TWO CHATS ON ONE SYSTEM and names the seam as a FILE so they cannot collide: LAB owns the teardown spec and the measured diff and WRITES NO COMBAT CODE, COMBAT owns the implementation and the STATUS column, neither lane edits the other's. This gate holds that seam. The spec is 28 numbered items across the frame, the trinity, the ability economy, geometry, enemy design, structure and RF4's deliberate omissions, each ending in SPECED or BUILT or DIFFERS-ON-PURPOSE, and every BOHEMIA-TODAY number is RE-DERIVED off the running fight by tools/bohemia_rf4_teardown_measure.js so no status can be justified by a sentence somebody typed. MEASURED: 320 bodies across 40 arenas, and EIGHT OF RF4's PILLARS ARE ALREADY BUILT -- the cover and line-of-fire model, the environment genuinely fighting back (cover chews away under fire, cars cook off, decks and stairs, darkness), the field readouts, the ranges, target selection, the way out that made movement the win condition on 8/16, and armor 0 on all 320 bodies so there is no stat mitigation to hide behind. THE BIG DIVERGENCE IS ENCOUNTER SIZE: 8.0 per fight, min 8, max 8, and 0 OF 40 inside Wang's 3-6 band. The roster is deep (5 types, 8 HP tiers, 40 elites) and the encounter is flat, so the depth never surfaces. AND EIGHT IS NOT A RULING, checked before accusing anyone: the 6/27 addendum uses eight as the STRESS CASE (perfect play = zero damage at any enemy count, ONE ENEMY OR EIGHT) and the 6/30 DNA doc already adopted small fights as an explicit design axis, so the ceiling shipped as the constant. Because A GATE MUST NEVER OUTRANK A RULING this gate does NOT demand 3-6; it demands the divergence stay measured and DECLARED, and it goes red when COMBAT lands the encounter curve so the spec is rewritten rather than quietly becoming false. It also enforces two kinds of honesty that matter because COMBAT builds off this file: G3 fails if LAB's own diff touches any engine module or slice, and F2/F3 fail if the spec stops admitting that RF4-05, RF4-08 and RF4-10 -- the trinity and the ability economy, the heart of the recreation -- are SECONDHAND because every primary source domain is egress-blocked from this environment, so COMBAT must not implement a number off them. A lead dressed as a fact is how two chats on one system go wrong.", False),
