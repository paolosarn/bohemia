...
const path=require('path');const P=n=>path.join(__dirname,n);
const read=n=>fs.existsSync(P(n))?fs.readFileSync(P(n),'utf8').toLowerCase():null;
...
for(const v of [2,3,4,5]){
  ok('GDD v'+v+' is live at root', V[v]!==null);
  ok('GDD v'+v+' is not archived', !fs.existsSync(P('../laws/ARCHIVE_BOHEMIA_GDD_v'+v+'.md'))&&!fs.existsSync(P('../archive/BOHEMIA_GDD_v'+v+'.md')));
}
...
const reg=read('bohemia_superseded.txt')||'';
