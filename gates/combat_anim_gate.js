...
/* ---- 7. FLEE/SURRENDER-RUN (round 3, Paolo's animation-session prompt,
   records/BOHEMIA_COMBAT_ANIM_REQUESTS_2_7_20_26.txt): a downed-nerve enemy
   who breaks and runs (V35 fleeing) had NO distinct visual -- enemyFrame
   fell back to the plain walk cycle, which never reads as "giving up."
   Two baked variants (rolled once per break, e._fleeVar): panic sprint
   (both hands thrown high, full-tilt run, forward pitch) and stumbling
   scramble (hands half-up, off-balance, a periodic backward glance-over-
   the-shoulder snap). Structure matches crawl-dying (round 2B, c492da0)
   exactly per the ask: same CANDIDATE_SRC/CAND_BEATS shape, same bake-site
   convention, verified visually (headless Playwright, all 8 facings + the
   full phase cycle) before shipping, not just these numeric checks. ---- */
for (const c of ['flee-sprint', 'flee-scramble']) {
  ok('CANDIDATE_SRC carries ' + c, !!alpha.match(new RegExp("'" + c + "':\"((?:[^\"\\\\]|\\\\.)*)\"")));
  ok('CAND_BEATS carries ' + c, new RegExp("'" + c + "':4").test(alpha));
}
{
  const sm = alpha.match(/'flee-sprint':"((?:[^"\\]|\\.)*)"/);
  const cm2 = alpha.match(/'flee-scramble':"((?:[^"\\]|\\.)*)"/);
  let sprint = null, scramble = null;
  try { sprint = eval('(' + sm[1].replace(/\\"/g, '"') + ')'); scramble = eval('(' + cm2[1].replace(/\\"/g, '"') + ')'); }
  catch (e) { ok('flee clips eval', false, e.message); }
  if (sprint && scramble) {
    const dirs = ['E', 'S', 'SE'];
    const sSamples = dirs.flatMap(d => [0, 0.25, 0.5, 0.75].map(ph => sprint(d, ph)));
    ok('flee-sprint: both hands thrown high, every facing every phase (surrender-while-running reads instantly)',
      sSamples.every(o => Math.abs(o.upL || 0) > 2 && Math.abs(o.upR || 0) > 2));
    ok('flee-sprint: the legs actually stride (a real run gait, not a stand-and-wave)',
      dirs.filter(d => !['N', 'S'].includes(d)).some(d => {
        const a = sprint(d, 0.25), b = sprint(d, 0.75);
        return Math.abs((a.thighR || 0) - (b.thighR || 0)) > 0.5;
      }));
    ok('flee-sprint: body pitched forward harder than the base run gait (0.16 vs run\'s 0.10)',
      sprint('E', 0.1).spine === 0.16);
    const cSamples = dirs.flatMap(d => [0, 0.25, 0.5, 0.75].map(ph => scramble(d, ph)));
    ok('flee-scramble: hands stay HALF up, visibly lower than the sprint\'s full-raise (off-balance, not a clean surrender)',
      cSamples.every(o => Math.abs(o.upL || 0) < 2 && Math.abs(o.upL || 0) > 0.5));
    ok('flee-scramble: a real backward glance-over-the-shoulder snap exists in the loop (spine/head go NEGATIVE against the forward pitch)',
      dirs.some(d => Math.min(...[0.6, 0.7, 0.75, 0.8].map(ph => scramble(d, ph).spine)) < -0.05));
    ok('flee-scramble: the two variants are mechanically distinct (different arm magnitude, sprint always exceeds scramble)',
      dirs.every(d => Math.abs(sprint(d, 0.1).upL) > Math.abs(scramble(d, 0.1).upL)));
  }
}
ok('bake: L.look.flee is the two variants, same array-of-arrays shape as death, 4 phases each',
  /L\.look\.flee=\[\[0,0\.25,0\.5,0\.75\]\.map\(p=>bake112\(L\.d,'flee-sprint',p\)\),\[0,0\.25,0\.5,0\.75\]\.map\(p=>bake112\(L\.d,'flee-scramble',p\)\)\]/.test(alpha));
ok('receiver maps L.flee into flee112, same array-of-arrays unpack as death', /flee112:L\.flee\?L\.flee\.map\(v=>v\.map\(b=>mkAt\(b,112,112\)\)\):null/.test(demo));
ok('the flee variant is rolled once per break (e._fleeVar), not re-rolled every frame',
  /e\.fleeing=true; e\._fleeAt=performance\.now\(\); e\._fleeVar=Math\.floor\(Math\.random\(\)\*2\)/.test(demo));
ok('enemyFrame: fleeing plays the baked flee clip (variant-indexed), not the plain walk cycle',
  /if\(e\.fleeing\)\{ const fv=L\.flee112&&L\.flee112\[e\._fleeVar\|\|0\]; if\(fv&&fv\.length\)return fv\[Math\.floor\(now\/150\)%fv\.length\]/.test(ef));
ok('enemyFrame: still falls back to the plain walk if the clip never baked (never a hard crash)',
  /if\(L\.walk112&&L\.walk112\.length\)return L\.walk112\[Math\.floor\(now\/170\)%L\.walk112\.length\]; return L\.idle112; \}/.test(ef));
