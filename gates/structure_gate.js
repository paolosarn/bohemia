...
if (G) {
  ...
  // MARATHON WAVE 2 (7/25/26): three genuinely new shapes, not recolors --
  //   GORGET     a throat-guard ring, zone-locked to neck + the collar lip
  //              (a different zone from every other gear kind)
  //   HALF APRON opt.bibless drops the chest bib -- shorter, no torso coverage
  //   SHORTALLS  opt.short cuts the legs at the knee -- no feet/ankle cuffs
  let gor = null, apBib = null, apHalf = null, coBib = null, coShort = null;
  try {
    gor = G.genGear(g, { ramp: R, kind: 'gorget' });
    apBib = G.genApron(g, { ramp: R }); apHalf = G.genApron(g, { ramp: R, bibless: true });
  } catch (e) { console.log('  wave2-gear/apron err: ' + e.message); }
  try {
    const W2 = makeW6('S');
    coBib = W2.genCoverall(g, { ramp: R, bib: true }); coShort = W2.genCoverall(g, { ramp: R, bib: true, short: true });
  } catch (e) { console.log('  wave2-coverall err: ' + e.message); }
  if (gor) {
    ok('gorget touches ONLY the neck + the collar lip (a real new zone, not a chestplate recolor)', [...parts(gor)].every(pt => pt === 3 || pt === 4));
    ok('gorget sits right at the collar, not down the chest', rows(gor).every(y => y <= 17));
  }
  if (apBib && apHalf) {
    ok('half apron renders', Object.keys(apHalf).length > 0);
    ok('half apron drops ALL torso coverage (no bib) -- the bib apron keeps it', !parts(apHalf).has(4) && parts(apBib).has(4));
    const topRow = (o) => Math.min.apply(null, Object.keys(o).map(k => ((+k) / 56) | 0));
    ok('half apron starts lower than the bib apron (a genuinely shorter garment)', topRow(apHalf) > topRow(apBib));
  }
  if (coBib && coShort) {
    ok('shortalls renders', Object.keys(coShort).length > 0);
    ok('shortalls drop the feet entirely (no ankle cuffs) -- the full-length bib overalls keep them', !parts(coShort).has(11) && !parts(coShort).has(12) && (parts(coBib).has(11) || parts(coBib).has(12)));
    const botRow = (o) => Math.max.apply(null, Object.keys(o).map(k => ((+k) / 56) | 0));
    ok('shortalls hem stops at the knee, well short of the full-length ankle hem', botRow(coShort) < botRow(coBib) - 6);
  }
