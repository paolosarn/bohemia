...
const B=path.join(__dirname,'../engine/BOHEMIA_GRAPHICS_ENGINE_MASTER_7_16_26.js');
const resolve=n=>['../engine/','./','../'].map(d=>path.join(__dirname,d,n)).find(q=>fs.existsSync(q));
...
if(fs.existsSync(B)){
  ...
  while((m=rx.exec(src))){
    ...
    if(rp){
      const h=cr.createHash('md5').update(fs.readFileSync(rp)).digest('hex');
