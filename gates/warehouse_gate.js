// WAREHOUSE / FLEX PARK GATE (7/22/26). A dead multi-tenant flex/light-industrial park — two
// rows of party-wall tenant bays with dock doors + office fronts on an internal drive aisle,
// shared truck court, some burned. Content-dominant (WALKABLE-LAND). Research-first.
const D = require('../engine/bohemia_warehouse.js'); const K = require('../engine/bohemia_district_kit.js');
...
for(const cfg of CONFIGS)for(let s=1;s<=3;s++){const r=D.generate(s*19+4,{streets:cfg}),t=counts(r),g=r.g,W=g[0].length,H=g.length;
 if(!(t[2]>1500 && (t[7]||0)>80 && (t[6]||0)>5 && t[1]>1800 && (t[12]||0)>400 && t[4]>7500))anatomy=false;
...
ok('tenant units + office bays + dock doors + fence + streets',anatomy);
...
ok('warehouse registered + industrial',!!K.get('warehouse')&&K.category('warehouse')==='industrial');
ok('tenant unit enterable + footprints (party-wall bays, one per unit)',D.generate(7,{streets:['S']}).footprints.length>=10&&/office/i.test(D.legend[2].enter||''));
ok('office(7) building, dock(6) portal, street(1) drive, fence(12) solid',
  D.legend[7].kind==='building'&&D.legend[6].kind==='portal'&&D.legend[1].kind==='drive'&&K.tileLayer(D.legend[12]).solid===true);
ok('deterministic',JSON.stringify(D.generate(70,{streets:['S']}).g)===JSON.stringify(D.generate(70,{streets:['S']}).g));
console.log('WAREHOUSE GATE: '+pass+' passed, '+fail+' failed  ('+CONFIGS.length+' configs)'); process.exit(fail?1:0);
